...
Issues:
  ...
  - pytest configured but Sophia tests mixed in
...
# Run tests excluding Sophia
pytest tests/ -v -k "not sophia" --cov=app --cov-report=xml --cov-fail-under=80

# Quick test run
pytest -q -k "not sophia"
...
### Environment Setup
- [ ] All API keys in ~/.config/sophia/env
- [ ] JWT_SECRET rotated and secured
- [ ] Database credentials in secure vault
- [ ] SSL certificates configured

### Testing
- [ ] Unit test coverage > 80% (excluding Sophia)
...
### Step 2: Run Tests
```bash
# Full test suite excluding Sophia
pytest tests/ -v -k "not sophia" --cov=app --cov-fail-under=80
...
1. **No test coverage metrics** - Run `pytest -k "not sophia" --cov`
2. **Secrets in plaintext** - Move to ~/.config/sophia/env minimum
...
jobs:
  test:
    ...
    steps:
      ...
      - name: Run tests
        run: |
          pytest tests/ -v -k "not sophia" --cov=app --cov-report=xml
...
```bash
make env.doctor
# Move all keys to ~/.config/sophia/env
```
...
```ini
[pytest]
addopts = -rA -k "not sophia"
filterwarnings = ignore::DeprecationWarning
markers =
    smoke: Quick smoke tests
    sophia: Sophia-specific tests (skip in Sophia)
```

**RECOMMENDATION:** Do not deploy to production until:
1. Test coverage verified > 80% (excluding Sophia)
