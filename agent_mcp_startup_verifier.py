...
class AgentMCPStartupVerifier:
    ...
    def discover_components(self) -> Dict:
        ...
        mcp_server_dirs = ["mcp_servers", "mcp_memory_server", "mcp_memory"]
        discovered_mcp = {}
        ...
        # Discover agents
        agent_dirs = ["backend/agents", "services", "backend/orchestration"]
        discovered_agents = {}
        ...
        # Discover services
        service_dirs = ["backend/routers", "backend/services"]
        discovered_services = {}
        ...
        return {
            ...
            "total_components": len(discovered_mcp)
        ...
        try:
            ...
            if "service" in agent_name.lower() or "router" in agent_name.lower():
                ...
                cmd = [
                    ...
                    "-m",
                    "uvicorn",
                    f"{Path(agent_path).stem}:app",
                    "--host",
                    "0.0.0.0",
                    "--port",
                    str(8000 + len(self.running_processes)),
                ]
            process = await asyncio.create_subprocess_exec(
                *cmd,
                cwd=agent_dir,
    ...
    def create_missing_component_dirs(self) -> bool:
        ...
        try:
            dirs_to_create = ["mcp_servers", "backend/agents", "backend/services"]
    ...
    async def fix_mcp_server_issues(self) -> bool:
        ...
        try:
            ...
            subprocess.run(
                [sys.executable, "-m", "pip", "install", "mcp-server"],
                check=True,
                capture_output=True,
            )
            return True
    ...
    def generate_startup_report(self) -> Dict:
        ...
        successful_steps = sum(
            1
            for result in self.startup_results.values()
        ...
        report = {
            ...
            "summary": {
                ...
                "startup_rate": (
                    f"{(running_components/total_discovered)*100:.1f}%"
            ...
            "running_processes": [
                {"name": proc["name"], "type": proc["type"], "pid": proc["pid"]}
                for proc in self.running_processes
            ],
            "step_results": self.startup_results,
            "status": (
                "SUCCESS"
                if successful_steps == total_steps and running_components > 0
...
async def main():
    ...
    try:
        ...
        print(
            f"Components: {report['summary']['running_components']}/{report['summary']['total_discovered']} ({report['summary']['startup_rate']})"
        )
