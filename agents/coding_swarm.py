...
from mcp_servers.ai_router import AIRouter, TaskType
    ...
    async def process(self, state: CodingSwarmState) -> Dict[str, Any]:
        ...
        try:
            ...
            for file_path in files_to_modify:
                try:
                    file_data = await self.github_tools.read_file_content(
                        state["repository"], file_path
                    )
...
"""
                    ...
                    try:
                        ...
                        code_changes["implementation_notes"].extend(
                            implementation.get("implementation_notes", [])
                        )
        ...
        try:
            ...
            for file_path, file_changes in modified_files.items():
                try:
                    ...
                    syntax_validation = await self.diff_analyzer.validate_code_syntax(
                        modified_content, file_extension
                    )
...
"""
                    ...
                    try:
                        ...
                        review_results["recommendations"].extend(
                            file_review.get("recommendations", [])
                        )
            ...
            if total_files > 0:
                avg_quality = sum(
                    review["quality_score"] for review in review_results["file_reviews"].values()
                ) / total_files
            ...
            review_results["review_summary"] = f"Reviewed {total_files} files. Overall quality: {review_results['overall_quality']}. Issues found: {total_issues}."
            ...
            return {
                "errors": state["errors"] + [f"Review error: {e}"],
                "current_agent": "Integrator"
            }
    ...
    async def process(self, state: CodingSwarmState) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                branch_result = await self.github_tools.create_branch(
                    state["repository"], branch_name
                )
                ...
                for file_path, file_data in modified_files.items():
                    try:
                        ...
                        # Commit the file
                        commit_result = await self.github_tools.commit_file_change(
                            repository=state["repository"],
                            file_path=file_path,
...
"""
                ...
                integration_results["integration_summary"] = f"Successfully integrated {len(integration_results['files_committed'])} files into branch {branch_name} and created PR #{integration_results.get('pr_number', 'N/A')}"
            ...
            return {
                "errors": state["errors"] + [f"Integration error: {e}"],
                "integration_complete": True
            }
...
class CodingSwarm:
    ...
    async def execute_coding_task(self, 
                                 task_description: str,
                                 repository: str,
