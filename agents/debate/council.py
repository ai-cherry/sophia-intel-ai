...
"""
        ...
        return {"agent_id": agent_id, "role": member["role"], "analysis": response, "timestamp": datetime.now()}
...
"""
        ...
        return {"cross_examination": response, "timestamp": datetime.now()}

    async def _get_final_vote(
        self, agent_id: str, proposal: Proposal, initial_analysis: Dict[str, Any], cross_exam: Dict[str, Any]
    ) -> Vote:
    ...
    def get_session_summary(self, session_id: str) -> Dict[str, Any]:
        ...
        for vote in session.votes:
            ...
            vote_summary[vote_type].append(
                {"agent": vote.agent_id, "reasoning": vote.reasoning, "confidence": vote.confidence}
            )
        ...
        return {
            ...
            "duration_minutes": (
                (session.ended_at - session.started_at).total_seconds() / 60 if session.ended_at else None
            ),
