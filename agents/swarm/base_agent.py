...
class BaseAgent(ABC):
    ...
    def __init__(
        ...
        self.metrics = {"tasks_completed": 0, "tasks_failed": 0, "average_completion_time": 0.0, "success_rate": 0.0}
    ...
    async def process_tasks(self) -> None:
        ...
        pending_tasks = [task for task in self.current_tasks.values() if task.status == TaskStatus.PENDING]
    ...
    async def communicate_with_ai(
        self, prompt: str, task_type: str = "code_generation", context: Optional[Dict[str, Any]] = None
    ) -> str:
        """
        ...
        request_data = {
            ...
            "prompt": prompt,
            "context": context or {},
            "preferences": {"cost_preference": "balanced", "quality_requirement": "high", "max_latency_ms": 10000},
            "metadata": {"agent_type": self.agent_type.value, "agent_name": self.name},
        }

        try:
            async with self.session.post(
                self.ai_router_url, json=request_data, timeout=aiohttp.ClientTimeout(total=30)
            ) as response:
    ...
    async def receive_message(self, sender: str, message: Dict[str, Any]) -> None:
        ...
        Args:
            ...
            message: Message content
        """
        self.message_queue.append({"sender": sender, "message": message, "timestamp": datetime.utcnow()})
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "capabilities": [
                {"name": cap.name, "description": cap.description, "confidence": cap.confidence_score}
