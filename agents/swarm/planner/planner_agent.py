...
class PlannerAgent(BaseAgent):
    ...
    async def _decompose_complex_task(self, task: AgentTask) -> Dict[str, Any]:
        ...
        ai_response = await self.communicate_with_ai(prompt=decomposition_prompt, task_type="analysis", context=context)
    ...
    async def _analyze_dependencies(self, task: AgentTask) -> Dict[str, Any]:
        ...
        ai_response = await self.communicate_with_ai(
            prompt=dependency_prompt, task_type="analysis", context={"tasks": tasks}
        )
    ...
    async def _estimate_resources(self, task: AgentTask) -> Dict[str, Any]:
        ...
        ai_response = await self.communicate_with_ai(
            prompt=estimation_prompt, task_type="analysis", context={"tasks": tasks}
        )
    ...
    async def _structure_text_plan(self, text_response: str) -> Dict[str, Any]:
        ...
        return {"objective": "Development mission", "phases": [], "tasks": [], "raw_response": text_response}
    ...
    async def _create_timeline(self, subtasks: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        timeline = {"start_date": start_date.isoformat(), "phases": [], "milestones": []}
    ...
    def _identify_critical_path(self, subtasks: List[Dict[str, Any]]) -> List[str]:
        ...
        critical_tasks = [
            task["id"] for task in subtasks if task.get("priority") == "high" or len(task.get("dependencies", [])) > 0
        ]
    ...
    async def _create_fallback_plan(self, mission: str, requirements: Dict[str, Any]) -> Dict[str, Any]:
