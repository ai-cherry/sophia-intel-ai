...
from .swarm.swarm_orchestrator import Priority, SwarmOrchestrator
...
async def start_mission():
    ...
    try:
        ...
        # Start mission
        mission_id = await swarm.start_mission(description=description, requirements=requirements, priority=priority)

        return jsonify({"mission_id": mission_id, "status": "queued", "message": "Mission started successfully"}), 201
...
async def cancel_mission(mission_id: str):
    ...
    try:
        ...
        if success:
            return jsonify(
                {"mission_id": mission_id, "status": "cancelled", "message": "Mission cancelled successfully"}
            )
        else:
...
async def list_missions():
    ...
    try:
        ...
        return jsonify({"missions": missions[:limit], "total": len(missions), "filtered": status_filter is not None})
...
async def demo_mission():
    ...
    try:
        ...
        mission_id = await swarm.start_mission(
            description=demo_description, requirements=demo_requirements, priority=Priority.HIGH
        )
