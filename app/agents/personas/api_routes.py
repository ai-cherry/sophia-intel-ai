...
class PersonaManager:
    ...
    async def interact_with_persona(self, persona_id: str, message: str, context: Dict[str, Any]) -> str:
    ...
    def process_feedback(self, persona_id: str, interaction_id: str, rating: int, feedback_text: str) -> bool:
...
async def initialize_persona(persona_id: str):
    ...
    try:
        ...
        if success:
            ...
            raise HTTPException(status_code=400, detail=f"Failed to initialize persona {persona_id}")
    ...
    try:
        ...
        while True:
            ...
            if persona_manager.has_persona(persona_id):
                ...
                await websocket.send_json({
                    "type": "error",
                    "message": f"Persona {persona_id} not available"
                })
...
async def get_learning_progress(persona_id: str):
    ...
    return {
        ...
        "learning_metrics": {
            ...
            "adaptation_rate": sum(persona.pattern_confidence.values()) / max(len(persona.pattern_confidence), 1)
        }
    }
