...
class SalesCoachAgent(BasePersonaAgent):
    ...
    async def _provide_deal_coaching(
        ...
        relevant_memories = self.retrieve_memories(
            f"deal coaching {deal_stage} {user_input}",
            limit=5
        )
    ...
    async def _provide_motivation(
        ...
        relevant_memories = self.retrieve_memories(
            f"motivation {motivation_type} {current_mood}",
            limit=3
        )
    ...
    async def _assess_rep_skills(self, rep_id: str, context: Dict[str, Any]) -> SalesSkillAssessment:
    ...
    async def _create_skill_development_plan(
        self, 
        assessment: SalesSkillAssessment, 
        skill_area: str, 
        input_text: str
    ) -> List[str]:
