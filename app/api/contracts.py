...
class ChatRequestV1(VersionedRequest):
    ...
    client_id: str | None = Field(default=None, max_length=100, description="Client identifier")
    user_context: dict[str, Any] | None = Field(default_factory=dict, description="User context data")
...
class StreamTokenV1(BaseModel):
    ...
    metadata: dict[str, Any] | None = Field(default_factory=dict, description="Token metadata")
...
class ChatRequestV2(ChatRequestV1):
    ...
    optimization_mode: OptimizationMode | None = Field(default=OptimizationMode.AUTO, description="Optimization mode")
    swarm_type: SwarmType | None = Field(default=None, description="Specific swarm type to use")
    use_memory: bool = Field(default=True, description="Enable memory context")
    max_response_tokens: int | None = Field(default=None, ge=1, le=100000, description="Max response tokens")
    temperature: float | None = Field(default=None, ge=0, le=2, description="LLM temperature")
    optimization_hints: dict[str, Any] | None = Field(default_factory=dict, description="Optimization hints")

class ChatResponseV2(ChatResponseV1):
    """Version 2 chat response with enhanced features"""
    api_version: APIVersion = APIVersion.V2
    manager_response: str | None = Field(default=None, description="AI manager response")
    intent: str | None = Field(default=None, description="Detected intent")
    confidence: float | None = Field(default=None, ge=0, le=1, description="Intent confidence")
    swarm_used: str | None = Field(default=None, description="Swarm type used")
...
class WebSocketMessage(BaseModel):
    ...
    timestamp: str | None = Field(default_factory=lambda: datetime.utcnow().isoformat())
...
class WebSocketChatData(BaseModel):
    ...
    swarm_type: str | None = Field(default=None)
    optimization_mode: str | None = Field(default="balanced")
...
class ComponentHealth(BaseModel):
    ...
    details: dict[str, Any] | None = Field(default_factory=dict, description="Health details")
...
class ChatOrchestratorInterface(Protocol):
    ...
    async def handle_chat(self, request: ChatRequestV1 | ChatRequestV2) -> ChatResponseV1 | ChatResponseV2:
        """Handle chat request"""
        ...
...
class APIContractFactory:
    ...
    @staticmethod
    def create_chat_request(data: dict[str, Any]) -> ChatRequestV1 | ChatRequestV2:
        """
    ...
    def create_chat_response(
        ...
        if version == APIVersion.V1:
            ...
            v1_data = {
                ...
                if k not in ["manager_response", "intent", "confidence", "swarm_used", "tokens_used"]
            }
...
class BackwardCompatibilityAdapter:
    ...
    def adapt_request(self, raw_data: dict[str, Any]) -> ChatRequestV1 | ChatRequestV2:
        """
...
class RequestValidationMiddleware:
    ...
    async def validate_request(self, request_data: dict[str, Any]) -> ChatRequestV1 | ChatRequestV2:
        """
    ...
    async def validate_response(
        self,
        request: ChatRequestV1 | ChatRequestV2,
        response_data: dict[str, Any]
    ) -> ChatResponseV1 | ChatResponseV2:
        """
