...
def create_api_response(success: bool, data: Any = None, error: Optional[str] = None) -> APIResponse:
...
class ChatRequestV2(ChatRequestV1):
    ...
    optimization_mode: Optional[OptimizationMode] = Field(default=OptimizationMode.AUTO, description="Optimization mode")
    ...
    max_response_tokens: Optional[int] = Field(default=None, ge=1, le=100000, description="Max response tokens")
    temperature: Optional[float] = Field(default=None, ge=0, le=2, description="LLM temperature")
    optimization_hints: Optional[dict[str, Any]] = Field(default_factory=dict, description="Optimization hints")
...
class RequestValidationMiddleware:
    ...
    async def validate_request(self, request_data: dict[str, Any]) -> Union[ChatRequestV1, ChatRequestV2]:
        """
