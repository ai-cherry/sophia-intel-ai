...
class GlobalState:
    ...
    def _load_and_validate_config(self):
        ...
        try:
            ...
            try:
                loaded_from = (
                    self.config.get("loaded_from")
                    if isinstance(self.config, dict)
                    else getattr(self.config, "loaded_from", None)
                )
                if loaded_from:
                    logger.info(f"✅ Configuration loaded from: {loaded_from}")
            except Exception:
                pass
            # Check validation status (optional)
            if self.validation and self.validation.get("overall_status") == "unhealthy":
                logger.warning("⚠️  Configuration validation failed")
                for issue in (self.validation.get("critical_issues", []) if self.validation else []):
                    logger.error(f"❌ {issue}")
                env_name = (
                    self.config.get("environment_name") if isinstance(self.config, dict) else getattr(self.config, "environment_name", None)
                )
                if env_name == "prod":
...
async def get_redis_client(state: GlobalState = Depends(get_state)):
    ...
    if not state.redis_client:
        try:
            ...
            redis_url = (
                config.get("redis_url") if isinstance(config, dict) else getattr(config, "redis_url", None)
            )
            if not redis_url:
            ...
            try:
                host = (
                    config.get("redis_host") if isinstance(config, dict) else getattr(config, "redis_host", "")
                )
                port = (
                    config.get("redis_port") if isinstance(config, dict) else getattr(config, "redis_port", "")
                )
                if host or port:
                    logger.info(f"✅ Redis connection established to {host}:{port}")
            except Exception:
                pass
        except Exception as e:
            logger.error(f"❌ Failed to connect to Redis: {e}")
            env_name = (
                config.get("environment_name") if isinstance(config, dict) else getattr(config, "environment_name", None)
            )
            if config and env_name == "prod":
...
def get_weaviate_client(state: GlobalState = Depends(get_state)):
    ...
    if not state.weaviate_client:
        try:
            ...
            weaviate_url = config.get("weaviate_url") if isinstance(config, dict) else getattr(config, "weaviate_url", None)
            weaviate_api_key = config.get("weaviate_api_key") if isinstance(config, dict) else getattr(config, "weaviate_api_key", None)
