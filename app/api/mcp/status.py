...
class MCPStatusAPI:
    ...
    def __init__(self):
        ...
        # Orchestrator dependencies are intentionally omitted here to avoid
        # tight coupling. We rely on the registry and lightweight health probes.
    ...
    async def _get_core_status(self) -> dict[str, Any]:
        """Get core domain server status"""
        servers = self.registry.get_servers_for_domain(MemoryDomain.CODE)
        server_status = []
...
async def get_all_domains_status() -> dict[str, Any]:
    ...
    try:
        _status = await mcp_api.get_domain_server_status("")
        ...
        return {
            ...
            "summary": {
                ...
                "operational_servers": sum(
                    [
                        _status["domain_metrics"]["operational_servers"],
                ...
                "total_connections": sum(
                    [
                        _status["domain_metrics"]["total_connections"],
...
async def get_server_status(server_name: str) -> dict[str, Any]:
    ...
    try:
        ...
        allocation = None
        for domain in [MemoryDomain.CODE, MemoryDomain.SOPHIA, MemoryDomain.SHARED]:
...
async def get_metrics_summary() -> dict[str, Any]:
    ...
    try:
        _metrics = await mcp_api._calculate_domain_metrics(MemoryDomain.CODE)
