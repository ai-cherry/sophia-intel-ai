...
class MCPRequest(BaseModel):
    ...
    domain: Optional[str] = Field(
        None, description="Preferred domain (, sophia, shared)"
    )
...
async def mcp_git(request: MCPRequest) -> JSONResponse:
    ...
    try:
        orchestrator = await get_orchestrator()
        # Git operations typically belong to core domain
        domain = MCPDomain.CORE
        if request.domain:
    ...
    try:
        ...
        return JSONResponse(
            content={
                ...
                "domains": {
                    "": "Code analysis, development tools, git operations",
