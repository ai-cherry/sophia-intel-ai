...
class PortkeyLoadBalancer:
    ...
    def __init__(self):
        ...
        if not _HAS_PORTKEY:
            raise ImportError("portkey_ai not installed; skipping Portkey load balancer")
...
def initialize_portkey_balancer():
    ...
    if not portkey_balancer:
        if not _HAS_PORTKEY:
            logger.warning("portkey_ai not available; Portkey balancer disabled for this run")
            return None
        portkey_balancer = PortkeyLoadBalancer()
    return portkey_balancer
# Auto-initialize if environment variables are set and Portkey is available
if _HAS_PORTKEY and os.getenv("PORTKEY_API_KEY") and os.getenv("OPENROUTER_API_KEY"):
    try:
        portkey_balancer = PortkeyLoadBalancer()
