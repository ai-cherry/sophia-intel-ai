...
async def deploy_swarm() -> Dict[str, Any]:
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to deploy research swarm: {str(e)}"
        )
...
async def get_research_status() -> Dict[str, Any]:
    ...
    if not research_status["deployed"]:
        return {
            "status": "not_deployed",
            "message": "Research swarm not deployed"
        }
...
async def approve_implementation(
    ...
    if not research_swarm:
        raise HTTPException(
            status_code=400,
            detail="Research swarm not deployed"
        )
...
async def test_improvements(
    ...
    if not research_results:
        raise HTTPException(
            status_code=400,
            detail="No improvements to test. Complete research first."
        )
...
async def get_factory_integration_status() -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            ...
            "agents_available": list(research_swarm.research_agents.keys()) if research_swarm else []
        }
    except:
        return {
            "integrated": False,
            "error": "Could not check factory integration"
        }
