...
async def _get_server_health_status(
    ...
    try:
        ...
        # Check  domain
        _allocations = mcp_registry.get_servers_for_domain(MemoryDomain.CODE)
        if any(alloc.server_name == server_name for alloc in _allocations):
            domain = ""
            business_context = _get__business_context(server_name)
        # Check Sophia domain
...
async def _get_server_performance_metrics(server_name: str) -> dict[str, Any]:
    ...
    return base_metrics
def _get__business_context(server_name: str) -> str:
    """Get business context for  servers"""
    context_map = {
        "_filesystem": "code_repository_management",
        "_code_analysis": "technical_debt_monitoring",
        "_design": "architecture_documentation",
...
def _get_mythology_agents_status() -> list[dict[str, Any]]:
    ...
    return [
        ...
        },
        {
            ...
            "assigned_mcp_servers": ["_code_analysis", "_design"],
            "status": "operational",
        ...
        },
        {
            ...
            "assigned_mcp_servers": ["_filesystem", "shared_indexing"],
            "status": "operational",
            "primary_metric": {
                "label": "System Health",
                "value": "99.8%",
...
async def get_domain_status(
    ...
    Args:
        domain: Domain to query (, sophia, shared)
    """
    try:
        if domain not in ["", "sophia", "shared"]:
