...
Ensures technical domain handles only repository/code operations
...
class DomainEnforcer:
    """
    Enforces strict domain boundaries between technical and business domains
    ...
    def _initialize_access_matrix(
        ...
        matrix[(UserRole.ADMIN, MemoryDomain.CODE)] = AccessLevel.FULL
        matrix[(UserRole.ADMIN, MemoryDomain.SOPHIA)] = AccessLevel.FULL
        # Developer roles for 
        matrix[(UserRole.DEVELOPER, MemoryDomain.CODE)] = AccessLevel.FULL
        matrix[(UserRole.DEVOPS, MemoryDomain.CODE)] = AccessLevel.FULL
        matrix[(UserRole.ARCHITECT, MemoryDomain.CODE)] = AccessLevel.FULL
        ...
        matrix[(UserRole.BUSINESS_ANALYST, MemoryDomain.CODE)] = AccessLevel.READ
        matrix[(UserRole.DEVELOPER, MemoryDomain.SOPHIA)] = AccessLevel.READ
        # Readonly has read access to both
        matrix[(UserRole.READONLY, MemoryDomain.CODE)] = AccessLevel.READ
    ...
    def _check_resource_restrictions(self, request: DomainRequest) -> list[str]:
        ...
        if request.resource_path:
            #  path restrictions
            if request.target_domain == MemoryDomain.CODE:
        ...
        return {
            "_domain": {
                ...
                "allowed_roles": [
                    ...
                    if self._get_user_access(role, MemoryDomain.CODE)
                    != AccessLevel.NONE
                ],
            },
            "sophia_domain": {
...
def get_domain_access_summary(user_role: UserRole) -> dict[str, Any]:
    ...
    """
    _access = domain_enforcer._get_user_access(user_role, MemoryDomain.CODE)
    sophia_access = domain_enforcer._get_user_access(user_role, MemoryDomain.SOPHIA)
    summary = {
        "role": user_role.value,
        "_access": _access.value,
        "sophia_access": sophia_access.value,
        "allowed_operations": [],
    }
    # Add allowed operations based on access level
    if _access != AccessLevel.NONE:
        for op in domain_enforcer._operations:
            required = domain_enforcer._get_required_access_level(op)
            if domain_enforcer._has_sufficient_access(_access, required):
    ...
    if _access != AccessLevel.NONE or sophia_access != AccessLevel.NONE:
