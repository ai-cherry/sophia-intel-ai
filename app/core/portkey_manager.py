...
class PortkeyManager:
    ...
    def __init__(self, config_path: Optional[Path] = None):
        ...
        if not _HAS_PORTKEY or not self.portkey_api_key:
            self._disabled = True
            logger.warning(
                "Portkey disabled (missing package or API key). Functions will raise when used."
            )
    ...
    def get_client(self, provider: str) -> Portkey:
        ...
        """
        if self._disabled:
            raise RuntimeError("Portkey is disabled; missing portkey_ai or PORTKEY_API_KEY")
