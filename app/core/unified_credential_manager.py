...
class UnifiedCredentialManager:
    ...
    def __init__(self, config_path: Optional[Path] = None):
        self.config_path = config_path or Path(__file__).parent.parent.parent / "config" / "credentials.enc"
    ...
    def get_redis_config(self) -> Dict[str, Any]:
        ...
        if not redis_config["url"]:
            if redis_config["password"]:
                redis_config["url"] = f"redis://:{redis_config['password']}@{redis_config['host']}:{redis_config['port']}/{redis_config['db']}"
            else:
                redis_config["url"] = f"redis://{redis_config['host']}:{redis_config['port']}/{redis_config['db']}"
