...
class WebSocketManager:
    ...
    async def handle_client_message(
        ...
        try:
            ...
            elif msg_type == "_operation":
                return await self._handle__operation(client_id, message)
    ...
    def _get_message_domain(self, message_type: str) -> DomainType:
        ...
        elif "" in message_type or "tactical" in message_type:
            return DomainType._TACTICAL
    ...
    async def _handle__operation(
        ...
        if not connection or not connection.user:
            return {
                "type": "error",
                "message": "Authentication required for  operations",
            }
        # Check  access
        has_permission = await self.authenticator.check_permission(
            connection.user, "view__operations"
        )
        if not has_permission:
            return {"type": "error", "message": "Insufficient  permissions"}
        return {
