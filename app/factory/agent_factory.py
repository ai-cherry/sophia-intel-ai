...
from app.swarms.core.swarm_base import SwarmType, SwarmExecutionMode, SwarmConfig, SwarmMetrics, SwarmCapability
...
class AgentFactory:
    ...
    def _initialize_comprehensive_catalog(self):
        ...
        logger.info(f"🏭 Comprehensive Agent Factory initialized with {len(self.inventory)} agents in catalog")
    ...
    def _create_default_comprehensive_catalog(self):
        ...
        try:
            ...
            logger.info(f"📚 Created comprehensive catalog with {len(catalog_agents)} specialized agents")
    ...
    def _create_basic_fallback_catalog(self):
        ...
        basic_agents = [
            self._create_basic_agent("developer", AgentSpecialty.DEVELOPER, [AgentCapability.CODING]),
            self._create_basic_agent("analyst", AgentSpecialty.ANALYST, [AgentCapability.DATA_ANALYSIS]),
    ...
    def _create_basic_agent(self, name: str, specialty: AgentSpecialty, capabilities: List[AgentCapability]) -> AgentBlueprint:
    ...
    def create_agent_from_blueprint(self, blueprint_id: str, custom_config: Optional[Dict[str, Any]] = None) -> str:
    ...
    def list_agent_blueprints(self, filters: Optional[Dict[str, Any]] = None) -> List[Dict[str, Any]]:
        ...
        if filters:
            ...
            for agent in agents:
                ...
                if 'capabilities' in filters:
                    ...
                    if not all(any(cap.value == req_cap for cap in agent.capabilities) 
    ...
    def search_agent_blueprints(self, query: str) -> List[Dict[str, Any]]:
        ...
        for agent in self.inventory.values():
            ...
            if (query in agent.metadata.name.lower() or 
                query in agent.metadata.description.lower()):
    ...
    def create_swarm_from_inventory(self, swarm_config: Dict[str, Any]) -> str:
        ...
        for capability in required_capabilities:
            ...
            agents = [a for a in self.inventory.values() 
        ...
        for blueprint in selected_blueprints:
            agent_def = AgentDefinition(
                ...
                model=blueprint.config.model_settings.model if hasattr(blueprint.config.model_settings, 'model') else "openai/gpt-4o-mini",
        ...
        logger.info(f"🐝 Created inventory-based swarm '{swarm_id}' with {len(selected_blueprints)} agents")
    ...
    def create_swarm_from_template(self, template_id: str, overrides: Optional[Dict[str, Any]] = None) -> str:
    ...
    def get_inventory_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "most_popular_specialties": sorted(specialty_counts.items(), key=lambda x: x[1], reverse=True)[:5],
            "most_common_capabilities": sorted(capability_counts.items(), key=lambda x: x[1], reverse=True)[:10]
        }
    ...
    def _apply_custom_config_to_blueprint(self, blueprint: AgentBlueprint, custom_config: Dict[str, Any]) -> AgentBlueprint:
...
async def get_agent_blueprint(blueprint_id: str):
    ...
    try:
        ...
        if not blueprint:
            raise HTTPException(status_code=404, detail=f"Agent blueprint '{blueprint_id}' not found")
...
async def get_available_capabilities():
    ...
    try:
        ...
        for capability in AgentCapability:
            ...
            agents_count = len([a for a in factory.inventory.values() if capability in a.capabilities])
    ...
    try:
        ...
        if swarm_id in factory.created_swarms:
            ...
            for agent_def in blueprint.agents:
                blueprint_id = agent_def.metadata.get('blueprint_id') if hasattr(agent_def, 'metadata') else None
