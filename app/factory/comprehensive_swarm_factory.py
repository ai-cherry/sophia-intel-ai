...
class SwarmFactoryConfig:
    ...
    # Memory and context
    memory_domain: MemoryDomain = MemoryDomain.CODEGENERAL
...
class ComprehensiveSwarmFactory:
    ...
    def _initialize_preconfigured_swarms(self):
        ...
        self.swarm_configs["_recon"] = SwarmFactoryConfig(
            name="Technical Reconnaissance Battalion",
            swarm_type=SwarmType.MILITARY_RECON,
            coordination_pattern=CoordinationPattern.PARALLEL,
            max_cost_per_execution=1.5,
            memory_domain=MemoryDomain.CODE,
            slack_delivery_config=create_technical_delivery_config("#-ops"),
            auto_deliver_results=True,
            tags=["technical", "recon", "pathfinders", "scanning", "intelligence"],
        )
        # Military Strike Force
        self.swarm_configs["_strike"] = SwarmFactoryConfig(
            name="Technical Coding Strike Force",
            ...
            slack_delivery_config=create_technical_delivery_config("#-ops"),
            auto_deliver_results=True,
            tags=["technical", "strike", "operators", "coding", "implementation"],
        )
        ...
        self.swarm_configs["hybrid_tactical"] = SwarmFactoryConfig(
            ...
            memory_domain=MemoryDomain.CODEGENERAL,
            ...
            auto_deliver_results=True,
            tags=["hybrid", "tactical", "analysis", "sophia", ""],
        )
    ...
    async def _create_military_swarm(
        ...
        # Build a minimal technical swarm without legacy  dependencies
        # Compose a few technical agent profiles suitable for the requested type
        from app.swarms.core.micro_swarm_base import AgentProfile
        if config.swarm_type == SwarmType.MILITARY_RECON:
            agents = [
                AgentProfile(
                    role=AgentRole.ANALYST,
                    name="TECH-RECON - Intelligence Analyst",
                    description="Rapid technical reconnaissance and codebase scanning",
                    model_preferences=["deepseek-coder", "gpt-4"],
                    specializations=["code_analysis", "architecture_review", "security_scan"],
                    reasoning_style="Fast, precise analysis for technical reconnaissance",
                    confidence_threshold=0.85,
                    max_tokens=6000,
                    temperature=0.2,
                )
            ]
        elif config.swarm_type == SwarmType.MILITARY_STRIKE:
            agents = [
                AgentProfile(
                    role=AgentRole.STRATEGIST,
                    name="TECH-STRIKE - Implementation Strategist",
                    description="Guides coding execution with emphasis on quality and speed",
                    model_preferences=["gpt-4", "claude-3-sonnet"],
                    specializations=["implementation_planning", "refactoring", "code_quality"],
                    reasoning_style="Decisive, high-signal guidance for coding operations",
                    confidence_threshold=0.85,
                    max_tokens=6000,
                    temperature=0.25,
                )
            ...
            agents = [
                AgentProfile(
                    role=AgentRole.STRATEGIST,
                    name="TECH-COMMAND - Technical Orchestrator",
                    description="Coordinates technical tasks with reliability focus",
                    model_preferences=["gpt-4o"],
                    specializations=["coordination", "risk_management"],
                    reasoning_style="Balanced coordination with risk-aware decision-making",
                    confidence_threshold=0.85,
                    max_tokens=6000,
                    temperature=0.2,
                )
...
async def create_tactical_recon_swarm() -> str:
    """Quick create technical tactical reconnaissance swarm"""
    factory = get_comprehensive_factory()
    return await factory.create_swarm(factory.swarm_configs["_recon"])
...
async def create_coding_strike_swarm() -> str:
    """Quick create technical coding strike force swarm"""
    factory = get_comprehensive_factory()
    return await factory.create_swarm(factory.swarm_configs["_strike"])
