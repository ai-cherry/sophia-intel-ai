...
class SwarmIntelligence:
    ...
    async def evolve_strategy(self, tool: str, performance_data: List[Dict]) -> Dict:
        ...
        # Select winners
        winners = sorted(results, key=lambda x: x.get("fitness", 0), reverse=True)[:3]
        # Update gene pool
    ...
    async def _shadow_test(self, mutations: List[Dict]) -> List[Dict]:
        ...
        for mutation in mutations:
            ...
            results.append({**mutation, "fitness": fitness})
        return results
...
class FactoryMCPSwarm:
    ...
    def _setup_routes(self):
        ...
        async def mcp_call(call: ToolCall, background: BackgroundTasks):
            ...
            with tracer.start_as_current_span(f"mcp.{call.tool}") as span:
                ...
                try:
                    ...
                    return ToolResponse(
                        ...
                        execution_time=execution_time,
                        cache_hit=False,
                        evolution_score=self.evolution_loops.get(
                            call.tool, DroidEvolution(call.tool)
                        ).evolution_score,
                    )
                except Exception as e:
                    execution_time = (time.time() - start_time) * 1000
    ...
    async def _run_droid_evolved(self, params: Dict, context: Dict) -> Dict:
        ...
        evolution.success_rate = sum(
            1 for p in evolution.performance_history[-100:] if p["success"]
        ) / min(len(evolution.performance_history), 100)
        evolution.avg_response_time = np.mean(
            [p["time"] for p in evolution.performance_history[-50:]]
        )
    ...
    def _calculate_swarm_performance(self) -> Dict:
        ...
        return {
            ...
            "avg_success_rate": np.mean(
                [droid.success_rate for droid in self.evolution_loops.values()]
            ),
            "avg_response_time": np.mean(
                [droid.avg_response_time for droid in self.evolution_loops.values()]
            ),
        }
