...
class AdvancedSecretsManager:
        ...
        for service, config in service_configs.items():
            try:
                ...
                success = await self.esc_client.create_environment(
                    name=f"rotated-secrets/{service}", definition=rotation_config
                )
    ...
    async def trigger_emergency_rotation(self, service: str, reason: str) -> dict[str, Any]:
        ...
        if service not in self.secrets:
            return {"status": "failed", "error": f"Service {service} not found"}
...
class MockESCClient:
    ...
    async def rotate_environment(
        self, name: str, reason: str, notify_webhooks: bool = True
    ) -> bool:
...
if __name__ == "__main__":
    ...
    async def test_secrets_manager():
        ...
        emergency_result = await manager.trigger_emergency_rotation(
            "lambda-labs", "Potential compromise detected"
        )
