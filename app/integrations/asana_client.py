...
class AsanaClient:
    ...
    def __init__(self):
        ...
        cfg = cm.get_integration_config("asana")
        # If not explicitly enabled but token is present, treat as enabled
        ...
        if not self.config.get("enabled"):
            raise ValueError("Asana integration not enabled. Set ASANA_ENABLED=true or provide ASANA_API_TOKEN/ASANA_PAT_TOKEN.")
        if not self.api_token:
            raise ValueError("Asana API token not configured (ASANA_API_TOKEN or ASANA_PAT_TOKEN)")
        self.headers = {
