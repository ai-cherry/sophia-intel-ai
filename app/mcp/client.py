...
Ensures proper domain isolation between Sophia and Artemis
"""
...
class MCPBridgeClient:
    ...
    def __init__(self, domain: str = "artemis"):
        """
        Initialize MCP client

        Args:
            domain: Domain context (artemis, sophia, or shared)
        """
        self.domain = domain
        ...
        self.endpoints = {
            "artemis": {
                "memory": f"http://{self.memory_host}:{self.memory_port}/artemis",
                "bridge": f"ws://{self.memory_host}:{self.bridge_port}/artemis",
            },
            "sophia": {
    ...
    async def execute_artemis_task(self, request: Dict[str, Any]) -> Dict[str, Any]:
        """
        Execute task in Artemis domain (for coding tasks)
        ...
        Returns:
            Task result
        """
        if self.domain != "artemis":
            logger.warning("Switching to Artemis domain for coding task")
            self.domain = "artemis"
            self.connected = False
            await self.connect()
        ...
        request["domain"] = "artemis"

        # Execute task (would call actual Artemis service in production)
        result = {
            "task_id": task_id,
            "status": "completed",
            "domain": "artemis",
...
async def get_artemis_client() -> MCPBridgeClient:
    """Get client configured for Artemis domain"""
    client = MCPBridgeClient(domain="artemis")
