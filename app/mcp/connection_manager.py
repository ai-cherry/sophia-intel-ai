...
Centralizes connection management for Sophia (business + technical domains)
"""
import asyncio
import logging
import random
...
class MCPConnectionManager:
    ...
    async def _perform_health_check(self, server_name: str) -> bool:
        """
        Perform actual health check if an endpoint is known via env mapping.
        Falls back to retaining previous status (no random simulation).

        Configure per-server health endpoints via env, e.g.:
        MCP_HEALTH_sophia_web_search=http://localhost:8081/health
        MCP_HEALTH_shared_indexing=http://localhost:8084/health
        """
        import os, time
        try:
            import httpx  # type: ignore
        except Exception:
            # If httpx not available, retain previous status
            return self.health_status.get(server_name, True)
        env_key = f"MCP_HEALTH_{server_name}"
        url = os.getenv(env_key)
        if not url:
            # No mapping; retain previous status
            return self.health_status.get(server_name, True)
        try:
            ...
            async with httpx.AsyncClient(timeout=timeout) as client:
                for attempt in (1, 2):  # 1 retry
                    try:
                        ...
                        self.connection_metrics[server_name]["last_latency_ms"] = latency_ms
            ...
            logger.debug(f"Health probe failed for {server_name}: {e}")
            return False
