...
from app.mcp.server_template import MCPServerBase, validate_required_params, sanitize_params, with_retry
    ...
    async def search_contacts(self, search_term: str, limit: int = 50) -> List[Dict[str, Any]]:
        ...
        payload = {
            ...
            "properties": [
                ...
                "jobtitle", "lastmodifieddate", "createdate", "hs_lead_status"
            ],
            "limit": limit
        }
    ...
    async def get_company(self, company_id: str) -> Dict[str, Any]:
        ...
        params = {
            "properties": [
                ...
                "phone", "city", "state", "country", "createdate", "lastmodifieddate",
        ...
        for deal_id in deal_ids:
            ...
            deal_params = {
                "properties": [
                    ...
                    "dealtype", "leadsource", "createdate", "lastmodifieddate"
                ]
            }
            
            deal_response = await self.session.get(deal_url, headers=self._get_headers(), params=deal_params)
...
class SlackClient:
    ...
    async def search_messages(self, query: str, count: int = 50, sort: str = "timestamp") -> Dict[str, Any]:
        ...
        response = await self.session.get(url, headers=self._get_headers(use_user_token=True), params=params)
        response.raise_for_status()
    ...
    async def get_employee_sentiment(self, employee_user_id: str, days_back: int = 7) -> Dict[str, Any]:
        ...
        sentiment_indicators = {
            'positive': ['great', 'awesome', 'excellent', 'love', 'perfect', 'amazing', '👍', '🎉', '✅'],
            'negative': ['frustrated', 'annoying', 'terrible', 'hate', 'awful', 'broken', '😞', '😤', '❌'],
            'stress': ['urgent', 'asap', 'emergency', 'critical', 'deadline', 'overwhelmed', 'stressed']
        }
    ...
    async def get_client_channel_activity(self, client_name: str, days_back: int = 30) -> Dict[str, Any]:
        ...
        for channel in channels.get('channels', []):
            ...
            if (client_name_lower in channel_name or 
                any(word in channel_name for word in client_name_lower.split())):
        ...
        for channel in client_channels:
            ...
            urgency_keywords = ['urgent', 'asap', 'emergency', 'critical', 'issue', 'problem', 'down', 'broken']
            channel_urgency = 0
...
class GongClient:
    ...
    def __init__(self):
        ...
        self.secret = os.getenv('GONG_CLIENT_SECRET', 'eyJhbGciOiJIUzI1NiJ9.eyJleHAiOjIwNTQxNTA4ODUsImFjY2Vzc0tZXkiOiJUVjMzQlBaNVVONDRRS1pDWjJVQ0FLUlhIUTZRM0w1TiJ9.zgPvDQQIvU1kvF_9ctjcKuqC5xKhlpZo7MH5v7AYufU')
    ...
    async def search_calls(self, account_name: str = None, contact_email: str = None, 
                          from_date: datetime = None, limit: int = 50) -> List[Dict[str, Any]]:
    ...
    async def search_emails(self, account_name: str = None, contact_email: str = None,
                           from_date: datetime = None, limit: int = 50) -> List[Dict[str, Any]]:
        ...
        filter_params = {
            "contentSelector": {
                "exposureType": "EMAIL"  # Filter for email conversations only
            }
        }
    ...
    async def search_all_conversations(self, account_name: str = None, contact_email: str = None,
                                     from_date: datetime = None, limit: int = 50) -> Dict[str, Any]:
...
class RevenueOpsGateway(MCPServerBase):
    ...
    async def _register_tools(self):
        ...
        async def search_contacts(search_term: str, sources: List[str] = None, limit: int = 50) -> Dict[str, Any]:
            """
            ...
            if "salesforce" in sources:
                try:
                    ...
                    for contact in sf_contacts:
                        results["contacts"].append(ContactInfo(
                            id=contact['Id'],
                            name=contact['Name'],
        ...
        async def conversation_insights(account_id: str = None, contact_email: str = None, 
                                      days_back: int = 30, include_emails: bool = True) -> Dict[str, Any]:
            """
                    ...
                    if email_id:
                        try:
                            ...
                            insights["emails"].append({
                                ...
                                "direction": email.get('direction', 'unknown'),  # inbound/outbound
                                "type": "email"
                            })
                        except Exception as e:
        ...
        async def slack_client_context(client_name: str, days_back: int = 30) -> Dict[str, Any]:
            ...
            try:
                ...
                negative_keywords = ['frustrated', 'issue', 'problem', 'broken', 'terrible', 'complaint']
                ...
                return {
                    ...
                    'overall_health': {
                        ...
                        'urgency_level': 'high' if sentiment_analysis['urgent'] > 3 else 'medium' if sentiment_analysis['urgent'] > 0 else 'low',
                        'sentiment_trend': 'positive' if sentiment_analysis['positive'] > sentiment_analysis['negative'] else 'negative' if sentiment_analysis['negative'] > sentiment_analysis['positive'] else 'neutral'
                    }
                }
        ...
        async def employee_sentiment_analysis(employee_slack_id: str = None, employee_email: str = None, 
            ...
            try:
                ...
                if not employee_slack_id:
                    ...
                    return {"error": "Employee Slack ID mapping not implemented", "employee_email": employee_email}
                
                sentiment_data = await self.slack.get_employee_sentiment(employee_slack_id, days_back)
                ...
                if overall_sentiment == 'negative' or stress_percentage > 30:
                    recommendations.append("🚨 Schedule 1:1 check-in - employee showing signs of stress")
                    recommendations.append("💬 Consider workload adjustment or additional support")
                
                if stress_percentage > 50:
                    recommendations.append("⚠️ Immediate intervention recommended - high stress indicators")
                
                if overall_sentiment == 'positive':
                    recommendations.append("🌟 Employee showing positive engagement - good time for stretch assignments")
                
                if sentiment_data.get('total_messages', 0) < 5:
                    recommendations.append("📢 Low communication activity - check if employee needs support or is disengaged")
        ...
        async def team_health_analysis(team_channel_id: str = None, team_name: str = None, 
            ...
            try:
                ...
                collaboration_keywords = ['thanks', 'great job', 'awesome', 'collaboration', 'teamwork', 'help', 'support']
                conflict_keywords = ['disagree', 'frustrated', 'blocked', 'issue', 'problem', 'concern']
                innovation_keywords = ['idea', 'innovation', 'creative', 'solution', 'improvement', 'experiment']
                ...
                team_health_score = max(0, min(100, collaboration_score - conflict_score + innovation_score))
                
                # Generate recommendations
                recommendations = []
                if team_health_score < 40:
                    recommendations.append("🚨 Team health needs attention - consider team building activities")
                
                if conflict_score > 10:
                    recommendations.append("⚠️ Conflict indicators detected - facilitate team discussion")
                
                if collaboration_score < 20:
                    recommendations.append("🤝 Low collaboration - encourage more team interaction")
                
                if innovation_score < 10:
                    recommendations.append("💡 Low innovation indicators - consider brainstorming sessions")
                
                if active_members < 3:
                    recommendations.append("📢 Low participation - check if team members are engaged")
                ...
                return {
                    ...
                    'member_activity': dict(list(user_activity.items())[:10]),  # Top 10 active members
                    'recommendations': recommendations,
                    'health_status': 'healthy' if team_health_score > 70 else 'needs_attention' if team_health_score > 40 else 'critical'
                }
        ...
        async def ceo_dashboard_intelligence(focus_areas: List[str] = None, days_back: int = 7) -> Dict[str, Any]:
            """
            ...
            try:
                ...
                if "client_health" in focus_areas:
                    ...
                    key_clients = ["Sunset Apartments", "Metro Properties", "Urban Living"]  # Pay Ready clients
                    ...
                    for client in key_clients:
                        try:
                            ...
                            client_health_summary[client] = {
                                'mention_frequency': client_context.get('overall_health', {}).get('mention_frequency', 0),
                                'urgency_level': client_context.get('overall_health', {}).get('urgency_level', 'low'),
                                'sentiment_trend': client_context.get('overall_health', {}).get('sentiment_trend', 'neutral')
                            }
                            
                            # Add to risk alerts if needed
                            if client_context.get('overall_health', {}).get('urgency_level') == 'high':
                                dashboard_data['risk_alerts'].append(f"🚨 {client} showing high urgency indicators in Slack")
                ...
                if "team_sentiment" in focus_areas:
                    ...
                    team_channels = ["general", "product", "engineering", "sales", "support"]  # Pay Ready teams
                    team_health_summary = {}
                    
                    for team in team_channels:
                        try:
                            team_health = await team_health_analysis(team_name=team, days_back=days_back)
                            team_health_summary[team] = {
                                'health_score': team_health.get('metrics', {}).get('overall_health_score', 50),
                                'status': team_health.get('health_status', 'unknown'),
                                'active_members': team_health.get('metrics', {}).get('active_members', 0)
                            }
                            
                            # Add to action items if needed
                            if team_health.get('health_status') == 'critical':
                                dashboard_data['action_items'].append(f"⚠️ {team.title()} team health critical - immediate attention needed")
                ...
                if "operational_issues" in focus_areas:
                    ...
                    operational_keywords = ["down", "outage", "bug", "critical", "emergency", "incident"]
                    operational_issues = []
                    
                    for keyword in operational_keywords:
                        try:
                            search_result = await self.slack.search_messages(f'"{keyword}"', count=10)
                    ...
                    if len(operational_issues) > 10:
                        dashboard_data['risk_alerts'].append(f"🚨 High volume of operational issues detected ({len(operational_issues)} in {days_back} days)")
                ...
                dashboard_data['executive_summary'] = {
                    ...
                    'overall_status': 'healthy' if len(dashboard_data['risk_alerts']) == 0 else 'needs_attention' if len(dashboard_data['risk_alerts']) < 3 else 'critical',
                    'key_metrics': {
                        'client_health_average': 75,  # Would calculate from actual data
                        'team_health_average': 80,   # Would calculate from actual data
                        'operational_stability': 'stable' if len(operational_issues) < 5 else 'unstable'
                    }
                }
    ...
    async def _calculate_account_health(self, account_data: Dict[str, Any]) -> AccountHealth:
        ...
        if "account" in sf_data:
            ...
            if account.get("LastActivityDate"):
                last_activity = datetime.fromisoformat(account["LastActivityDate"].replace('Z', '+00:00'))
        ...
        if opportunities_data:
            open_opps = [opp for opp in opportunities_data if opp.get("StageName") not in ["Closed Won", "Closed Lost"]]
