...
# LangGraph Postgres checkpoint is optional and its import path varies by version
try:  # Newer usage in some versions
    from langgraph.checkpoint import PostgresCheckpoint  # type: ignore
except Exception:  # Fallback to explicit postgres module
    try:
        from langgraph.checkpoint.postgres import PostgresCheckpoint  # type: ignore
    except Exception:
...
class UnifiedMemoryBus:
    ...
    def __init__(
        ...
        # MemOS integration
        self.memory = Memory()
        # LangGraph checkpointer for ASIP swarm (optional)
        self.checkpointer = PostgresCheckpoint(pg_pool) if PostgresCheckpoint else None
        # 5-tier cache system
    ...
    async def swarm_handoff(
        ...
        with tracer.start_as_current_span("swarm_handoff") as span:
            ...
            # Save checkpoint for persistence (if available)
            if self.checkpointer is not None:
                ...
                logger.warning("LangGraph PostgresCheckpoint unavailable; skipping checkpoint save")
    ...
    async def search_semantic(
        self, query_vector: List[float], tenant_id: str = "default", limit: int = 10
    ) -> List[MemoryCube]:
        """
        ...
        with tracer.start_as_current_span("semantic_search") as span:
            span.set_attributes({"tenant_id": tenant_id, "limit": limit})
    ...
    async def _set_in_neon_cold(
        ...
        try:
            async with self.pg_wrapper.pool.acquire() as conn:
                ...
                """,
                    key,
                    orjson.dumps(cube.__dict__).decode(),
                    tenant_id,
    ...
    async def _vector_search(
        self, query_vector: List[float], tenant_id: str
    ) -> Optional[MemoryCube]:
