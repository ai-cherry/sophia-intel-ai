...
tier_hits = meter.create_counter(
    "sophia_tier_hits_total",
    description="Cache hits by tier"
)
tier_latency = meter.create_histogram(
    "sophia_tier_latency_seconds", 
    description="Cache operation latency by tier"
)
...
class TieredCacheSystem:
        ...
        try:
            async with self.pg_pool.acquire() as conn:
                ...
                result = await conn.execute(
                    "DELETE FROM cache_cold_storage WHERE cache_key = $1",
                    cache_key
                )
    ...
    async def _set_in_neon_cold(self, key: str, entry: CacheEntry, tenant_id: str) -> None:
        ...
        try:
            async with self.pg_pool.acquire() as conn:
                ...
                """, key, orjson.dumps(entry.value).decode(), tenant_id,
                    time.time(), time.time(), entry.importance_score)
        except Exception as e:
    ...
    async def _background_optimizer(self) -> None:
        ...
        while True:
            try:
                ...
                logger.info(f"Cache performance: {hit_rate:.3f} hit rate (target: {self.target_hit_rate:.3f})")
