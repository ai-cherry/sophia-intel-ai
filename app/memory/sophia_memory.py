...
class SophiaMemoryService(BaseMemoryService):
            ...
            try:
                ...
                if docs_dir.exists():
                    ...
                    for fp in docs_dir.glob("*.json"):
                        ...
                        if q in content and self._apply_filters(doc, filters):
                            results.append(doc)
    ...
    def _apply_filters(self, doc: Dict[str, Any], filters: Optional[Dict]) -> bool:
        if not filters:
            return True
        try:
    ...
    def _build_weaviate_filter(self, filters: Dict) -> Dict:
        ...
        for key, value in filters.items():
            where["operands"].append(
                {"path": [key], "operator": "Equal", "valueString": str(value)}
            )
