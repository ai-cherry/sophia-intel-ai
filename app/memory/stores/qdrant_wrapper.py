...
vector_operations = meter.create_counter(
    "sophia_vector_operations_total", description="Vector operations by type"
)
...
class AdaptiveEfCalculator:
    ...
    def update_performance(self, ef: int, latency_ms: float, accuracy: float):
        ...
        self.performance_history[ef].append(
            {"latency_ms": latency_ms, "accuracy": accuracy, "timestamp": time.time()}
        )
...
class SemanticCache:
    ...
    def _calculate_query_hash(
        ...
        query_data = {"vector": quantized, "k": k, "filters": filters or {}}
...
class QuantizedQdrantWrapper:
    ...
    async def create_collection(
        ...
        if quantization == QuantizationType.BINARY:
            quantization_config = BinaryQuantization(binary=BinaryQuantization())
            ...
            quantization_config = ProductQuantization(
                product=ProductQuantization(compression="x32", always_ram=True)
            )
        ...
        try:
            ...
            logger.info(
                f"✅ Created quantized collection: {collection_name} ({quantization.quant_name})"
            )
            return collection
    ...
    async def delete_vectors(
        self, collection_name: str, vector_ids: List[str], tenant_id: str = "default"
    ) -> Dict[str, Any]:
    ...
    async def get_performance_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "quantization_distribution": {
                quant.quant_name: sum(
                    1 for c in self.collections.values() if c.quantization == quant
                )
        ...
        for field, condition in filters.items():
            if isinstance(condition, dict):
                if "match" in condition:
                    conditions.append(
                        FieldCondition(key=field, match=Match(value=condition["match"]))
                    )
...
async def create_quantized_qdrant(client: AsyncQdrantClient) -> QuantizedQdrantWrapper:
    ...
    try:
        ...
        logger.info(
            f"✅ Qdrant connection verified - {len(collections.collections)} existing collections"
        )
    except Exception as e:
