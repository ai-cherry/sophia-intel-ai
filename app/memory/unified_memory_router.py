...
class MemoryDomain(Enum):
    ...
    CODE = "code"      # Coding/Builder domain
...
class UnifiedMemoryRouter:
    ...
    async def upsert_chunks(
        ...
        try:
            ...
            if weaviate_client is None:
                import os
                if os.getenv("MEMORY_L2_FAIL_FAST", "false").lower() in ("1", "true", "yes"):
                    raise RuntimeError("Weaviate (L2) unavailable and MEMORY_L2_FAIL_FAST is enabled")
    ...
    async def search(
        ...
        if rerank and len(hits) > 5:
            reranked = await self._rerank(query, hits[:20])
            hits = reranked[:k]
        else:
            hits = hits[:k]
