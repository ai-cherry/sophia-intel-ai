...
class UnifiedMemoryStore:
    ...
    """
    def __init__(self, agent_id: str, enable_sync: bool = True, cache_size: int = 1000):
        """
    ...
    async def store(
        ...
        request = AgnoEmbeddingRequest(texts=[text], use_case=use_case)
    ...
    async def _section_level_retrieval(
        ...
        # First get documents
        doc_results = await self._document_level_retrieval(query, filters, limit * 2)
        # Then search within documents for sections
        section_results = []
        for doc in doc_results:
            # Generate query embedding using Agno service
            query_request = AgnoEmbeddingRequest(texts=[query], use_case="search")
    ...
    async def _snippet_level_retrieval(
        ...
        # Get sections first
        section_results = await self._section_level_retrieval(query, filters, limit * 2)
    ...
    async def update(
        ...
        if regenerate_embeddings:
            ...
            request = AgnoEmbeddingRequest(texts=[text], use_case="general")
            await self.embedding_service.embed(request)
...
if __name__ == "__main__":
    async def test_unified_memory():
        ...
        doc_results = await store.retrieve(
            "machine learning", level=RetrievalLevel.DOCUMENT, limit=5
        )
