...
class NeonMigrationManager:
    ...
    async def validate_shadow_branch(self) -> Dict[str, Any]:
        ...
        with create_memory_operation_span("validate_shadow_branch", "default") as span:
            try:
                ...
                logger.info(
                    f"✅ Shadow branch validation: {validation_results['passed_checks']}/{validation_results['total_checks']} checks passed"
                )
    ...
    async def execute_migration_step(
        ...
        with create_memory_operation_span("execute_migration_step", "default") as span:
            span.set_attributes({"step_id": step.id, "step_name": step.name})
                ...
                try:
                    ...
                    if step.validation_query:
                        ...
                        if result != step.expected_result:
                            raise ValueError(
                                f"Validation failed: expected {step.expected_result}, got {result}"
                            )
                    ...
                    logger.info(
                        f"✅ Migration step completed: {step.name} ({step.execution_time_ms:.1f}ms)"
                    )
        ...
        with create_memory_operation_span(
            ...
            try:
                ...
                if migration_results["success"]:
                    logger.info(
                        f"✅ Migration completed successfully: {migration_results['completed_steps']}/{migration_results['total_steps']} steps"
                    )
                else:
                    logger.error(
                        f"❌ Migration failed: {migration_results['failed_steps']} failed steps"
                    )
        ...
        with create_memory_operation_span("rollback_migration", "default") as span:
            ...
            try:
                ...
                if rollback_results["success"]:
                    ...
                    logger.error(
                        f"❌ Rollback partially failed: {rollback_results['failed_steps']} failed steps"
                    )
        ...
        with create_memory_operation_span("canary_deployment", "default") as span:
            ...
            try:
                ...
                performance_acceptable = (
                    canary_results["metrics"]["performance_delta"] > -10
                )  # <10% degradation
                error_rate_acceptable = (
                    canary_results["metrics"]["error_rate_shadow"]
    ...
    async def zero_downtime_cutover(
        self, old_connection: str, new_connection: str
    ) -> Dict[str, Any]:
        """
        ...
        with create_memory_operation_span("zero_downtime_cutover", "default") as span:
            try:
                ...
                logger.info(
                    f"✅ Zero-downtime cutover completed - {cutover_results['downtime_ms']:.1f}ms downtime"
                )
...
async def create_neon_migration_manager(
    neon_api_key: str = None, project_id: str = None
