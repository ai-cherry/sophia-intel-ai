    ...
    async def save(self, key: str, data: Any, ttl: Optional[int] = None) -> bool:
        ...
        try:
            ...
            if ttl:
                ...
                meta_data = {"expires_at": time.time() + ttl, "created_at": time.time()}
...
class S3StorageAdapter(StorageAdapter):
    ...
    async def load(self, key: str) -> Optional[Any]:
        ...
        try:
            ...
            response = await asyncio.get_event_loop().run_in_executor(
                None, self.s3_client.get_object, self.bucket_name, key
            )
    ...
    async def delete(self, key: str) -> bool:
        ...
        try:
            await asyncio.get_event_loop().run_in_executor(
                None, self.s3_client.delete_object, self.bucket_name, key
            )
    ...
    async def exists(self, key: str) -> bool:
        ...
        try:
            await asyncio.get_event_loop().run_in_executor(
                None, self.s3_client.head_object, self.bucket_name, key
            )
            return True
        except self.s3_client.exceptions.NoSuchKey:
            return False
