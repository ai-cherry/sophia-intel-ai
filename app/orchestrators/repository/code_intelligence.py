...
class CodeIntelligence:
    ...
    async def _detect_patterns(self) -> Dict[str, List[Dict]]:
        ...
        for file_info in self.file_cache.values():
            # Singleton pattern
            if any('instance' in func.lower() or 'singleton' in func.lower() for func in file_info.functions):
            ...
            # Factory pattern
            if any('factory' in cls.lower() or 'create' in func.lower() for cls in file_info.classes for func in file_info.functions):
                patterns['design_patterns'].append({
                    'type': 'factory',
                    'file': file_info.path,
                    'confidence': 0.6
    ...
    async def _calculate_metrics(self) -> Dict[str, Any]:
        ...
        if self.file_cache:
            ...
            metrics['most_dependencies'] = [
                {'path': path, 'count': len(deps)}
                for path, deps in sorted_by_deps[:5]
            ]
    ...
    async def _get_recent_changes(self, file_path: str) -> List[Dict]:
        ...
        try:
            ...
            result = await asyncio.create_subprocess_exec(
                'git', 'log', '-5', '--pretty=format:%H|%an|%ad|%s', str(rel_path),
                cwd=self.repo_path,
                stdout=asyncio.subprocess.PIPE,
                stderr=asyncio.subprocess.DEVNULL
            )
