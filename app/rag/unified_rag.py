...
context synthesis, and integration with dual orchestrators (/Sophia)
"""
import asyncio
import logging
import time
...
class RAGDomain(Enum):
    ...
    CODE = "code"  # Technical intelligence domain
...
class UnifiedRAGSystem:
    ...
    async def query_execution_history(
        self,
        query: str,
        domain: RAGDomain = RAGDomain.CODE,
    ...
    async def _determine_strategy(self, context: RAGContext) -> RAGStrategy:
        ...
        if query_length < 3:
            ...
            has_technical_terms
                and context.domain == RAGDomain.CODE
    ...
    async def _extract_insights(
        ...
        elif context.domain == RAGDomain.CODE:
            technical_sources = [
                s
                for s in sources
