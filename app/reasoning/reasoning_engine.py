...
class ReActStrategy(ReasoningStrategy):
    ...
    async def _should_act(self, context: ReasoningContext) -> bool:
        ...
        recent_actions = sum(
            1 for step in context.previous_steps[-3:]
...
"""
    ...
    def _format_recent_thoughts(self, context: ReasoningContext) -> str:
        ...
        thoughts = [
            step.content for step in context.previous_steps[-5:]
...
class ReasoningEngine:
    ...
    def _record_metrics(self, context: ReasoningContext):
        ...
        metrics = {
            ...
            "thought_steps": sum(1 for s in context.previous_steps if s.step_type == ReasoningStepType.THOUGHT),
            "action_steps": sum(1 for s in context.previous_steps if s.step_type == ReasoningStepType.ACTION),
            "average_confidence": sum(s.confidence for s in context.previous_steps) / len(context.previous_steps) if context.previous_steps else 0
        }
