"""
Web Research Teams for Sophia (business + technical domains)
...
class ResearchDomain(Enum):
    ...
    TECHNICAL = "technical"  # Technical domain (sidecar integration)
...
class SophiaResearchTeam(WebResearchTeam):
    ...
    async def competitive_analysis(
        ...
        return report
class ResearchTeam(WebResearchTeam):
    """-specific research team for Technical/Code domains"""
    def __init__(self):
...
if __name__ == "__main__":
    async def main():
        ...
        # Test  research
        _team = ResearchTeam()
        report = await _team.api_research("OpenAI", "v1")
        logger.info(f" Research: {report.summary}")
        logger.info(f"Confidence: {report.total_confidence:.2%}")
    asyncio.run(main())
