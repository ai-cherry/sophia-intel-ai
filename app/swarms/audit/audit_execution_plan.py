...
from app.swarms.audit.badass_audit_config import (
    BADASS_AGENTS, AUDIT_FORMATIONS, COLLABORATION_PATTERNS, 
    QUALITY_GATES, AgentSpec, get_formation_config, get_agents_for_formation
)
from app.swarms.audit.strategic_planning_enhancement import (
    StrategicPlanningEngine, PlanningPhase
)
...
class BadassAuditOrchestrator:
    ...
    def __init__(self, formation: str = "full_spectrum", codebase_path: str = ".", enable_strategic_planning: bool = True):
        ...
        if enable_strategic_planning:
            self.strategic_planner = StrategicPlanningEngine(
                formation=f"strategic_{formation}",
    ...
    def _generate_analysis_tasks(self) -> List[AnalysisTask]:
        ...
        tasks.extend([
            ...
            AnalysisTask(
                ...
                category="security",
                priority=1,
                assigned_agents=["security_commander", "vulnerability_hunter", "compliance_officer"],
        ...
        tasks.extend([
            AnalysisTask(
                ...
                expected_duration=8,
                dependencies=["architecture_deep_dive", "security_vulnerability_hunt", "performance_profiling_analysis"],
                requires_consensus=True
            ),
            AnalysisTask(
    ...
    async def execute_badass_audit(self) -> Dict[str, Any]:
        ...
        logger.info(f"📊 {len(self.analysis_tasks)} tasks planned with {len(self.agent_specs)} agents")
        ...
        try:
            ...
            if self.strategic_planner:
                ...
                strategic_insights = await self.strategic_planner.execute_comprehensive_strategic_planning()
    ...
    async def _execute_parallel_discovery(self) -> Dict[str, Any]:
        ...
        # Process results
        discovery_summary = {}
        for i, (task, result) in enumerate(zip(discovery_tasks, results)):
    ...
    async def _execute_deep_analysis(self) -> Dict[str, Any]:
        ...
        analysis_tasks = [
            task for task in self.analysis_tasks 
    ...
    async def _execute_cross_validation(self) -> Dict[str, Any]:
        ...
        validation_tasks = [task for task in self.analysis_tasks if task.category in ["integration", "cross_cutting", "data_privacy"]]
    ...
    async def _execute_final_validation(self) -> Dict[str, Any]:
        ...
        validation_task = next(
            (task for task in self.analysis_tasks if task.category == "synthesis"),
            None
        )
    ...
    async def _execute_report_generation(self) -> Dict[str, Any]:
        ...
        report_task = next(
            (task for task in self.analysis_tasks if task.category == "reporting"),
            None
        )
        ...
        if report_task and self._dependencies_satisfied(report_task):
            ...
            audit_summary = {
                "executive_summary": result.get("executive_summary", "Comprehensive audit completed"),
                "overall_score": self._calculate_overall_score(),
                "total_findings": len(self.execution.findings),
                "critical_findings": len([f for f in self.execution.findings if f.get("severity") == "critical"]),
    ...
    def _integrate_strategic_insights(self, strategic_insights: Dict[str, Any]):
        ...
        for task in self.analysis_tasks:
            if task.id in priority_adjustments:
                ...
                logger.info(f"📈 Adjusted priority for {task.id}: {original_priority} -> {task.priority}")
    ...
    def _assess_deployment_readiness(self) -> str:
        ...
        critical_count = len([f for f in self.execution.findings if f.get("severity") == "critical"])
