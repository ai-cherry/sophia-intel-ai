...
class VirtualKeyAllocator:
    ...
    def allocate_for_swarm(
        agent_count: int, preferred_providers: list[str] = None
    ) -> dict[str, str]:
        """
...
def validate_virtual_keys() -> bool:
    ...
    logger.info(
        f"✅ Total parallel capacity: {calculate_swarm_capacity(list(PORTKEY_VIRTUAL_KEYS.values()))}"
    )
...
if __name__ == "__main__":
    ...
    logger.info("=" * 70)
    logger.info("PORTKEY VIRTUAL KEYS CONFIGURATION")
    logger.info("=" * 70)
    ...
    for name, key in PORTKEY_VIRTUAL_KEYS.items():
        if key in PROVIDER_CONFIGS:
            ...
            logger.info(
                f"   {name:12} → {key:20} → {config['provider']:10} ({config['tpm_limit']:,} TPM)"
            )
        else:
            logger.info(f"   {name:12} → {key:20}")

    # Calculate total capacity
    capacity = calculate_swarm_capacity(list(PORTKEY_VIRTUAL_KEYS.values()))
    logger.info("\n💪 Total Parallel Capacity:")
    logger.info(f"   TPM: {capacity['total_tpm']:,}")
    logger.info(f"   RPM: {capacity['total_rpm']:,}")
    logger.info(f"   Providers: {', '.join(capacity['providers'])}")
    ...
    for swarm_type, mapping in OPTIMAL_AGENT_MAPPING.items():
        ...
        logger.info(f"\n   {swarm_type}:")
        logger.info(f"      Agents: {len(mapping)}")
        logger.info(f"      TPM: {swarm_capacity['total_tpm']:,}")
        logger.info(f"      Providers: {', '.join(swarm_capacity['providers'])}")
