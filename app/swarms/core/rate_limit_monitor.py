...
class RateLimitInfo:
    ...
    def can_handle_request(self, estimated_tokens: int) -> bool:
        ...
        if self.configured_tpm > 0:
            if self.tokens_used_minute + estimated_tokens > self.configured_tpm * 0.9:  # 90% threshold
                return False
...
class RateLimitMonitor:
    ...
    def get_best_key_for_task(
        ...
        for virtual_key, rate_info in self.rate_limits.items():
            ...
            # Calculate score
            score = (
                availability * 0.4 +  # Availability is important
                (1 / max(rate_info.avg_latency, 0.1)) * 0.3 +  # Lower latency is better
                rate_info.success_rate * 0.3  # Higher success rate is better
            )
        ...
        for virtual_key, rate_info in self.rate_limits.items():
            if rate_info.provider == provider:
                return {
                    ...
                    "last_rate_limit": rate_info.last_rate_limit.isoformat() if rate_info.last_rate_limit else None
                }
        ...
        for virtual_key, rate_info in self.rate_limits.items():
            metrics["providers"][rate_info.provider] = {
                ...
                "rate_limits": {
                    ...
                    "last_hit": rate_info.last_rate_limit.isoformat() if rate_info.last_rate_limit else None
                }
            }
...
if __name__ == "__main__":
    ...
    monitor.record_request("anthropic-vk-b42804", tokens_used=1000, latency=2.1, success=False, error="rate limit exceeded")
