...
class TaskRouter:
    ...
    def route_task(self, requirements: TaskRequirements) -> Tuple[str, Dict[str, Any]]:
        ...
        for virtual_key, provider, score in scored_candidates:
            if self.rate_monitor.rate_limits[virtual_key].can_handle_request(requirements.estimated_tokens):
                ...
                logger.info(f"Routed {requirements.task_type.value} to {provider} (score: {score:.2f})")
    ...
    def _score_provider(self, provider: str, requirements: TaskRequirements) -> float:
        ...
        if requirements.max_latency_seconds:
            ...
            elif requirements.max_latency_seconds < 2.0 and "ultra_fast" not in provider_config.get("special_features", []):
    ...
    def _get_fallback_provider(self, requirements: TaskRequirements) -> Tuple[str, Dict[str, Any]]:
        ...
        if "vkj-openrouter-cc4151" in self.rate_monitor.rate_limits:
            if self.rate_monitor.rate_limits["vkj-openrouter-cc4151"].can_handle_request(requirements.estimated_tokens):
...
def route_coding_task(code: str, task: str = "generate") -> Tuple[str, Dict[str, Any]]:
    ...
    requirements = TaskRequirements(
        ...
        required_capabilities=["specialized_coding"] if task_type == TaskType.CODE_GENERATION else None
    )
...
def route_research_task(query: str, need_realtime: bool = False) -> Tuple[str, Dict[str, Any]]:
    ...
    requirements = TaskRequirements(
        ...
        required_capabilities=["real_time_search", "citations"] if need_realtime else ["thorough_analysis"]
    )
...
if __name__ == "__main__":
    ...
    test_tasks = [
        ...
        TaskRequirements(TaskType.WEB_RESEARCH, estimated_tokens=500, required_capabilities=["real_time_search"]),
