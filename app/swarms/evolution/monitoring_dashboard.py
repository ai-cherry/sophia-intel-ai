...
class ExperimentalEvolutionMonitor:
    ...
    async def _monitor_adapter(self, swarm_type: str, adapter: ExperimentalSwarmEvolutionAdapter):
        ...
        try:
            ...
            metrics = await self._calculate_adapter_metrics(
                swarm_type, adapter, status, performance
            )
    ...
    async def _calculate_engine_metrics(
        self,
        swarm_type: str,
        ...
        if len(history) >= 2:
            ...
            older_avg = (
                statistics.mean([m.average_fitness for m in history[-10:-5]])
                if len(history) >= 10
                else recent_avg
            )
    ...
    async def _calculate_adapter_metrics(
        self,
        swarm_type: str,
    ...
    def _generate_recommendations(
        ...
        if not recommendations:
            recommendations.append(
                "✅ No specific recommendations - system appears to be operating within normal parameters"
            )
