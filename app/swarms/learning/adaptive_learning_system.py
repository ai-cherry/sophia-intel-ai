...
class ExperienceReplayBuffer:
    ...
    by_knowledge_type: Dict[KnowledgeType, List[str]] = field(default_factory=lambda: defaultdict(list))
...
class ReinforcementLearningAlgorithm(LearningAlgorithm):
    ...
    def _encode_state(self, problem_context: Dict[str, Any], execution_context: Dict[str, Any]) -> str:
    ...
    def _encode_action(self, execution_mode: SwarmExecutionMode, agent_states: Dict[str, Any]) -> str:
                    ...
                    if target_modes:
                        knowledge = LearnedKnowledge(
                            ...
                            pattern={
                                ...
                                'transfer_confidence': min([exp.confidence for exp in successful_experiences])
                            },
                            conditions=self._merge_conditions([exp.problem_context for exp in successful_experiences]),
                            expected_outcome={
                                'avg_quality': np.mean([exp.quality_score for exp in successful_experiences])
                            },
                            confidence=0.7,
                            applicable_modes=target_modes,
                            problem_types=list(set(exp.problem_type for exp in successful_experiences))
                        )
                        learned_knowledge.append(knowledge)
...
class CrossAgentKnowledgeDistiller:
    ...
    async def distill_knowledge(
        ...
        with tracer.start_span("knowledge_distillation", kind=SpanKind.INTERNAL) as span:
            ...
            logger.info(f"🧠 Distilled knowledge from {len(teacher_agents)} teachers to {len(student_agents)} students")
...
class AdaptiveLearningSystem:
        ...
        with tracer.start_span("capture_experience", kind=SpanKind.INTERNAL) as span:
            experience = LearningExperience(
                ...
                performance_metrics={
                    ...
                    'agent_utilization': sum(metrics.agent_utilization.values()) / max(len(metrics.agent_utilization), 1),
    ...
    async def _process_learning_experience(self, experience: LearningExperience):
        ...
        try:
            ...
            if LearningType.REINFORCEMENT in self.learning_algorithms:
                rl_knowledge = await self.learning_algorithms[LearningType.REINFORCEMENT].learn_from_experience(
                    experience, {'learning_type': 'reinforcement'}
                )
                if rl_knowledge:
                    learned_knowledge.append(rl_knowledge)
            
            # Transfer learning (if we have similar experiences)
            similar_experiences = self.experience_buffer.get_similar_experiences(experience, limit=5)
            if len(similar_experiences) >= 2:
                if LearningType.TRANSFER in self.learning_algorithms:
                    transfer_knowledge = await self.learning_algorithms[LearningType.TRANSFER].learn_from_experience(
            ...
            if learned_knowledge:
                logger.debug(f"🎓 Learned {len(learned_knowledge)} new knowledge items in {learning_time:.3f}s")
        ...
        with tracer.start_span("apply_knowledge", kind=SpanKind.INTERNAL) as span:
            ...
            if knowledge.knowledge_type == KnowledgeType.EXECUTION_PATTERN:
                ...
                if 'optimal_agent_count' in pattern:
                    application_result['modifications']['agent_count'] = pattern['optimal_agent_count']
            ...
            logger.debug(f"🎯 Applied knowledge {knowledge.id} with confidence {knowledge.confidence}")
    ...
    async def distill_cross_agent_knowledge(
        ...
        teacher_agents = [
            agent for agent, perf in agent_performance.items()
    ...
    async def get_learning_insights(self) -> Dict[str, Any]:
        ...
        if self.learning_metrics['knowledge_created'] > 0:
            effectiveness = self.learning_metrics['knowledge_applied'] / self.learning_metrics['knowledge_created']
        ...
        return {
            ...
            'recent_activity': {
                ...
                'success_rate_last_hour': sum(1 for exp in recent_experiences if exp.success) / max(len(recent_experiences), 1)
            },
        ...
        try:
            ...
            if self.learning_system:
                ...
                await self.learning_system.capture_experience(
                    ...
                    solution=result.__dict__ if hasattr(result, '__dict__') else {'result': str(result)},
