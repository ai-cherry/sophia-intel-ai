...
from app.swarms.core.swarm_base import SwarmExecutionMode, SwarmBase, SwarmConfig
    ...
    async def capture_execution_experience(
        ...
        return await self.learning_system.capture_experience(
            ...
            solution={
                ...
                'quality_score': np.mean([r.get('quality_score', 0.0) for r in result]) if result else 0.0,
...
class LearningEnhancedParallelExecution(LearningEnhancedExecutionStrategy):
        ...
        with tracer.start_span("learning_parallel_execution", kind=SpanKind.INTERNAL) as span:
            ...
            async def execute_learning_agent(agent, agent_id: str):
                agent_context = {
                    **shared_context,
                    'agent_id': agent_id,
                    'peer_agents': [f"agent_{i}" for i in range(len(swarm.agents)) if f"agent_{i}" != agent_id]
                }
            ...
            coordination_analysis = await self._analyze_parallel_coordination(valid_results, execution_time)
            ...
            await self.capture_execution_experience(swarm, problem, valid_results, execution_metadata)
    ...
    async def _get_coordination_knowledge(
        ...
        coordination_knowledge = await self.learning_system.get_applicable_knowledge(context, limit=2)
        ...
        return {
            ...
            'optimal_agent_count': coordination_strategies.get('optimal_agent_count', len(swarm.agents)),
    ...
    async def _analyze_parallel_coordination(
        ...
        knowledge_applications = [
            r.get('learning_metadata', {}).get('knowledge_applied', 0)
            for r in results
        ]
        knowledge_sharing_success = np.mean(knowledge_applications) if knowledge_applications else 0.0
        
        effectiveness = (success_rate * 0.4 + consistency_score * 0.4 + min(knowledge_sharing_success / 5.0, 1.0) * 0.2)
...
class LearningEnhancedSequentialExecution(LearningEnhancedExecutionStrategy):
            ...
            for i, agent in enumerate(swarm.agents):
                ...
                try:
                    ...
                    result['learning_metadata'] = {
                        ...
                        'sequential_improvements': await self._calculate_sequential_improvement(results, result)
                    }
    ...
    async def _get_sequential_knowledge(
        ...
        if previous_results:
            ...
            patterns.update({
                ...
                'recommended_approach': 'build_on_success' if success_trend[-1] else 'course_correct'
            })
    ...
    async def _update_progressive_knowledge(
        ...
        if agent_position > 0:
            updated_knowledge['next_agent_recommendations'] = await self._generate_next_agent_recommendations(
    ...
    async def _analyze_sequential_learning(
        ...
        effectiveness = (success_rate * 0.4 + improvement_trend * 0.4 + min(avg_inheritance / 10.0, 1.0) * 0.2)
...
class LearningEnhancedDebateExecution(LearningEnhancedExecutionStrategy):
    ...
    async def execute_with_learning(
        ...
        with tracer.start_span("learning_debate_execution", kind=SpanKind.INTERNAL) as span:
            ...
            debate_analysis = await self._analyze_debate_learning(results, execution_time, debate_knowledge)
    ...
    async def _analyze_debate_learning(
        ...
        adversarial_scores = [
            r.get('debate_metadata', {}).get('adversarial_score', 0.0)
            for r in results
        ]
...
class LearningEnhancedHierarchicalExecution(LearningEnhancedExecutionStrategy):
    ...
    async def execute_with_learning(
        ...
        with tracer.start_span("learning_hierarchical_execution", kind=SpanKind.INTERNAL) as span:
            ...
            try:
                ...
                coordinator_result = await swarm._execute_single_agent(coordinator, problem, coordinator_context)
    ...
    async def _get_hierarchical_knowledge(
        ...
        hierarchical_knowledge = await self.learning_system.get_applicable_knowledge(context, limit=3)
    ...
    async def _execute_workers_with_distillation(
        ...
        async def execute_worker_with_distillation(worker, worker_id: int, task_assignment: Dict[str, Any]):
            worker_context = {
                **base_context,
                'role': 'worker',
                'worker_id': worker_id,
    ...
    async def _analyze_hierarchical_learning(
        ...
        worker_results = [r for r in results if r.get('role') == 'worker' or 'distillation_metadata' in r]
        
        # Calculate coordination quality
        coordinator_success = len([r for r in coordinator_results if r.get('success', False)]) > 0
        worker_success_rate = (
            sum(1 for r in worker_results if r.get('success', False)) / max(len(worker_results), 1)
        )
...
class LearningEnhancedExecutionFactory:
    ...
    def _initialize_strategies(self):
        ...
        self._strategies = {
            ...
            SwarmExecutionMode.HIERARCHICAL: LearningEnhancedHierarchicalExecution(self.learning_system),
            # Note: CONSENSUS and EVOLUTIONARY would be implemented similarly
        }
    
    def get_strategy(self, execution_mode: SwarmExecutionMode) -> Optional[LearningEnhancedExecutionStrategy]:
    ...
    async def execute_with_learning(
        ...
        if strategy:
            ...
            logger.warning(f"No learning strategy for {swarm.config.execution_mode}, using original execution")
...
if __name__ == "__main__":
    ...
    async def demo():
        ...
        from app.swarms.core.swarm_base import SwarmConfig, SwarmType
