...
class MiddlewareChain:
    ...
    async def process_before_execution(
        ...
        for middleware in self.middleware_components:
            try:
                ...
                logger.warning(
                    f"Middleware {type(middleware).__name__} before_execution failed: {e}"
                )
    ...
    async def process_during_execution(
        ...
        for middleware in self.middleware_components:
            try:
                updates = await middleware.during_execution(
                    swarm, problem, context, partial_results
                )
                if updates:
                    combined_updates.update(updates)
            except Exception as e:
                logger.warning(
                    f"Middleware {type(middleware).__name__} during_execution failed: {e}"
                )
    ...
    async def process_after_execution(
        ...
        for middleware in self.middleware_components:
            try:
                insights = await middleware.after_execution(
                    swarm, problem, context, results, execution_metadata
                )
                if insights:
                    combined_insights.update(insights)
            except Exception as e:
                logger.warning(
                    f"Middleware {type(middleware).__name__} after_execution failed: {e}"
                )
...
class KnowledgeApplicationMiddleware(LearningMiddleware):
    ...
    async def before_execution(
        ...
        with tracer.start_span("knowledge_application", kind=SpanKind.INTERNAL) as span:
                    ...
                    knowledge_metadata.append(
                        {
                            ...
                            "expected_improvement": application_result.get(
            ...
            logger.debug(
                f"🎯 Applied {len(knowledge_metadata)} knowledge items with {len(applied_modifications)} modifications"
            )
            ...
            return {
                "knowledge_enhancements": {
                    ...
                    "expected_improvements": [
                        km["expected_improvement"] for km in knowledge_metadata
                    ],
                }
            }
...
class RealTimeLearningMiddleware(LearningMiddleware):
    ...
    async def before_execution(
        ...
        execution_id = context.get("experience_capture", {}).get(
            "execution_id", f"rt_{uuid4().hex[:8]}"
        )
    ...
    async def during_execution(
        ...
        if (
            current_time - last_learning_time < self.learning_interval
            or len(partial_results) < self.min_partial_results
        ):
            return {}
    ...
    async def after_execution(
        ...
        logger.debug(
            f"📈 Real-time learning impact analysis: {len(execution_adjustments)} adjustments"
        )
    ...
    async def _extract_transfer_patterns(
        ...
        for source_mode, episodes in mode_groups.items():
            ...
            successful_episodes = [
                ...
                and ep["metadata"].get("quality_score", 0) > 0.7
            ]
...
class LearningMiddlewareFactory:
    ...
    def create_standard_middleware_chain(
        self, include_real_time: bool = True, include_cross_modal: bool = True
    ) -> MiddlewareChain:
        ...
        if include_cross_modal and self.memory_system:
            chain.add_middleware(
                CrossModalLearningMiddleware(self.learning_system, self.memory_system)
            )
    ...
    def create_lightweight_middleware_chain(self) -> MiddlewareChain:
        ...
        chain.add_middleware(
            KnowledgeApplicationMiddleware(self.learning_system, max_knowledge_items=3)
        )
...
async def inject_learning_middleware(
    swarm: SwarmBase, middleware_chain: MiddlewareChain
) -> SwarmBase:
    ...
    async def middleware_enhanced_solve_problem(problem: dict[str, Any]):
        ...
        try:
            ...
            enhanced_problem, enhanced_context = await swarm.apply_patterns(
                problem, enhanced_context
            )
    ...
    logger.info(
        f"🔧 Injected learning middleware chain with {len(middleware_chain.middleware_components)} components"
    )
    return swarm
