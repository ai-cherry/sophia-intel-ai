...
class TranscriptionAgent(BaseSalesAgent):
    ...
    def _get_latest_segments(self, transcripts: List[TranscriptSegment], 
                           since_seconds: int = 30) -> List[TranscriptSegment]:
...
class SentimentAgent(BaseSalesAgent):
    ...
    async def process(self, context: AgentContext) -> AgentOutput:
        ...
        priority = AgentPriority.MEDIUM
        if (analysis["overall_sentiment"] < -0.3 or 
            analysis["stress_indicators"] > 0.7):
            priority = AgentPriority.HIGH
    ...
    def _get_recent_transcripts(self, transcripts: List[TranscriptSegment], 
                              seconds: int = 60) -> List[TranscriptSegment]:
    ...
    def _analyze_overall_sentiment(self, segments: List[TranscriptSegment]) -> float:
        ...
        positive_keywords = {
            ...
            'fantastic', 'outstanding', 'brilliant', 'excited', 'interested'
        }
        
        negative_keywords = {
            'terrible', 'awful', 'hate', 'horrible', 'disappointed', 'frustrated',
            'concerned', 'worried', 'problem', 'issue', 'difficult', 'impossible'
        }
    ...
    def _detect_stress_indicators(self, segments: List[TranscriptSegment]) -> float:
        ...
        stress_keywords = {
            'pressure', 'stress', 'urgent', 'deadline', 'rush', 'busy',
            'overwhelming', 'difficult', 'challenging', 'struggle'
        }
...
class CompetitiveAgent(BaseSalesAgent):
    ...
    def _get_recent_transcripts(self, transcripts: List[TranscriptSegment], 
                              seconds: int = 120) -> List[TranscriptSegment]:
    ...
    def _assess_competitive_threat(self, segments: List[TranscriptSegment]) -> float:
        ...
        for segment in segments:
            ...
            if any(phrase in text for phrase in ["already using", "current vendor", "satisfied with"]):
                threat_level += 0.3
            ...
            if any(phrase in text for phrase in ["not satisfied", "problems with", "looking for alternative"]):
                threat_level -= 0.1
...
class RiskAssessmentAgent(BaseSalesAgent):
    ...
    async def process(self, context: AgentContext) -> AgentOutput:
        ...
        analysis = {
            ...
            "decision_maker_involvement": self._assess_decision_maker_involvement(recent_transcripts, context),
    ...
    def _get_recent_transcripts(self, transcripts: List[TranscriptSegment], 
                              seconds: int = 300) -> List[TranscriptSegment]:
    ...
    def _assess_decision_maker_involvement(self, segments: List[TranscriptSegment], 
    ...
    def _analyze_timeline_indicators(self, segments: List[TranscriptSegment]) -> Dict[str, Any]:
        ...
        total_urgency = sum(urgency_weights.get(mention["type"], 0.5) 
                          for mention in timeline_analysis["timeline_mentions"])
        
        if timeline_analysis["timeline_mentions"]:
            timeline_analysis["urgency_level"] = total_urgency / len(timeline_analysis["timeline_mentions"])
        
        return timeline_analysis
    
    def _calculate_win_probability(self, segments: List[TranscriptSegment], 
...
class CoachingAgent(BaseSalesAgent):
    ...
    async def process(self, context: AgentContext) -> AgentOutput:
        ...
        priority = AgentPriority.MEDIUM
        if (analysis["performance_score"] < 0.6 or 
            len(analysis["coaching_recommendations"]) > 2):
            priority = AgentPriority.HIGH
    ...
    def _get_recent_transcripts(self, transcripts: List[TranscriptSegment], 
                              seconds: int = 180) -> List[TranscriptSegment]:
    ...
    def _analyze_questioning_techniques(self, segments: List[TranscriptSegment]) -> Dict[str, Any]:
        ...
        for segment in segments:
            if self._is_internal_speaker(segment.speaker_name) and '?' in segment.text:
                ...
                if any(word in text for word in self.coaching_framework["questioning_techniques"]["open_questions"]):
                    analysis["open_questions"] += 1
                else:
                    analysis["closed_questions"] += 1
                
                if any(phrase in text for phrase in self.coaching_framework["questioning_techniques"]["discovery_questions"]):
    ...
    def _assess_active_listening(self, segments: List[TranscriptSegment]) -> float:
        ...
        for segment in internal_segments:
            ...
            if any(ack in text for ack in self.coaching_framework["active_listening"]["acknowledgments"]):
                listening_score += 0.2
            
            # Check for clarifications
            if any(clar in text for clar in self.coaching_framework["active_listening"]["clarifications"]):
                listening_score += 0.3
            
            # Check for summaries
            if any(summ in text for summ in self.coaching_framework["active_listening"]["summaries"]):
                listening_score += 0.4
    ...
    def _evaluate_objection_handling(self, segments: List[TranscriptSegment]) -> Dict[str, Any]:
        ...
        if objection_analysis["objections_detected"]:
            total_quality = sum(obj["response_quality"] for obj in objection_analysis["objections_detected"])
            objection_analysis["handling_quality"] = total_quality / len(objection_analysis["objections_detected"])
                ...
                if any(signal in text for signal in buying_signals):
                    ...
                    for j in range(i+1, min(i+3, len(segments))):
                        if self._is_internal_speaker(segments[j].speaker_name):
                            ...
                            if (any(trial in response for trial in framework["trial_close"]) or
                                any(assume in response for assume in framework["assumption"]) or
                                any(direct in response for direct in framework["direct"])):
...
class SummaryAgent(BaseSalesAgent):
    ...
    def _generate_follow_up_recommendations(self, call_data: RealtimeCallData) -> List[Dict]:
        ...
        if any("schedule" in seg.text.lower() or "meeting" in seg.text.lower() for seg in transcripts):
            recommendations.append({
                "type": "scheduling",
...
class SalesAgentOrchestrator:
    ...
    async def process_call_data(self, call_data: RealtimeCallData, 
    ...
    def get_recent_outputs(self, agent_type: str = None, 
                          priority: AgentPriority = None) -> List[AgentOutput]:
...
async def example_sales_intelligence_usage():
    ...
    from .gong_realtime import RealtimeCallData, CallMetadata, TranscriptSegment
