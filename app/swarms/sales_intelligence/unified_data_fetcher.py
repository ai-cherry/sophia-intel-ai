...
# Import INTEGRATIONS from proper config file
INTEGRATIONS = {}
                    ...
                    if response.status == 200:
                        ...
                        for workspace in workspaces:
                            ...
                            async with session.get(
                                projects_url, headers=headers, params=params
                            ) as proj_response:
            ...
            async with aiohttp.ClientSession() as session:
                ...
                async with session.get(contacts_url, headers=headers, params=params) as response:
                    if response.status == 200:
                        ...
                        async with session.get(
                            deals_url, headers=headers, params=params
                        ) as deals_response:
...
class UnifiedDataFetcher:
    ...
    async def fetch_all_data(self) -> UnifiedDataSummary:
        ...
        if "asana" in self.fetchers:
            fetch_tasks.append(
                self._safe_fetch("asana", self.fetchers["asana"].fetch_workspace_data())
            )

        if "linear" in self.fetchers:
            fetch_tasks.append(
                self._safe_fetch("linear", self.fetchers["linear"].fetch_teams_data())
            )
...
async def example_usage():
    ...
    logger.info(f"Connected platforms: {fetcher.get_connected_platforms()}")
    logger.info(f"Platform stats: {fetcher.get_platform_stats()}")
    ...
    logger.info(f"Last sync: {summary.last_sync}")
    ...
    for platform in ["gong", "asana", "linear", "notion", "hubspot"]:
        ...
        if platform_data:
            logger.info(f"\n{platform.title()}:")
            logger.info(f"  Count: {platform_data.count}")
            logger.info(f"  Data type: {platform_data.data_type}")
            logger.info(f"  Metadata: {platform_data.metadata}")
