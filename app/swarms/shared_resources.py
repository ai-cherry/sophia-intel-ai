...
class SharedToolsInterface(ABC):
    ...
    @abstractmethod
    async def get_optimal_model_for_role(self, role: str, personality: PersonalityType, task_complexity: float = 0.5) -> Dict[str, Any]:
        """Get optimal model configuration for a role with personality considerations"""
        pass
    
    @abstractmethod
    async def create_specialized_agent(self, role: str, config: Dict[str, Any], personality: PersonalityType) -> Agent:
...
class SharedResourceManager:
    ...
    async def _initialize_model_pool(self):
        ...
        self.model_pool = {
            "strategic_models": {
                "high_complexity": {"model": "claude-3-sonnet", "provider": "portkey", "temperature": 0.7},
                "medium_complexity": {"model": "gpt-4", "provider": "portkey", "temperature": 0.6},
                "low_complexity": {"model": "claude-3-haiku", "provider": "portkey", "temperature": 0.5}
            },
            "tactical_models": {
                "high_precision": {"model": "claude-3-opus", "provider": "portkey", "temperature": 0.3},
                "analysis": {"model": "gpt-4-turbo", "provider": "portkey", "temperature": 0.4},
                "rapid_response": {"model": "claude-3-haiku", "provider": "portkey", "temperature": 0.2}
            },
            "shared_models": {
    ...
    async def get_model_for_personality(self, role: str, personality: PersonalityType, complexity: float = 0.5) -> Dict[str, Any]:
    ...
    async def create_agent_with_personality(self, role: str, config: Dict[str, Any], personality: PersonalityType) -> Agent:
        ...
        if personality == PersonalityType.SOPHIA_STRATEGIC:
            agent_config["instructions"] = self._enhance_with_sophia_personality(config["instructions"], role)
            ...
            agent_config["instructions"] = self._enhance_with_artemis_personality(config["instructions"], role)
            agent_config["extra_data"] = {
    ...
    async def get_isolated_memory_client(self, orchestrator_id: str, session_id: str):
        ...
        isolation_key = f"{orchestrator_id}:{session_id}"
    ...
    async def cleanup_resources(self, orchestrator_id: str):
        ...
        keys_to_remove = [key for key in self.memory_clients.keys() if key.startswith(orchestrator_id)]
        for key in keys_to_remove:
            del self.memory_clients[key]
            
        # Release resource locks
        locks_to_release = [lock for lock in self.resource_locks.keys() if lock.startswith(orchestrator_id)]
