...
class QdrantToWeaviateMigrator:
    ...
    def print_migration_summary(self, results: Dict, total_time: float):
        ...
        print(f"\n" + "="*80)
        print(f"🎉 QDRANT → WEAVIATE MIGRATION SUMMARY")
        print(f"="*80)
        ...
        print(f"   • 75% memory reduction with RQ compression")
        print(f"   • <400ms query latency with optimized HNSW")
        print(f"   • Multi-tenant isolation for {len(self.agent_swarms)} swarms")
        print(f"   • Portkey integration with virtual keys")
