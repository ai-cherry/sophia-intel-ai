...
export function KnowledgePanel({ onSendToChat }) {
  ...
  const performUniversalSearch = async () => {
    ...
    try {
      ...
      const [mcpResponse, notionResponse] = await Promise.allSettled([
        ...
        fetch(
          `${API_BASE_URL}/context/search_multi_service?query=${encodeURIComponent(searchQuery.trim())}&services=rag,vector,notion&limit=10`,
          {
            method: "GET",
            headers: {
  ...
  const loadPendingPrinciples = async () => {
    ...
    try {
      const response = await fetch(
        `${API_BASE_URL}/api/notion/principles/pending`,
        {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
  ...
  const approvePrinciple = async (principleId) => {
    try {
      const response = await fetch(
        `${API_BASE_URL}/api/notion/principles/${principleId}/approve`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
      ...
      if (response.ok) {
        ...
        setPendingPrinciples((prev) =>
          prev.filter((p) => p.id !== principleId),
        );
      }
    } catch (error) {
  ...
  const createKnowledgePage = async () => {
    if (!newKnowledgeTitle.trim() || !newKnowledgeContent.trim() || isCreating)
      return;
          ...
          >
            <div className="space-y-2">
              ...
              <p className="text-xs text-muted-foreground">
                Searches MCP services, vector databases, and Notion knowledge
                base
              </p>
            </div>
                ...
                {searchResults.length === 0 &&
                  ...
                  !isSearching && (
                    <div className="text-center text-muted-foreground py-8">
                      ...
                      <p className="text-sm">
                        Includes memory, documents, and Notion pages
                      </p>
                    </div>
                  )}
                ...
                {searchResults.length > 0 && (
                  <div className="space-y-2">
                    <h3 className="font-medium text-sm flex items-center space-x-2">
                      ...
                      <span>
                        Memory & Vector Results ({searchResults.length})
                      </span>
                    </h3>
                          ...
                          <div className="flex items-center space-x-2 pt-2">
                            ...
                            {onSendToChat && (
                              <Button
                                ...
                                size="sm"
                                onClick={() =>
                                  sendToChat(result.content || "", result.title)
                                }
                                className="flex items-center space-x-1"
                              >
              ...
              <div className="space-y-3">
                {pendingPrinciples.length === 0 && !isLoadingPrinciples && (
                  <div className="text-center text-muted-foreground py-8">
                    ...
                    <p className="text-sm">
                      All canonical principles have been reviewed
                    </p>
                  </div>
                )}
                  ...
                  <Card key={index} className="p-4">
                    <div className="space-y-3">
                      ...
                      <div className="flex items-center justify-between text-xs text-muted-foreground">
                        <span>
                          Created: {formatTimestamp(principle.created_time)}
                        </span>
                      </div>
          ...
          >
            <div className="space-y-4">
              ...
              <div className="text-xs text-muted-foreground">
                <p>
                  Knowledge will be saved to the SOPHIA knowledge base in
                  Notion.
                </p>
                <p>
                  Use this to capture insights, procedures, or important
                  information.
                </p>
              </div>
            </div>
