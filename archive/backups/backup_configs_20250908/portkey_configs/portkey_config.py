...
class PortkeyConfig:
    ...
    def __post_init__(self):
        ...
        self.environment = (
            Environment(env_str) if env_str in ["dev", "staging", "prod"] else Environment.DEV
        )
...
class LoadBalanceStrategy:
    ...
    def to_config(self) -> dict[str, Any]:
        return {"mode": "loadbalance", "targets": self.targets}
...
class FallbackStrategy:
    ...
    def to_config(self) -> dict[str, Any]:
        return {"strategy": "fallback", "targets": self.targets}
...
class ABTestStrategy:
    ...
    def to_config(self) -> dict[str, Any]:
        return {"mode": "ab_test", "targets": self.targets, "metric": self.metric}
...
class PortkeyGateway:
        ...
        **kwargs,
        ...
        if role in [Role.PLANNER, Role.CRITIC] and self.config.semantic_cache_enabled:
            headers["x-portkey-cache"] = json.dumps(
                {"mode": "semantic", "max_age": self.config.cache_max_age_seconds}
            )

        # Add Portkey config for OpenRouter
        config = {"provider": "openrouter", "api_key": os.getenv("OPENROUTER_API_KEY")}
        headers["x-portkey-config"] = json.dumps(config)

        # Make the request
        response = self.chat_client.chat.completions.create(
            model=model, messages=messages, temperature=temperature, extra_headers=headers, **kwargs
        )
    ...
    @with_circuit_breaker("external_api")
    def embed(
        self, texts: list[str], model: str = "BAAI/bge-large-en-v1.5", **kwargs
    ) -> list[list[float]]:
        """
        ...
        response = self.embed_client.embeddings.create(
            model=model, input=texts, extra_headers=headers, **kwargs
        )
        ...
        **kwargs,
        ...
        for i in range(0, len(texts), batch_size):
            ...
            response = await self.async_embed_client.embeddings.create(
                model=model, input=batch, extra_headers=headers, **kwargs
            )
