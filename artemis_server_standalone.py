...
from app.artemis.agent_factory import artemis_factory, router as factory_router
...
async def startup_event():
    ...
    logger.info("⛔️ Initializing Artemis AGNO Technical Orchestrator with specialized tactical teams...")
    orchestrator_initialized = await artemis_orchestrator.initialize()
    if orchestrator_initialized:
        logger.info("🚀 Artemis AGNO Orchestrator ready with specialized technical intelligence teams")
    else:
...
async def health_check():
    orchestrator_status = await artemis_orchestrator.get_orchestrator_status() if orchestrator_initialized else {"status": "not_initialized"}
    return {
...
async def tactical_chat_legacy_endpoint(agent_id: str, request: dict):
    ...
    if orchestrator_initialized and agent_id in ["ares", "athena_tactical", "apollo_tech", "artemis_universal", "artemis"]:
        try:
            context = TechnicalContext(
            ...
            return {
                ...
                "command_type": response.command_type,
                "findings": response.findings[:3] if response.findings else [],  # Limit for legacy compatibility
...
async def tactical_status():
    ...
    return {
        ...
        "command_center": {
            "ares": {"active": True, "name": "Ares Rex", "operations": 312, "status": "combat_ready"},
            "athena_tactical": {"active": True, "name": "Athena Prime", "operations": 445, "status": "shields_up"},
            "apollo_tech": {"active": True, "name": "Apollo Prime", "operations": 156, "status": "engineering"}
        },
...
if __name__ == "__main__":
    port = int(os.getenv("ARTEMIS_PORT", "8000"))
    print(f"""
