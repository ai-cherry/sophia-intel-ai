...
class SophiaV7Auditor:
            ...
            "resourcePolicy": {
                ...
                "rules": [
                    ...
                    },
                    {
                        ...
                        "condition": {
                            "match": {
                                "expr": "request.resource.attr.domain in ['', 'github', 'linear']"
                            }
                        },
                    },
                ],
            ...
            if principal in rule["roles"] and action in rule["actions"]:
                ...
                if condition == "true":
                    ...
                    if "['gong', 'hubspot', 'salesforce']" in condition:
                        return (
                            domain in ["gong", "hubspot", "salesforce"]
                            and rule["effect"] == "EFFECT_ALLOW"
                        )
                    elif "['', 'github', 'linear']" in condition:
                        return (
                            domain in ["", "github", "linear"]
                            and rule["effect"] == "EFFECT_ALLOW"
                        )
    ...
    def audit_dependencies(self):
        ...
        print(
            f"  📊 Dependency readiness: {self.audit_results['dependencies']['readiness_score']:.1f}/10"
        )
