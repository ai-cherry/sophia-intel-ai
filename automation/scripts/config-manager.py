...
class ConfigurationManager:
    ...
    def get_configuration(
        ...
        config["computed"] = {
            ...
            "namespace": config["platforms"]
            .get(plat.value, {})
            .get("namespace", "sophia-intel-ai"),
        }
        return config
    def _merge_configs(
    ...
    def _generate_env_files(self, config: Dict[str, Any]) -> Dict[str, Path]:
        ...
        if config["computed"]["platform"] == "kubernetes":
            env_content.append(
                f"REDIS_URL=redis://redis-service.{namespace}.svc.cluster.local:6379"
            )
            env_content.append(
                f"POSTGRES_URL=postgresql://sophia:${{POSTGRES_PASSWORD}}@postgres-service.{namespace}.svc.cluster.local:5432/sophia"
            )
            env_content.append(
                f"WEAVIATE_URL=http://weaviate-service.{namespace}.svc.cluster.local:8080"
            )
        else:
            env_content.append("REDIS_URL=redis://localhost:6380")
            env_content.append(
                "POSTGRES_URL=postgresql://sophia:${POSTGRES_PASSWORD}@localhost:5432/sophia"
            )
...
def main():
    ...
    parser.add_argument(
        "--action",
        choices=["validate", "generate", "export"],
        default="validate",
    ...
    try:
        ...
        if args.action == "validate":
            ...
            config_str = config_manager.export_config(
                args.format, environment, platform, args.profile
            )
            if args.output:
