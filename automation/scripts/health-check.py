...
class SystemHealth:
    ...
    def to_dict(self):
        return {
            ...
            'summary': {
                'healthy_components': sum(1 for c in self.components if c.status == HealthStatus.HEALTHY),
                'degraded_components': sum(1 for c in self.components if c.status == HealthStatus.DEGRADED),
                'unhealthy_components': sum(1 for c in self.components if c.status == HealthStatus.UNHEALTHY),
...
class HealthChecker:
    def __init__(self):
        ...
        self.metrics = {
            'health_status': Gauge('sophia_component_health_status', 'Component health status (1=healthy, 0.5=degraded, 0=unhealthy)', ['component'], registry=self.registry),
            'response_time': Gauge('sophia_component_response_time_seconds', 'Component response time in seconds', ['component'], registry=self.registry),
            'check_total': Counter('sophia_health_checks_total', 'Total number of health checks', ['component', 'status'], registry=self.registry),
        }
    ...
    async def check_service_health(self, service_name: str, url: str, health_path: str = "/healthz") -> ComponentHealth:
    ...
    async def check_system_health(self) -> SystemHealth:
        ...
        checks = {
            ...
            'postgres': os.getenv('POSTGRES_URL', 'postgresql://sophia:sophia123@localhost:5432/sophia'),
        ...
        system_info = {
            ...
            'uptime_seconds': time.time() - os.path.getmtime('/proc/self/stat') if os.path.exists('/proc/self/stat') else 0
        }
        
        # Performance metrics
        performance_metrics = {
            'total_response_time': sum(c.response_time for c in valid_components),
            'average_response_time': sum(c.response_time for c in valid_components) / len(valid_components) if valid_components else 0,
    ...
    try:
        ...
        return JSONResponse(
            content={
                ...
                "components_summary": {
                    "healthy": sum(1 for c in health.components if c.status == HealthStatus.HEALTHY),
                    "degraded": sum(1 for c in health.components if c.status == HealthStatus.DEGRADED),
                    "unhealthy": sum(1 for c in health.components if c.status == HealthStatus.UNHEALTHY)
                }
            },
    ...
    if args.check_only:
        ...
        if args.format == "json":
            ...
            for component in health.components:
                ...
                print(f"{status_icon} {component.name}: {component.message} ({component.response_time:.3f}s)")
