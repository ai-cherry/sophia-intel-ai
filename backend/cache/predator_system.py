...
class PredictivePrefetcher:
    ...
    def record_access(self, key: str, timestamp: float = None):
        ...
        if len(self.access_patterns) > self.max_patterns:
            ...
            oldest_key = min(self.access_patterns.keys(), 
                           key=lambda k: max(self.access_patterns[k]) if self.access_patterns[k] else 0)
    ...
    def predict_next_keys(self, current_key: str, limit: int = 5) -> List[Tuple[str, float]]:
        ...
        for key, access_times in self.access_patterns.items():
            ...
            recent_accesses = [t for t in access_times if t > current_time - 3600]  # Last hour
...
class CachePredatorSystem:
    ...
    def _record_performance(self, start_time_ns: int, tier: str) -> None:
        ...
        if self.performance_samples:
            ...
            self.metrics.avg_retrieval_time_ms = total_latency / len(self.performance_samples)
        ...
        while True:
            try:
                ...
                total_hits = (self.metrics.l1_hits + self.metrics.l2_hits + 
                            self.metrics.l3_hits + self.metrics.l4_hits + self.metrics.l5_hits)
                total_requests = total_hits + self.metrics.total_misses
                
                if total_requests > 0:
                    current_hit_rate = total_hits / total_requests
                    
                    logger.info(f"Cache hit rate: {current_hit_rate:.3f} (target: {self.target_hit_rate:.3f})")
    ...
    async def _predictive_prefetcher(self):
        ...
        while True:
            try:
                ...
                recent_keys = [key for key, timestamp in self.access_history 
        ...
        return {
            ...
            "prefetch_performance": {
                ...
                "prefetch_accuracy": (
                    ...
                    if (self.metrics.prefetch_hits + self.metrics.prefetch_misses) > 0 else 0
                )
            },
            "performance": {
