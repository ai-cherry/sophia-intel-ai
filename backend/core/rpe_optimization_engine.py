...
class RPEOptimizationEngine:
    ...
    async def evaluate_any_activity(
        ...
        try:
            ...
            rpe_delta = total_impact / baseline['employee_count'] if baseline['employee_count'] > 0 else 0
            ...
            async with self.db_pool.acquire() as conn:
                    ...
                    for comp2 in components[i+1:]:
                        ...
                        if similarity > 0.7:  # Should be connected but aren't
                            ...
                            gaps.append({
                                ...
                                'urgency': 'critical' if gap_impact['daily_rpe_loss'] > 1000 else 'high'
                            })
                ...
                for need in unaddressed_needs:
                    gaps.append({
                        ...
                        'rpe_opportunity': need['potential_revenue'] / baseline['employee_count'] if baseline['employee_count'] > 0 else 0,
                        'resolution': f"Create project: {need['suggested_project']}",
                        'urgency': need['urgency']
                    })

                return sorted(gaps, key=lambda x: x.get('rpe_loss', x.get('rpe_opportunity', 0)), reverse=True)
    ...
    async def _get_current_baseline(self) -> Dict:
        ...
        # Check cache first
        if (self._baseline_cache and self._baseline_cache_time and 
            datetime.utcnow() - self._baseline_cache_time < timedelta(hours=1)):
    ...
    async def _calculate_revenue_impact(self, activity_type: str, activity_data: Dict, source: str) -> float:
        """Calculate direct revenue impact"""
        try:
    ...
    async def _determine_impact_type(self, activity_data: Dict) -> RPEImpactType:
        ...
        elif any(keyword in description for keyword in ['automation', 'efficiency', 'cost reduction']):
