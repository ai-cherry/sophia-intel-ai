...
class OptimizedStream:
    ...
    def __init__(self, user_id: str, compression: bool = True):
        ...
        self.send_stream, self.recv_stream = anyio.create_memory_object_stream(
            max_buffer_size=500,  # Larger buffer for better throughput
            item_type=bytes
        )
...
class MCPStreamSystem:
    ...
    async def process_bi_query(self, stream_id: str, query: str, context: Optional[Dict] = None) -> Dict[str, Any]:
    ...
    async def get_metrics(self) -> Dict[str, Any]:
        ...
        total_operations = sum(s.metrics.messages_sent + s.metrics.messages_received for s in self.active_streams.values())
