...
class PMIntelligenceMCPServer:
    ...
    def register_tools(self):
        ...
        self.tools["evaluate_rpe_impact"] = {
            ...
            "parameters": {
                "activity_type": {"type": "string", "description": "Type of activity (project, task, okr, etc.)"},
                "activity_data": {"type": "object", "description": "Activity data dictionary"},
                "source_system": {"type": "string", "description": "Source system (linear, asana, notion, etc.)"}
            },
        ...
        self.tools["auto_escalate_priorities"] = {
            ...
            "parameters": {
                "signal_source": {"type": "string", "description": "Signal source (gong, hubspot, industry, all)", "default": "all"}
            },
        ...
        self.tools["optimize_project_portfolio"] = {
            ...
            "parameters": {
                "optimization_target": {"type": "string", "description": "Optimization target", "default": "rpe_maximize"}
            },
        ...
        self.tools["generate_rpe_report"] = {
            ...
            "parameters": {
                "report_type": {"type": "string", "description": "Report type (executive, detailed, trends)", "default": "executive"}
            },
    ...
    async def _evaluate_rpe_impact(self, activity_type: str, activity_data: Dict, source_system: str) -> Dict:
    ...
    async def _detect_collaboration_gaps(self) -> List[Dict]:
        ...
        for gap in gaps:
            ...
            enriched_gap["estimated_resolution_time"] = await self._estimate_gap_resolution_time(gap)
    ...
    async def _auto_escalate_priorities(self, signal_source: str = "all") -> List[Dict]:
        ...
        for escalation in escalations:
            ...
            escalation["estimated_impact_timeline"] = await self._estimate_escalation_timeline(escalation)
    ...
    async def _optimize_project_portfolio(self, optimization_target: str = "rpe_maximize") -> Dict:
        ...
        for project in projects:
            try:
                ...
                project_impacts.append({
                    ...
                    'composite_score': impact.projected_rpe_impact * impact.confidence_score * resource_efficiency,
        ...
        return {
            ...
            'optimization_summary': {
                ...
                'high_impact_projects': len([p for p in optimized_portfolio if p['rpe_impact'] > 2000]),
                'average_confidence': sum(p['confidence'] for p in optimized_portfolio) / len(optimized_portfolio) if optimized_portfolio else 0,
                'resource_efficiency_range': {
                    'min': min(p['resource_efficiency'] for p in optimized_portfolio) if optimized_portfolio else 0,
                    'max': max(p['resource_efficiency'] for p in optimized_portfolio) if optimized_portfolio else 0
                }
            }
        }
    ...
    async def _query_project_intelligence(self, query: str, context: Dict = None) -> Dict:
        ...
        if any(keyword in query_lower for keyword in ['rpe', 'revenue per employee', 'revenue/employee']):
            # Direct RPE query
            baseline = await self.rpe_engine._get_current_baseline()
            current_rpe = baseline['current_revenue'] / baseline['employee_count'] if baseline['employee_count'] > 0 else 0
        ...
        elif any(keyword in query_lower for keyword in ['gap', 'collaboration', 'blockage', 'missing']):
        ...
        elif any(keyword in query_lower for keyword in ['project', 'priority', 'optimize', 'portfolio']):
        ...
        if report_type == "executive":
            return {
                ...
                'key_metrics': {
                    ...
                    'total_projected_rpe': optimization['total_projected_rpe'],
                    'optimization_opportunity': optimization['total_projected_rpe'] - dashboard_data['current_metrics']['rpe']
                },
                ...
                'executive_summary': await self._generate_executive_summary(dashboard_data, gaps, optimization)
            }
            ...
            return {
                ...
                'detailed_analysis': await self._generate_detailed_analysis(dashboard_data, gaps, optimization)
            }
    ...
    async def _generate_rpe_insights(self, query: str, baseline: Dict) -> str:
        ...
        try:
            ...
            if trends and len(trends) > 1:
                ...
                return f"RPE has been {trend_direction} by ${trend_amount:,.2f} over the last 30 days. " + \
    ...
    async def _generate_executive_summary(self, dashboard_data: Dict, gaps: List[Dict], optimization: Dict) -> str:
        """Generate executive summary"""
        current_rpe = dashboard_data['current_metrics']['rpe']
        total_projected = optimization['total_projected_rpe']
        critical_gaps = len([g for g in gaps if g.get('urgency') == 'critical'])

        return f"Current RPE: ${current_rpe:,.2f}. Portfolio optimization could increase to ${total_projected:,.2f}. " + \
               f"{critical_gaps} critical collaboration gaps require immediate attention. " + \
               f"Recommend prioritizing top 3 projects for maximum RPE impact."

    async def _generate_detailed_analysis(self, dashboard_data: Dict, gaps: List[Dict], optimization: Dict) -> Dict:
