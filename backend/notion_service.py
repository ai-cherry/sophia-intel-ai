...
class NotionService:
    ...
    async def search_databases(self, query: str, database_id: Optional[str] = None, max_results: int = 20) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=30.0) as client:
                response = await client.post(
                    f"{self.base_url}/databases/{target_db}/query",
                    headers=headers,
                    json=search_payload
                )
    ...
    async def get_pending_principles(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=30.0) as client:
                response = await client.post(
                    f"{self.base_url}/databases/{self.principles_db_id}/query",
                    headers=headers,
                    json=query_payload
                )
                response.raise_for_status()
