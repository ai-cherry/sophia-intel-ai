...
class OpusConfig(BaseModel):
    ...
    provider: str = Field(default="openrouter", description="Provider: openrouter, anthropic, or portkey")
    ...
    requirements: Optional[Dict[str, str]] = Field(default=None, description="Task-specific requirements")
...
def calculate_cost(provider: str, model: str, input_tokens: int, output_tokens: int) -> float:
    ...
    pricing = {
        ...
        "portkey": {
            "anthropic/claude-opus-4-1-20250805": {"input": 0.016, "output": 0.080}  # Slightly higher for routing
        }
    }
...
async def chat_with_opus(
    ...
    if request.config.requirements and request.config.requirements.get("volume") == "high":
        provider_priority = ["openrouter", "anthropic", "portkey"]  # OpenRouter for cost optimization
...
async def sophia_opus_integration():
    ...
    sophia_request = ChatRequest(
        messages=[
            ChatMessage(role="user", content="Hello! Please respond with 'Opus 4.1 integration working perfectly!'")
        ],
