    ...
    async def create_destination(self, config: DestinationConfig) -> Dict[str, Any]:
        ...
        try:
            request_data = {
                "workspaceId": self.workspace_id,
                "name": config.name,
                "destinationDefinitionId": await self._get_destination_definition_id(config.destination_type),
                "connectionConfiguration": config.connection_config
            }
            ...
            async with self.session.post(
                ...
                if response.status == 200:
                    ...
                    error_text = await response.text()
                    logger.error(f"❌ Failed to create destination: {response.status} - {error_text}")
                    return {"error": error_text}
                    ...
                    if job_id in self.active_jobs:
                        ...
                        if job_info.get("endedAt"):
                            ...
                            job_result.duration_seconds = (
                                ...
                                datetime.fromisoformat(job_result.start_time.replace('Z', '+00:00'))
                            ).total_seconds()
