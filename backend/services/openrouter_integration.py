...
import aiohttp
import hashlib
from backend.services.intelligent_cache import cached, get_cache
...
class OpenRouterIntegration:
    ...
    def select_model(
        ...
        if require_function_calling:
            available_models = [
                m for m in available_models 
                if self.models[m].supports_function_calling
            ]
    ...
    async def get_usage_stats(self) -> Dict[str, Any]:
        ...
        if self.usage_stats.total_tokens_used > 0:
            stats["cost_per_token"] = self.usage_stats.total_cost_usd / self.usage_stats.total_tokens_used
            stats["tokens_per_request"] = self.usage_stats.total_tokens_used / max(self.usage_stats.successful_requests, 1)
        else:
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        try:
            if self.session:
                async with self.session.get(f"{self.base_url}/models", timeout=aiohttp.ClientTimeout(total=5)) as response:
                    if response.status == 200:
