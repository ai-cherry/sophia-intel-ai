...
class PerformanceMonitor:
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            logger.info(f"✅ Performance monitor connected to Redis at {self.redis_host}:{self.redis_port}")
    ...
    async def get_performance_summary(self) -> Dict[str, Any]:
        ...
        all_p95_times = [m.p95_response_time_ms for m in self.endpoint_metrics.values() if m.p95_response_time_ms > 0]
...
async def sophia_performance_monitor():
    ...
    print(f"✅ System metrics: CPU {system_metrics.cpu_usage_percent:.1f}%, Memory {system_metrics.memory_usage_percent:.1f}%")
    
    # Get performance summary
    summary = await monitor.get_performance_summary()
    print(f"📊 Performance summary: {summary['overall_status']}, {summary['total_requests']} requests")
    
    # Get endpoint metrics
    endpoint_metrics = await monitor.get_endpoint_metrics()
    for endpoint, metrics in endpoint_metrics.items():
        print(f"🎯 {endpoint}: P95 {metrics['p95_response_time_ms']:.1f}ms, Error rate {metrics['error_rate']:.1f}%")
