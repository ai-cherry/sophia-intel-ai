...
class SwarmOrchestrator:
    ...
    def __init__(self, team: Team, config: SwarmConfiguration, memory: Optional[Any] = None):
        """
    ...
    async def run_debate(self, task: str, context: Optional[dict[str, Any]] = None) -> DebateResult:
        ...
        try:
            ...
            if critic_output:
                ...
                logger.info(
                    f"Critic verdict: {critic_output.verdict}, validated: {result.critic_validated}"
                )

            # Phase 3: Judge decision
            logger.info("Phase 3: Judge evaluating proposals and critic feedback...")
            judge_output = await self._run_judge_phase(
                proposals, critic_output, task, context, result
            )
            ...
            if judge_output:
                ...
                logger.info(
                    f"Judge decision: {judge_output.decision}, validated: {result.judge_validated}"
                )
            ...
            result.execution_time_ms = int((time.time() - start_time) * 1000)
            logger.info(
                f"Debate completed in {result.execution_time_ms}ms with "
    ...
    async def _run_generator_phase(
        self, task: str, context: Optional[dict[str, Any]], result: DebateResult
        ...
        for agent_name in generator_agents:
            generator_tasks.append(self._get_generator_proposal(agent_name, task, context))

        # Execute generators in parallel with timeout
        try:
            timeout = min(
                self.config.timeout_seconds / 3, 100
            )  # 1/3 of total timeout for generators
            generator_results = await asyncio.wait_for(
                asyncio.gather(*generator_tasks, return_exceptions=True), timeout=timeout
            )
            ...
            for agent_name, result_or_error in zip(generator_agents, generator_results):
                if isinstance(result_or_error, Exception):
                    ...
                    logger.info(
                        f"Received proposal from {agent_name}: "
    ...
    async def _get_generator_proposal(
        self, agent_name: str, task: str, context: Optional[dict[str, Any]]
    ) -> GeneratorProposal:
        """
    ...
    async def _run_judge_phase(
        ...
        proposals_text = self._format_proposals_for_review(proposals)
        critic_text = (
            self._format_critic_for_judge(critic_output)
            if critic_output
            else "No critic review available"
        )
    ...
    async def _evaluate_runner_gate(
        ...
        # Check reliability
        reliability_passed = (
            critic_output is not None and judge_output is not None and len(result.errors) == 0
        )
        ...
        if accuracy_score >= self.config.accuracy_threshold:
            if risk_level == RiskLevel.LOW and self.config.auto_approve_low_risk:
                ...
                reason = (
                    f"Manual approval required: {risk_level} risk, accuracy={accuracy_score:.1f}"
                )
                approval_actions = [
            ...
            reason = (
                f"Below accuracy threshold: {accuracy_score:.1f} < {self.config.accuracy_threshold}"
            )
    ...
    async def _run_agent_with_team(
        self, agent_name: str, messages: list[dict[str, str]], stream: bool = False
    ) -> str:
        """
        ...
        try:
            ...
            if hasattr(self.team, "run"):
                response = await asyncio.to_thread(
                    self.team.run, messages=messages, agent=agent_name, stream=stream
                )
                ...
                logger.warning("Team does not have run method, using mock response")
...
"""
    ...
    def _format_critic_for_judge(self, critic: CriticOutput) -> str:
        ...
        if critic.findings:
            findings_text = "\n".join(
                [f"- {category}: {', '.join(items)}" for category, items in critic.findings.items()]
            )
...
"""
    ...
    def _parse_critic_response(self, response: str) -> CriticOutput:
        ...
        return CriticOutput(
            verdict=verdict, findings={}, must_fix=[], nice_to_have=[], confidence_score=0.5
        )
