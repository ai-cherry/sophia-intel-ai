...
class ParallelSwarmOrchestrator(SwarmOrchestrator):
        ...
        for i, agent_name in enumerate(generator_agents):
            ...
            if agent_id not in self.portkey_clients:
                ...
                generator_tasks.append(
                    self._get_parallel_generator_proposal(agent_name, agent_id, task, context)
                )
    ...
    async def _get_parallel_generator_proposal(
        self, agent_name: str, agent_id: str, task: str, context: Optional[dict[str, Any]]
    ) -> GeneratorProposal:
        """
        ...
        try:
            ...
            response = await asyncio.to_thread(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": f"You are {agent_name}, a code generator using {model}",
                    },
    ...
    async def _run_critic_phase(
        ...
        if critic_id in self.portkey_clients:
            logger.info("🔍 Running critic with dedicated virtual key (no blocking)")
