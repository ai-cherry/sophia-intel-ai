...
from dev_mcp_unified.auth.rbac_manager import Permission, User, rbac_manager
...
class SophiaHandler:
    ...
    async def _handle_research_request(
        self, request: UniversalRequest, user: User
    ) -> dict[str, Any]:
    ...
    async def _handle_general_business_query(
        self, request: UniversalRequest, user: User
    ) -> dict[str, Any]:
...
class ArtemisHandler:
    ...
    async def _handle_general_technical_query(
        self, request: UniversalRequest, user: User
    ) -> dict[str, Any]:
...
class UniversalOrchestrator:
    ...
    async def process_request(self, request: UniversalRequest) -> OrchestrationResult:
        ...
        if not user:
            ...
            from dev_mcp_unified.auth.rbac_manager import Permission, User, UserRole
        ...
        self.classifier.classify_task_type(request.user_query)
    ...
    async def stream_response(
        ...
        try:
            ...
            yield {"type": "error", "error": str(e), "timestamp": datetime.utcnow().isoformat()}
