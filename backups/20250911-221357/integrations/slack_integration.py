...
class SlackClient:
    ...
    def __init__(self):
        ...
        cfg = cm.get_integration_config("slack")
        # If tokens exist but not explicitly enabled, enable implicitly for dev
        if not cfg.get("enabled") and (cfg.get("bot_token") or cfg.get("slack_bot_token") or cfg.get("slack_api_token")):
            cfg["enabled"] = True
        self.config = cfg
        if not self.config.get("enabled", False):
            raise SlackIntegrationError("Slack integration is not enabled. Set SLACK_ENABLED=true and provide tokens.")
        ...
        try:
            ...
            if method.upper() == "GET":
                async def do():
                    ...
                    return resp
                response = await with_retries(do)
