...
import { wsBaseUrl, apiBaseUrl } from '@/config/environment';
import { connectWithBackoff } from '@/lib/ws';
...
const UnifiedSuperDashboard: React.FC = () => {
  ...
  const connectWebSocket = () => {
    const conn = connectWithBackoff(
      ...
      () => {
        setConnected(true);
        sendCommand({ type: 'subscribe_all' });
        refreshAllData();
      },
      () => setConnected(false),
      { maxRetries: 8, initialDelayMs: 500 }
    );
    // @ts-ignore store to allow cleanup
    ws.current = (conn as any).socket;
  };
  ...
  const refreshAllData = async () => {
    try {
      const response = await fetch(`${apiBaseUrl}/orchestrator/status`);
      if (response.ok) {
        const data = await response.json();
        console.log('Fetched Sophia system data:', data);
        
        // Transform the API response to match our UnifiedSystem interface
        const transformedSystems: UnifiedSystem[] = data.systems.map((system: any) => ({
          id: system.id,
          name: system.name,
          type: system.type === 'mcp_server' ? 'MCP_SERVER' : 'SERVICE',
          status: system.status === 'active' ? 'active' : 'idle',
          capabilities: system.capabilities || [],
          metrics: {
            requests: system.metrics?.requests || 0,
            errors: system.metrics?.errors || 0,
            ...(system.metrics || {})
          },
          config: system.config || {},
          connections: [],
          last_activity: new Date().toISOString(),
          error_count: system.metrics?.errors || 0,
          metadata: {}
        }));
        ...
        // Set cost metrics
        setCostMetrics({
          total_cost_usd: data.cost_today || 0,
          total_tokens: data.tokens_today || 0,
          total_requests: 0,
          model_costs: {},
          provider_costs: {},
          daily_costs: []
        ...
        console.log(`Updated dashboard with ${transformedSystems.length} systems`);
      } else {
        console.error('Failed to fetch Sophia system status:', response.status);
        setConnected(false);
      }
    } catch (error) {
      console.error('Error fetching Sophia system data:', error);
      setConnected(false);
    }
  };
            ...
            <div className="flex items-center space-x-4">
              ...
              <div>
                ...
                <p className="text-sm text-gray-600 dark:text-gray-400">
                  Intelligent business intelligence & agent management
                </p>
              </div>
            </div>
