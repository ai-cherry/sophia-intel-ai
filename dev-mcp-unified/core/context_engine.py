"""
Context Engine for Multi-LLM MCP Server
Provides context strategies and rule-based selection.
"""
...
class ContextEngine:
    ...
    def _load_rules(self, rules_path: Optional[str]) -> Dict[str, Any]:
        path = Path(rules_path) if rules_path else Path(__file__).resolve().parents[1] / "config" / "rules.yaml"
        if path.exists():
    ...
    def _ctx_full_with_deps(self, target_file: Optional[str]) -> ContextBundle:
        ...
        if target_file:
            ...
            # naive import scan for python
            try:
                tree = ast.parse(files[target_file])
                imports = [n.names[0].name for n in ast.walk(tree) if isinstance(n, ast.Import)]
                rels = []
                for name in imports[:5]:  # limit
    ...
    def _ctx_snippet(self, target_file: Optional[str]) -> ContextBundle:
        ...
        if target_file:
            ...
            files[target_file] = content[:8000]  # cap
