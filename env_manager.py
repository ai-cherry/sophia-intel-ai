...
class SecureEnvManager:
    ...
    def rotate_secrets(self, service: Optional[str] = None) -> bool:
        ...
        try:
            ...
            if service:
                    ...
                    for key in key_patterns[service]:
                        if key in env_dict:
                            ...
                            if 'JWT' in key or 'SECRET' in key or 'TOKEN' in key:
                                ...
                                env_dict[key] = f"rotated_{hashlib.sha256(os.urandom(32)).hexdigest()[:32]}"
                            print(f"✅ Rotated: {key}")
                else:
                ...
                security_keys = [
                    'JWT_SECRET_KEY', 'API_KEY_SALT', 'ENCRYPTION_KEY', 
                    'MCP_AUTH_TOKEN', 'BACKUP_ENCRYPTION_KEY'
                ]
    ...
    def write_env(self, env_dict: Dict[str, str]) -> bool:
        ...
        try:
            ...
            categories = {
                ...
                'DATABASE': [k for k in env_dict.keys() if any(db in k for db in ['NEON', 'QDRANT', 'REDIS', 'DATABASE'])],
                'AI_PROVIDERS': [k for k in env_dict.keys() if any(ai in k for ai in ['OPENAI', 'ANTHROPIC', 'MISTRAL', 'LLAMA', 'HUGGING', 'GROK', 'DEEPSEEK', 'EXA'])],
                'SECURITY': [k for k in env_dict.keys() if any(sec in k for sec in ['JWT', 'KEY', 'SECRET', 'TOKEN', 'AUTH'])],
                'WORKFLOW': [k for k in env_dict.keys() if any(wf in k for wf in ['N8N', 'ESTUARY', 'GITHUB', 'PULUMI'])],
                'MONITORING': [k for k in env_dict.keys() if any(mon in k for mon in ['SENTRY', 'PROMETHEUS', 'GRAFANA', 'DATADOG'])],
    ...
    def get_status(self) -> Dict:
        ...
        if backups:
            ...
            status["last_backup"] = datetime.fromtimestamp(latest_backup.stat().st_mtime).isoformat()
