...
class EnvironmentVariableResolver:
        ...
        for file_path in self.project_root.rglob("*.py"):
            try:
                ...
                if re.search(cloud_url_pattern, content):
                    ...
                    postgres_pattern = r'postgresql://.*?os\.getenv\(["\']CLOUD_URL["\']'
    ...
    def resolve_circular_references(self) -> dict[str, str]:
        ...
        for circular_ref in self.circular_refs:
            ...
            if circular_ref.severity == "critical":
                ...
                resolved_value = self.safe_fallbacks.get(var_name, f"resolved_{var_name.lower()}")
            ...
            if circular_ref.variable == var_name:
                for file_path in circular_ref.files:
                    if "business" in file_path.lower():
                        return "business"
                    elif "coding" in file_path.lower() or "artemis" in file_path.lower():
                        return "coding"
    ...
    def create_environment_files(self) -> list[str]:
        ...
        with open(env_file, "w") as f:
            f.write("# Resolved Environment Variables - Circular References Eliminated\n")
    ...
    def _patch_file_content(self, content: str, circular_ref: CircularReference) -> str:
        ...
        if circular_ref.variable == "CLOUD_URL":
            ...
            redis_replacement = r'f"redis://{os.getenv(\'REDIS_HOST\', \'localhost\')}:\1"'
            patched_content = re.sub(redis_pattern, redis_replacement, patched_content)

            # Replace postgresql://CLOUD_URL patterns
            postgres_pattern = r'f?"?postgresql://\{os\.getenv\(["\']CLOUD_URL["\']\)\}:(\d+)"?'
            postgres_replacement = (
                r'f"postgresql://{os.getenv(\'POSTGRES_HOST\', \'localhost\')}:\1"'
            )
            patched_content = re.sub(postgres_pattern, postgres_replacement, patched_content)

            # Replace https://CLOUD_URL patterns (Qdrant)
            qdrant_pattern = r'f?"?https://\{os\.getenv\(["\']CLOUD_URL["\']\)\}:(\d+)"?'
            qdrant_replacement = r'f"https://{os.getenv(\'QDRANT_HOST\', \'localhost\')}:\1"'
            patched_content = re.sub(qdrant_pattern, qdrant_replacement, patched_content)

        if "import os" in patched_content and "def get_safe_env_var" not in patched_content:
...
def main():
    ...
    if circular_refs:
        ...
        print("\n📊 Resolution Summary:")
        print(f"   Total circular references: {report['summary']['total_circular_refs']}")
