    ...
    def __init__(self):
        ...
        self.environments = {
            'production': {
                ...
                'required_secrets': [
                    'OPENAI_API_KEY', 'ANTHROPIC_API_KEY', 'LAMBDA_API_KEY',
                    'EXA_API_KEY', 'HUGGINGFACE_API_TOKEN', 'TOGETHER_AI_API_KEY',
                    'OPENROUTER_API_KEY', 'PORTKEY_API_KEY', 'PULUMI_ACCESS_TOKEN',
                    'ARIZE_SPACE_ID', 'ARIZE_API_KEY', 'APIFY_API_TOKEN',
                    'DOCKER_USER_NAME', 'DOCKER_PERSONAL_ACCESS_TOKEN',
                    'PHANTOM_BUSTER_API_KEY', 'TWINGLY_API_KEY', 'TAVILY_API_KEY',
                    'ZENROWS_API_KEY'
                ],
                'aliases': ['prod', 'live']
            },
            'staging': {
                ...
                'required_secrets': [
                    'OPENAI_API_KEY', 'ANTHROPIC_API_KEY', 'LAMBDA_API_KEY',
                    'EXA_API_KEY', 'PULUMI_ACCESS_TOKEN', 'HUGGINGFACE_API_TOKEN'
                ],
                'aliases': ['stage', 'test']
            },
            'development': {
                ...
                'required_secrets': [
                    'OPENAI_API_KEY', 'LAMBDA_API_KEY', 'EXA_API_KEY'
                ],
                'aliases': ['dev', 'local']
            }
        }
...
'''
    ...
    def get_environment_status(self, environment: Optional[str] = None) -> Dict:
        ...
        env_vars = {
            key: '***' if os.getenv(key) else None
            for key in env_config['required_secrets']
        }
    ...
    def _check_pulumi_connectivity(self, pulumi_env: str) -> Dict:
        ...
        try:
            result = subprocess.run([
                'pulumi', 'env', 'get', pulumi_env
            ], capture_output=True, text=True, timeout=10)
            
            if result.returncode == 0:
                return {
                    'status': 'connected',
                    'message': 'Pulumi ESC environment accessible'
                }
            else:
                return {
                    'status': 'error',
            ...
            return {
                'status': 'timeout',
                'message': 'Pulumi ESC request timed out'
            }
        except Exception as e:
            return {
                'status': 'error',
...
def main():
    ...
    if args.command == 'list':
        ...
        if not args.quiet:
            ...
            print(f"Secret Coverage: {val['coverage_percentage']:.1f}% ({val['present_secrets']}/{val['total_secrets']})")
        ...
        if result['status'] == 'success':
            print(f"✅ Environment monitoring setup complete")
            print(f"📊 Monitoring script: {result['monitoring_script']}")
            print(f"📝 Log file: {result['log_file']}")
        else:
            print(f"❌ Monitoring setup failed")
            sys.exit(1)
