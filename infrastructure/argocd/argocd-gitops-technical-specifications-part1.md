...
│   ├── artemis-apps.yaml           # Artemis service applications
...
```yaml
# environments/production/patches/artemis-resources.yaml
apiVersion: apps/v1
kind: Deployment
metadata:
...
### 3.1 Artemis Service Applications

```yaml
# Artemis Orchestrator Application
apiVersion: argoproj.io/v1alpha1
kind: Application
metadata:
  name: artemis-orchestrator
  namespace: argocd
  labels:
    app.kubernetes.io/name: artemis-orchestrator
    app.kubernetes.io/part-of: artemis-suite
    tier: orchestration
  annotations:
    argocd.argoproj.io/sync-wave: "10"
    notifications.argoproj.io/subscribe.on-deployed.slack: artemis-team
    notifications.argoproj.io/subscribe.on-sync-failed.slack: artemis-team
  finalizers:
    - resources-finalizer.argocd.argoproj.io
spec:
  project: artemis
  source:
    repoURL: https://github.com/sophia-intel-ai/gitops
    targetRevision: HEAD
    path: services/artemis/orchestrator/overlays/production
    kustomize:
      images:
      - ghcr.io/sophia-intel-ai/artemis-orchestrator:v2.1.0
  destination:
    server: https://kubernetes.default.svc
    namespace: artemis-system
  syncPolicy:
...
---
# Artemis Worker Pool Application
apiVersion: argoproj.io/v1alpha1
kind: Application
metadata:
  name: artemis-worker-pool
  namespace: argocd
  labels:
    app.kubernetes.io/name: artemis-worker
    app.kubernetes.io/part-of: artemis-suite
    tier: processing
  annotations:
...
spec:
  ...
  source:
    repoURL: https://github.com/sophia-intel-ai/gitops
    targetRevision: HEAD
    path: services/artemis/worker/overlays/production
  destination:
    server: https://kubernetes.default.svc
    namespace: artemis-system
  syncPolicy:
...
spec:
  ...
  - matrix:
      ...
      - list:
          elements:
          - service: artemis-orchestrator
            namespace: artemis-system
            path: services/artemis/orchestrator
          - service: artemis-worker
            namespace: artemis-system
            path: services/artemis/worker
