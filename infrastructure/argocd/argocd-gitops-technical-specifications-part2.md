  ...
  template:
    spec:
      ...
      - name: test
        ...
        env:
        - name: TARGET_URL
          value: "http://sophia-orchestrator.sophia-system.svc.cluster.local"
```
...
spec:
  ...
  service:
    ...
    targetPort: 8080
    gateways:
    - istio-system/sophia-gateway
    hosts:
    - sophia.sophia-sophia.local
  analysis:
    interval: 30s
    threshold: 5
    maxWeight: 50
    ...
    - name: acceptance-test
      type: pre-rollout
      url: http://flagger-loadtester.sophia-system/
      timeout: 30s
      metadata:
        type: bash
        cmd: "curl -sd 'test' http://sophia-orchestrator-canary.sophia-system:8080/health"
    - name: load-test
      type: rollout
      url: http://flagger-loadtester.sophia-system/
      metadata:
        cmd: "hey -z 2m -q 10 -c 2 http://sophia-orchestrator-canary.sophia-system:8080/"
```
...
data:
  ...
  policy.csv: |
    ...
    p, role:sophia-team, exec, create, sophia/*, deny  # No exec in prod
...
jobs:
  deploy:
    ...
    - name: Update Image Tag
      run: |
        cd services/sophia/orchestrator/overlays/production
        kustomize edit set image sophia-orchestrator=ghcr.io/sophia-intel-ai/sophia-orchestrator:${{ github.sha }}
    ...
    - name: Commit and Push
      run: |
        ...
        git add .
        git commit -m "Update sophia-orchestrator to ${{ github.sha }}"
        git push
    
    - name: Sync ArgoCD Application
      run: |
        argocd app sync sophia-orchestrator-production \
    ...
    - name: Wait for Rollout
      run: |
        argocd app wait sophia-orchestrator-production \
          --auth-token ${{ secrets.ARGOCD_TOKEN }} \
          --server argocd.sophia-intel-ai.com \
          --grpc-web \
          --timeout 600 \
...
metadata:
  ...
  annotations:
    argocd-image-updater.argoproj.io/image-list: |
      sophia=ghcr.io/sophia-intel-ai/sophia-orchestrator
    argocd-image-updater.argoproj.io/sophia.update-strategy: semver
    argocd-image-updater.argoproj.io/sophia.allow-tags: "regexp:^v[0-9]+\\.[0-9]+\\.[0-9]+$"
