...
data:
  ...
  test-rollback.sh: |
    ...
    argocd app sync sophia-test --revision v1.0.0
    argocd app wait sophia-test --health
    
    # Deploy bad version
    argocd app sync sophia-test --revision v1.0.1-broken
    
    # Wait for health check failure
    sleep 30
    
    # Trigger rollback
    argocd app rollback sophia-test 0
    argocd app wait sophia-test --health
    
    # Verify rollback successful
    REVISION=$(argocd app get sophia-test -o json | jq -r '.status.sync.revision')
    ...
    spec:
      ...
      - name: validator
        ...
        - |
          ...
          CURRENT_VERSION=$(kubectl get deploy sophia-orchestrator -o jsonpath='{.spec.template.spec.containers[0].image}')
          EXPECTED_VERSION="ghcr.io/sophia-intel-ai/sophia-orchestrator:v2.0.0"
    ...
    spec:
      ...
      - name: rollback
        ...
        - |
          ...
          # 5. Verify services
          for ns in sophia-system sophia-system; do
...
```yaml
# Example: Complete Sophia Service GitOps Configuration
# File: services/sophia/orchestrator/base/kustomization.yaml
...
- name: sophia-orchestrator
  newName: ghcr.io/sophia-intel-ai/sophia-orchestrator
  newTag: v2.1.0
...
```yaml
# File: services/sophia/orchestrator/overlays/production/kustomization.yaml
apiVersion: kustomize.config.k8s.io/v1beta1
kind: Kustomization
