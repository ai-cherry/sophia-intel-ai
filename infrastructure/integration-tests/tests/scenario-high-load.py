...
class HighLoadScenario:
    ...
    async def _deploy_ai_workload_simulators(self):
        """Deploy AI workload simulators ( and Sophia)"""
        # Deploy  simulator
        _deployment = client.V1Deployment(
            metadata=client.V1ObjectMeta(
                name="-simulator",
                namespace=self.test_namespace,
                labels={"app": "", "component": "ai-workload"},
            ),
            spec=client.V1DeploymentSpec(
                replicas=1,
                selector=client.V1LabelSelector(match_labels={"app": ""}),
        ...
        try:
            self.apps_v1.create_namespaced_deployment(
                namespace=self.test_namespace, body=_deployment
            )
            self.apps_v1.create_namespaced_deployment(
    ...
    async def _configure_ai_workload_scaling(self):
        ...
        _scaler = {
            ...
            "kind": "ScaledObject",
            "metadata": {"name": "-ai-scaler", "namespace": self.test_namespace},
            "spec": {
                "scaleTargetRef": {"name": "-simulator"},
                "minReplicaCount": 1,
                "maxReplicaCount": 20,
                "pollingInterval": 5,
                "cooldownPeriod": 30,
                "triggers": [
                    {
                        ...
                        "metadata": {
                            ...
                            "threshold": "10",
                            "query": f'sum(ai_inference_queue_depth{{namespace="{self.test_namespace}",app=""}})',
                        },
                    },
    ...
    async def _verify_normal_operation(self):
        ...
        code_sim = self.apps_v1.read_namespaced_deployment(
            name="code-simulator", namespace=self.test_namespace
        )
        sophia = self.apps_v1.read_namespaced_deployment(
            name="sophia-simulator", namespace=self.test_namespace
        )
        assert code_sim.status.ready_replicas == 1
...
ai_inference_queue_depth{{namespace="{self.test_namespace}",app=""}} {i * 5}
...
"""
    ...
    async def _verify_keda_scaling_response(self, start_time: float) -> Dict:
        ...
        while time.time() - start_time < 15:
            ...
            if code_sim.status.replicas > 1:
                metrics["code_scaled"] = True
                metrics["code_replicas"] = code_sim.status.replicas
            if sophia.status.replicas > 1:
                metrics["sophia_scaled"] = True
                metrics["sophia_replicas"] = sophia.status.replicas
            if metrics.get("code_scaled") and metrics["sophia_scaled"]:
                metrics["scaling_time"] = time.time() - start_time
                break
            await asyncio.sleep(1)
        # Verify scaling happened within target time
        assert metrics["_scaled"], " did not scale"
        ...
        logger.info(f"  : {metrics['_replicas']} replicas")
    ...
    async def _test_sustained_high_load(self) -> Dict:
        ...
        while time.time() - sustained_start < 120:
            ...
            metrics_data = """
# TYPE ai_inference_queue_depth gauge
ai_inference_queue_depth{namespace="%s",app=""} 100
# TYPE ai_training_batch_pending gauge
...
""" % (
            ...
            if code_sim.status.replicas >= 20:
    ...
    async def _test_load_reduction_and_recovery(self) -> Dict:
        ...
        for i in range(10, 0, -1):
            metrics_data = f"""
# TYPE ai_inference_queue_depth gauge
ai_inference_queue_depth{{namespace="{self.test_namespace}",app=""}} {i * 2}
# TYPE ai_training_batch_pending gauge
...
"""
        ...
        while time.time() - recovery_start < 90:
            ...
            if code_sim.status.replicas <= 2 and sophia.status.replicas <= 2:
                scaled_down = True
                metrics["scale_down_time"] = time.time() - recovery_start
                metrics["final_replicas"] = {
                    "code": code_sim.status.replicas,
    ...
    async def _generate_scenario_report(
        ...
        logger.info("\n⚖️ Scaling Results:")
        logger.info(f"  : 1 → {scaling_metrics['_replicas']} replicas")
