...
```bash
# Watch key metrics
watch -n 10 'kubectl get hpa,scaledobjects -A | grep -E "sophia|sophia"'
...
- **Sophia Dashboard**: https://sophia.sophia-intel-ai.com (KEDA tab)
...
kubectl scale deployment sophia-worker --replicas=10 -n sophia-system
```
...
kubectl get events -n sophia-system | grep -i scale | tail -20

# 2. Identify cause
curl -s "$PROMETHEUS_URL/api/v1/query?query=rate(keda_scaled_object_events_total[1m])"

# 3. Stabilize workload
kubectl patch scaledobject sophia-scaler -n sophia-system --type='json' \
  -p='[{"op": "replace", "path": "/spec/pollingInterval", "value": 20}]'

# 4. Reset circuit breaker
kubectl annotate scaledobject sophia-scaler -n sophia-system \
  keda.sh/circuit-breaker-triggered- --overwrite

# 5. Monitor recovery
watch -n 5 'kubectl get scaledobject sophia-scaler -n sophia-system -o jsonpath="{.status.conditions}"'
```
...
kubectl patch scaledobject sophia-scaler -n sophia-system \
  --type merge -p '{"spec":{"pollingInterval":5}}'

# 3. Check metrics latency
kubectl exec -n sophia-system deployment/sophia-worker -- \
  redis-cli -h redis.sophia-system.svc.cluster.local --latency
...
kubectl get secrets -n sophia-system | grep credentials

# 3. Test connectivity
# For Redis
kubectl run redis-test --rm -it --image=redis:alpine -n sophia-system -- \
  redis-cli -h redis.sophia-system.svc.cluster.local ping
...
kubectl delete scaledobject sophia-scaler -n sophia-system
kubectl apply -f scalers/sophia-scaledobject.yaml
```
...
kubectl get scaledobject sophia-scaler -n sophia-system -o yaml | grep -A5 threshold
...
kubectl scale deployment sophia-worker --replicas=20 -n sophia-system
...
python3 tests/load-test.py --redis-host redis.sophia-system.svc.cluster.local \
