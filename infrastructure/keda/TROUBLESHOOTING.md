...
kubectl describe scaledobject sophia-scaler -n sophia-system
...
kubectl get triggerauthentication -n sophia-system

# Verify secrets exist
kubectl get secrets redis-credentials -n sophia-system

# Test connection manually
kubectl run redis-test --rm -it --image=redis:alpine -- redis-cli -h redis.sophia-system.svc.cluster.local ping
```
...
```bash
# Check annotations
kubectl get scaledobject sophia-scaler -n sophia-system -o jsonpath='{.metadata.annotations}'

# Reset circuit breaker
kubectl annotate scaledobject sophia-scaler -n sophia-system keda.sh/circuit-breaker-triggered- --overwrite

# Monitor scale events
kubectl get events -n sophia-system | grep -i scale
```
...
kubectl get scaledobject sophia-scaler -n sophia-system -o jsonpath='{.spec.pollingInterval}'
```
...
kubectl run redis-cli --rm -it --image=redis:alpine -n sophia-system -- \
  redis-cli -h redis.sophia-system.svc.cluster.local ping

# Check Redis service
kubectl get svc -n sophia-system | grep redis
...
kubectl run -it --rm debug --image=alpine --restart=Never -n keda-system -- \
  nslookup redis.sophia-system.svc.cluster.local
```
...
watch -n 2 'kubectl get hpa,deployment -A | grep -E "sophia|sophia"'
...
kubectl get scaledobject sophia-scaler -n sophia-system -o json | \
  jq '.status'

# Test external metrics API
kubectl get --raw "/apis/external.metrics.k8s.io/v1beta1/namespaces/sophia-system/redis_list_length"
