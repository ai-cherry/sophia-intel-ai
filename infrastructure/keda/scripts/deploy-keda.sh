...
metadata:
  name: artemis-worker-hpa-backup
...
deploy_scaledobjects_canary() {
    ...
    # Deploy Artemis ScaledObject with reduced scope
    log_info "Deploying Artemis ScaledObject (canary)..."
    kubectl apply -f "$KEDA_DIR/scalers/artemis-scaledobject.yaml"

    # Wait and validate
    sleep 30
    validate_scaledobject "artemis-scaler" "$NAMESPACE_ARTEMIS"
