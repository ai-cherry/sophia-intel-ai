    ...
    cat > "$BASELINE_FILE" <<EOF
{
  "metadata": {
    ...
    "environment": {
      "sophia_namespace": "$NAMESPACE_ARTEMIS",
...
capture_hpa_metrics() {
    ...
    if kubectl get hpa -n "$NAMESPACE_ARTEMIS" &> /dev/null; then
        local sophia_hpas=$(kubectl get hpa -n "$NAMESPACE_ARTEMIS" -o json | \
            jq -c '.items[] | {
                name: .metadata.name,
...
capture_deployment_metrics() {
    ...
    # Sophia deployments
    local sophia_deps=$(kubectl get deployments -n "$NAMESPACE_ARTEMIS" -o json | \
        jq -c '.items[] | {
            name: .metadata.name,
...
query_prometheus_metrics() {
    ...
    declare -A queries=(
        ...
        ["sophia_queue_length"]='redis_list_length{list="sophia:task:queue"}'
...
capture_scaling_behavior() {
    ...
    for ((i=1; i<=iterations; i++)); do
        ...
        local sophia_replicas=$(kubectl get deployment sophia-worker -n "$NAMESPACE_ARTEMIS" \
        ...
        local sample=$(jq -n \
            --arg ts "$timestamp" \
            --arg ar "$sophia_replicas" \
            --arg sr "$sophia_replicas" \
            '{
                timestamp: $ts,
                sophia_replicas: ($ar | tonumber),
