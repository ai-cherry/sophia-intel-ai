...
enable_hpa_fallback() {
    ...
    # Sophia HPA
    if kubectl get hpa sophia-worker-hpa-backup -n "$NAMESPACE_ARTEMIS" &> /dev/null; then
        ...
        kubectl get hpa sophia-worker-hpa-backup -n "$NAMESPACE_ARTEMIS" -o yaml | \
            sed 's/sophia-worker-hpa-backup/sophia-worker-hpa/g' | \
            kubectl apply -f -
    else
...
remove_scaledobjects() {
    ...
    kubectl delete scaledobject sophia-scaler -n "$NAMESPACE_ARTEMIS" --ignore-not-found
...
verify_rollback() {
    ...
    if ! kubectl get hpa sophia-worker-hpa -n "$NAMESPACE_ARTEMIS" &> /dev/null; then
    ...
    local sophia_ready=$(kubectl get deployment sophia-worker -n "$NAMESPACE_ARTEMIS" \
...
EOF
    ...
    kubectl get hpa -A | grep -E "sophia|sophia" >> "$report_file" 2>/dev/null || echo "None found" >> "$report_file"
