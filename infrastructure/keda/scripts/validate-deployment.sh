...
validate_scaledobjects() {
    ...
    if kubectl get scaledobject artemis-scaler -n "$NAMESPACE_ARTEMIS" &> /dev/null; then
        local artemis_ready=$(kubectl get scaledobject artemis-scaler -n "$NAMESPACE_ARTEMIS" \
            -o jsonpath='{.status.conditions[?(@.type=="Ready")].status}' 2>/dev/null)

        local artemis_active=$(kubectl get scaledobject artemis-scaler -n "$NAMESPACE_ARTEMIS" \
            -o jsonpath='{.status.conditions[?(@.type=="Active")].status}' 2>/dev/null)

        if [ "$artemis_ready" = "True" ] && [ "$artemis_active" = "True" ]; then
            record_check "Artemis ScaledObject" "pass" "Ready and Active"
        else
            record_check "Artemis ScaledObject" "fail" "Not Ready or Active"
        fi

        # Check if HPA was created
        if kubectl get hpa artemis-scaler -n "$NAMESPACE_ARTEMIS" &> /dev/null; then
            record_check "Artemis HPA Created" "pass" "HPA exists"
        else
            record_check "Artemis HPA Created" "fail" "HPA missing"
        fi
    else
        record_check "Artemis ScaledObject" "fail" "Not Found"
    fi
...
validate_circuit_breaker() {
    ...
    local cb_enabled=$(kubectl get scaledobject artemis-scaler -n "$NAMESPACE_ARTEMIS" \
        -o jsonpath='{.metadata.annotations.keda\.sh/circuit-breaker-enabled}' 2>/dev/null)

    if [ "$cb_enabled" = "true" ]; then
        record_check "Circuit Breaker Enabled" "pass" "Configured on Artemis"
    else
        record_check "Circuit Breaker Enabled" "fail" "Not Configured"
    fi

    # Check max events configuration
    local max_events=$(kubectl get scaledobject artemis-scaler -n "$NAMESPACE_ARTEMIS" \
    ...
    if kubectl get hpa artemis-worker-hpa-backup -n "$NAMESPACE_ARTEMIS" &> /dev/null || \
       kubectl get hpa artemis-worker-hpa -n "$NAMESPACE_ARTEMIS" &> /dev/null; then
...
validate_connectivity() {
    ...
    kubectl run redis-test --image=redis:alpine --rm -i --restart=Never -n "$NAMESPACE_ARTEMIS" -- \
        redis-cli -h redis.artemis-system.svc.cluster.local ping 2>/dev/null | grep -q "PONG"
