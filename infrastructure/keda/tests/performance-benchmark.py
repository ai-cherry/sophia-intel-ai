...
class KEDAPerformanceBenchmark:
    ...
    async def _capture_state(self) -> Dict[str, Any]:
        ...
        try:
            code_dep = self.k8s_apps_v1.read_namespaced_deployment(
                name="code-worker", namespace=self.config.namespace_
            )
            state["deployments"]["code-worker"] = code_dep.status.replicas or 0
            state["total_replicas"] += code_dep.status.replicas or 0
        except Exception:
            pass
...
class MetricsCollector:
    ...
    async def collect_metrics(self) -> Dict[str, float]:
        ...
        async with aiohttp.ClientSession() as session:
            ...
            # Queue length ()
            queue_query = 'redis_list_length{list=":task:queue"}'
