...
class LambdaLabsScanner:
        ...
        for itype in instance_types:
            if isinstance(itype, dict):
                type_info = {
                    ...
                    "vcpus": (
                        itype.get("specs", {}).get("vcpus", 0)
                        if isinstance(itype.get("specs"), dict)
                        else 0
                    ),
                    "memory_gib": (
                        itype.get("specs", {}).get("memory_gib", 0)
                        if isinstance(itype.get("specs"), dict)
                        else 0
                    ),
                    "storage_gib": (
                        itype.get("specs", {}).get("storage_gib", 0)
                        if isinstance(itype.get("specs"), dict)
                        else 0
                    ),
                    "gpus": (
                        itype.get("specs", {}).get("gpus", [])
                        if isinstance(itype.get("specs"), dict)
                        else []
                    ),
                }
    ...
    def generate_recommendations(self):
        ...
        expensive_instances = [
            ...
            if any(
                t["name"] == i["instance_type"] and t["price_cents_per_hour"] > 1000
