...
"""
        ...
        return {
            ...
            "hosts_added": ["lambda-sophia-h100", "lambda-sophia-a100", "lambda-sophia-dev", "lambda-sophia-staging"]
        }
    ...
    def discover_lambda_instances(self) -> Dict:
        ...
        if not self.lambda_api_key:
            ...
            instances = {k: v for k, v in instances.items() if v and v != f"LAMBDA_{k.upper()}_IP_NOT_SET"}
            
            if not instances:
                raise Exception("No Lambda Labs instances configured. Set LAMBDA_*_IP environment variables.")
    ...
    def deploy_ssh_keys(self) -> Dict:
        ...
        for instance_name, ip in self.instances.items():
            ...
            try:
                ...
                if result.returncode == 0:
                    deployment_results[instance_name] = {"status": "success", "method": "ssh-copy-id"}
                else:
    ...
    def manual_key_deployment(self, ip: str, public_key: str) -> Dict:
        ...
        try:
            ...
            for auth in auth_methods:
                try:
                    if auth["password"]:
                        ssh.connect(ip, username=auth["username"], password=auth["password"], timeout=10)
                    else:
...
'''
    ...
    def generate_setup_report(self) -> Dict:
        ...
        successful_steps = sum(1 for result in self.setup_results.values() 
        ...
        for step_name, result in self.setup_results.items():
            if step_name in ["test_connections", "setup_remote_environment", "deploy_sophia_platform"]:
                if result["status"] == "SUCCESS" and isinstance(result.get("result"), dict):
                    successful_instances += sum(1 for instance_result in result["result"].values()
        ...
        report = {
            ...
            "summary": {
                ...
                "instance_success_rate": f"{(successful_instances/total_instances)*100:.1f}%" if total_instances > 0 else "0%"
            },
    ...
    def _generate_next_actions(self) -> List[str]:
        ...
        failed_steps = [name for name, result in self.setup_results.items() 
...
def main():
    ...
    print(f"Instances: {report['summary']['successful_instances']}/{report['summary']['total_instances']} ({report['summary']['instance_success_rate']})")
