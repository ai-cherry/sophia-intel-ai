...
class BaseMCPClient(ABC):
    ...
    async def semantic_search(
        self, query: str, k: int = 8, swarm_stage: Optional[str] = None, filters: Optional[Dict[str, Any]] = None
    ) -> List[SearchResult]:
        """
        ...
        try:
            ...
            # Cache results
            self.search_cache[cache_key] = {"results": results, "timestamp": time.time()}
    ...
    async def store_context(
        ...
        try:
            payload = {
                ...
                "service": self.service_name,
                "metadata": {**(metadata or {}), **self._get_context_fields(), "timestamp": time.time()},
            }
    ...
    async def execute_swarm_action(self, action: str, payload: Dict[str, Any]) -> Optional[Dict[str, Any]]:
...
class MCPClientManager:
    ...
    async def search_all(self, query: str, k: int = 8, swarm_stage: Optional[str] = None) -> List[SearchResult]:
