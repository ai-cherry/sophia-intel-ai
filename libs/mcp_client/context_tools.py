...
class ContextAwareToolWrapper:
    ...
    async def wrap_tool(
        self, tool_func: Callable, tool_name: Optional[str] = None, context_types: Optional[List[str]] = None
    ) -> Callable:
        """
        ...
        async def context_aware_wrapper(*args, **kwargs):
            ...
            try:
                ...
                context_query = self._build_context_query(actual_tool_name, args, kwargs)

                relevant_context = await self.mcp_client.query_context(
                    query=context_query, top_k=3, threshold=0.6, context_types=relevant_context_types
                )

                # Log context retrieval
                logger.info(f"Retrieved {len(relevant_context)} context entries for {actual_tool_name}")
                ...
                await self._store_tool_execution(
                    actual_tool_name, args, kwargs, result, execution_time, relevant_context, success=True
                )
                ...
                await self._store_tool_execution(
                    actual_tool_name, args, kwargs, {"error": str(e)}, execution_time, [], success=False
                )
    ...
    async def get_tool_suggestions(self, current_context: str, file_path: Optional[str] = None) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            relevant_context = await self.mcp_client.query_context(
                query=query, top_k=10, threshold=0.5, context_types=["tool_usage"]
            )
    ...
    async def get_similar_executions(
        self, tool_name: str, params: Dict[str, Any], limit: int = 5
    ) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            similar_executions = await self.mcp_client.query_context(
                query=query, top_k=limit, threshold=0.7, context_types=["tool_usage"]
            )
    ...
    async def learn_from_feedback(self, tool_name: str, execution_id: str, feedback: Dict[str, Any]):
        """
        ...
        try:
            ...
            await self.mcp_client.store_context(
                ...
                metadata={"tool_name": tool_name, "execution_id": execution_id, "feedback_type": "user_feedback"},
            )
    ...
    async def _store_tool_execution(
        ...
        try:
            ...
            execution_data = {
                "tool_name": tool_name,
                "parameters": {"args": [str(arg) for arg in args], "kwargs": {k: str(v) for k, v in kwargs.items()}},
    ...
    def _build_context_query(self, tool_name: str, args: tuple, kwargs: Dict[str, Any]) -> str:
    ...
    def _extract_common_params(self, tool_contexts: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for param, value_counts in param_counts.items():
            if value_counts:
                ...
                common_params[param] = {"value": most_common_value, "frequency": value_counts[most_common_value]}
...
class EnhancedFileOperations:
    ...
    async def read_file_with_context(self, file_path: str, include_context: bool = True) -> Dict[str, Any]:
        """
        ...
        if include_context:
            ...
            suggestions = await self.wrapper.get_tool_suggestions(f"reading file {file_path}", file_path)
        ...
        await self.mcp_client.store_context(
            ...
            metadata={"file_path": file_path, "operation": "read", "file_size": len(content)},
        )
