...
from .sophia_client import MCPServerError, SophiaMCPClient
...
class PredictiveAssistant:
    ...
    def __init__(self, mcp_client: SophiaMCPClient):
        ...
        self.prediction_stats = {"predictions_made": 0, "predictions_correct": 0, "last_updated": datetime.now()}

    async def predict_next_actions(
        self, current_context: str, recent_actions: List[Dict[str, Any]], max_suggestions: int = 5
    ) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            similar_sequences = await self._find_similar_sequences(action_sequence, current_context)

            # Generate predictions based on patterns
            predictions = self._generate_action_predictions(similar_sequences, current_context)

            # Enhance predictions with context-specific suggestions
            enhanced_predictions = await self._enhance_predictions_with_context(predictions, current_context)

            # Sort by confidence and return top suggestions
            enhanced_predictions.sort(key=lambda x: x.get("confidence", 0), reverse=True)
    ...
    async def suggest_relevant_files(self, current_task: str, current_files: List[str] = None) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            task_related = await self.repo_intelligence.semantic_code_search(query=current_task, max_results=10)
    ...
    async def predict_code_completion(
        self, file_path: str, current_code: str, cursor_position: Optional[Tuple[int, int]] = None
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            context_info = self._analyze_code_context(current_code, cursor_position, file_path)

            # Find similar code patterns
            similar_patterns = await self.repo_intelligence.find_similar_code(
                context_info["surrounding_code"], context_info["language"], similarity_threshold=0.6
            )

            # Generate completion suggestions
            suggestions = self._generate_code_completions(context_info, similar_patterns)
    ...
    async def get_proactive_help(
        self, current_situation: str, user_skill_level: str = "intermediate"
    ) -> List[Dict[str, Any]]:
        """
    ...
    async def learn_from_user_action(
        self, predicted_action: str, actual_action: str, context: Dict[str, Any], was_helpful: bool = True
    ):
        """
        ...
        try:
            ...
            logger.info(
                f"Learning recorded: predicted='{predicted_action}', actual='{actual_action}', accurate={predicted_action == actual_action}"
            )
    ...
    async def get_context_aware_suggestions(
        self, current_file: Optional[str] = None, selected_text: Optional[str] = None, recent_errors: List[str] = None
    ) -> List[Dict[str, Any]]:
        """
    ...
    async def _find_similar_sequences(self, action_sequence: List[str], current_context: str) -> List[Dict[str, Any]]:
        ...
        similar = await self.mcp_client.query_context(
            query=query, top_k=10, threshold=0.6, context_types=["tool_usage", "action_sequence"]
        )

        return similar

    def _generate_action_predictions(
        self, similar_sequences: List[Dict[str, Any]], current_context: str
    ) -> List[Dict[str, Any]]:
    ...
    async def _enhance_predictions_with_context(
        self, predictions: List[Dict[str, Any]], current_context: str
    ) -> List[Dict[str, Any]]:
    ...
    async def _find_co_occurring_files(self, current_files: List[str]) -> List[Dict[str, Any]]:
        ...
        for file in current_files:
            ...
            related = await self.mcp_client.query_context(
                query=query, top_k=5, threshold=0.5, context_types=["file_access", "code_change"]
            )
    ...
    async def _get_files_from_similar_sessions(self, current_task: str) -> List[Dict[str, Any]]:
        ...
        similar_sessions = await self.mcp_client.query_context(
            query=query, top_k=5, threshold=0.6, context_types=["session_summary", "file_access"]
        )
        ...
        for session in similar_sessions:
            try:
                ...
                for file in session_files:
                    files.append(
                        {"file_path": file, "confidence": session.get("score", 0.5), "reason": "Used in similar task"}
                    )
            except json.JSONDecodeError:
                continue
    ...
    def _rank_file_suggestions(self, suggestion_groups: List[Tuple[str, List[Dict[str, Any]]]]) -> List[Dict[str, Any]]:
        ...
        source_weights = {"task_related": 1.0, "co_occurrence": 0.8, "similar_sessions": 0.6}
        ...
        for source, suggestions in suggestion_groups:
            ...
            for suggestion in suggestions:
                ...
                if file_path not in all_suggestions:
                    all_suggestions[file_path] = {"file_path": file_path, "confidence": 0.0, "reasons": []}
    ...
    def _analyze_code_context(
        self, code: str, cursor_position: Optional[Tuple[int, int]], file_path: str
    ) -> Dict[str, Any]:
        ...
        lines = code.split("\n")
        language = (
            self.repo_intelligence.language_extensions.get(Path(file_path).suffix, "other") if file_path else "other"
        )

        context = {"language": language, "file_path": file_path, "total_lines": len(lines), "surrounding_code": code}
    ...
    def _generate_code_completions(
        self, context_info: Dict[str, Any], similar_patterns: List[Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
        ...
        for pattern in similar_patterns:
            ...
            if pattern_content:
                ...
                completion = self._extract_completion_from_pattern(pattern_content, context_info)
                if completion:
                    suggestions.append(
                        {
    ...
    def _extract_completion_from_pattern(self, pattern: str, context: Dict[str, Any]) -> Optional[str]:
    ...
    async def _identify_learning_opportunities(self, current_situation: str, skill_level: str) -> List[Dict[str, Any]]:
    ...
    async def _get_selection_suggestions(self, selected_text: str, current_file: Optional[str]) -> List[Dict[str, Any]]:
    ...
    async def _get_error_suggestions(self, errors: List[str], current_file: Optional[str]) -> List[Dict[str, Any]]:
    ...
    def _deduplicate_suggestions(self, suggestions: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        unique_suggestions.sort(
            key=lambda x: (priority_order.get(x.get("priority", "low"), 1), x.get("confidence", 0)), reverse=True
        )
