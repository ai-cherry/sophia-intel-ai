...
class RepositoryIntelligence:
    ...
    def __init__(self, mcp_client: SophiaMCPClient, repo_root: Optional[str] = None):
    ...
    async def index_repository(
        self, force_refresh: bool = False, include_docs: bool = True, include_tests: bool = True
    ) -> Dict[str, Any]:
        """
    ...
    async def find_similar_code(
        self, code_snippet: str, language: Optional[str] = None, similarity_threshold: float = 0.7
    ) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            feature_query = " ".join(
                [
                    f"function:{feat}" if feat.startswith("def ") or feat.startswith("function ") else f"pattern:{feat}"
                    for feat in features[:5]  # Use top 5 features
                ]
            )

            results = await self.mcp_client.query_context(
                query=feature_query, top_k=15, threshold=similarity_threshold, context_types=["code_index"]
            )

            # Filter and rank results by actual code similarity
            similar_results = []
            for result in results:
                similarity_score = self._calculate_code_similarity(code_snippet, result.get("content", ""))
    ...
    async def _analyze_repo_structure(self) -> Dict[str, Any]:
        ...
        structure = {"directories": {}, "file_types": {}, "languages": {}, "total_files": 0, "total_lines": 0}
    ...
    async def _index_source_files(self, files: List[Path], force_refresh: bool) -> Dict[str, Any]:
        ...
        return {"files_indexed": indexed_count, "files_failed": failed_count, "total_files": len(files)}
    ...
    def _extract_python_structures(self, content: str) -> List[Dict[str, Any]]:
        ...
        for i, line in enumerate(lines):
            ...
            if re.match(r"^\s*def\s+(\w+)\s*\(", line):
                ...
                if match:
                    structures.append({"type": "function", "name": match.group(1), "code": line.strip(), "line": i + 1})

            # Class definitions
            elif re.match(r"^\s*class\s+(\w+)", line):
                match = re.search(r"class\s+(\w+)", line)
                if match:
                    structures.append({"type": "class", "name": match.group(1), "code": line.strip(), "line": i + 1})
    ...
    async def _find_related_files(self, file_path: str, content: str) -> List[str]:
        ...
        results = await self.mcp_client.query_context(
            query=query, top_k=10, threshold=0.6, context_types=["code_index"]
        )
    ...
    async def _analyze_code_patterns(self, files: List[Path]) -> Dict[str, Any]:
        ...
        patterns = {"common_imports": {}, "design_patterns": [], "function_patterns": {}, "naming_conventions": {}}
    ...
    def _analyze_architectural_patterns(self, context_entries: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        return {"mvc_patterns": 0, "singleton_patterns": 0, "observer_patterns": 0, "factory_patterns": 0}

    async def _calculate_complexity_metrics(self) -> Dict[str, Any]:
        """Calculate code complexity metrics"""
        return {"cyclomatic_complexity": 0, "coupling_metrics": {}, "cohesion_metrics": {}}
    ...
    async def _generate_recommendations(self) -> List[str]:
        ...
        return ["Consider refactoring large files", "Improve test coverage", "Review dependency structure"]
