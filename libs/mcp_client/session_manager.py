...
class SophiaSessionManager:
    ...
    def __init__(self, sessions_dir: str = ".sophia_sessions", mcp_port: int = 8000, max_sessions: int = 100):
    ...
    async def start_session(
        self, session_id: Optional[str] = None, project_context: Optional[Dict[str, Any]] = None
    ) -> str:
        """
            ...
            if target_session in self.active_sessions:
                ...
                if self.mcp_client:
                    await self.mcp_client.store_context(
                        ...
                        metadata={"session_duration": duration, "activity_count": active_session["activity_count"]},
                    )
    ...
    async def get_context_for_action(
        self, action_description: str, file_path: Optional[str] = None, max_context: int = 5
    ) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            context = await self.mcp_client.get_relevant_context(current_action=action_description, file_path=file_path)
    ...
    async def store_activity(
        self, activity_type: str, description: str, data: Optional[Dict[str, Any]] = None
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            result = await self.mcp_client.store_context(
                ...
                metadata={"activity_type": activity_type, "session_id": self.current_session},
            )
    ...
    async def list_sessions(self, active_only: bool = False, limit: int = 20) -> List[Dict[str, Any]]:
        """
        ...
        for session_id, metadata in self.session_metadata.items():
            ...
            session_info = {"session_id": session_id, "is_active": session_id in self.active_sessions, **metadata}
    ...
    async def cleanup_old_sessions(self, max_age_days: int = 30, keep_active: bool = True) -> int:
        """
    ...
    async def _load_session_context(self, session_id: str):
        ...
        try:
            if self.mcp_client:
                ...
                context = await self.mcp_client.query_context(f"session:{session_id}", top_k=10, threshold=0.5)
