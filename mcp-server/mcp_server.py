...
app = FastAPI(
    title="SOPHIA Intel MCP Server", description="Model Control Plane for Lambda Labs GH200 servers", version="1.0.0"
)
...
def verify_token(x_mcp_token: Optional[str] = Header(None)):
    ...
    if not x_mcp_token or x_mcp_token != MCP_AUTH_TOKEN:
        raise HTTPException(status_code=401, detail="Invalid or missing X-MCP-Token header")
    return True
...
async def get_server_stats(server_key: str, authenticated: bool = Depends(verify_token)):
    ...
    try:
        ...
        return {"server": server_key, "stats": stats, "config": server_config}
...
async def rename_servers(authenticated: bool = Depends(verify_token)):
    ...
    for server_key, server_config in SERVERS.items():
        try:
            result = lambda_client.rename_instance(server_config["id"], server_config["name"])
            results.append(
                {
                    "server": server_key,
                    "status": "success",
            ...
            results.append(
                {"server": server_key, "status": "error", "error": str(e), "instance_id": server_config["id"]}
            )
...
async def get_server_details(server_key: str, authenticated: bool = Depends(verify_token)):
    ...
    try:
        ...
        return {"server": server_key, "config": server_config, "instance": instance, "stats": stats}
