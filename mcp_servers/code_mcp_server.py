...
class GitHubTools:
    ...
    async def commit_file_change(
        self, repository: str, file_path: str, content: str, commit_message: str, branch: str
    ) -> Dict:
    ...
    async def create_pull_request(
        self, repository: str, title: str, description: str, head_branch: str, base_branch: str = "main"
    ) -> Dict:
...
class MCPCodeServer:
    ...
    def __init__(self):
        ...
        self.app = FastAPI(
            title="MCP Code Server", description="Dedicated coding agent with GitHub integration", version="1.0.0"
        )
        ...
        async def execute_code_operation(request: CodeRequest):
            ...
            try:
                ...
                if request.operation == "read_repo":
                    ...
                    data = await self.github_tools.create_branch(request.repository, request.branch, "main")
                    ...
                    data = await self.github_tools.commit_file_change(
                        request.repository, request.file_path, request.content, request.commit_message, request.branch
                    )
                ...
                return CodeResponse(
                    ...
                    data=data,
                    metadata={"timestamp": datetime.utcnow().isoformat(), "repository": request.repository},
                )

            except Exception as e:
                logger.error(f"Code operation failed: {e}")
                return CodeResponse(success=False, operation=request.operation, error=str(e))
