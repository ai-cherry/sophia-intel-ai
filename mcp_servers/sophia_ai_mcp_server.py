...
class SophiaAIMCPServer:
    ...
    def __init__(self):
        ...
        self.api_config = {
            "serper": {"key": os.getenv("SERPER_API_KEY"), "endpoint": "https://google.serper.dev/search"},
            "perplexity": {"key": os.getenv("PERPLEXITY_API_KEY"), "endpoint": "https://api.perplexity.ai/chat/completions"},
            "brave": {"key": os.getenv("BRAVE_API_KEY"), "endpoint": "https://api.search.brave.com/res/v1/web/search"},
            "exa": {"key": os.getenv("EXA_API_KEY"), "endpoint": "https://api.exa.ai/search"},
            "tavily": {"key": os.getenv("TAVILY_API_KEY"), "endpoint": "https://api.tavily.com/search"}
        }
                ...
                types.Tool(
                    ...
                    inputSchema={
                        ...
                        "properties": {
                            ...
                            "apis": {
                                "type": "array",
                                "items": {"type": "string", "enum": ["serper", "brave", "exa", "tavily"]},
                        ...
                        async with session.get(endpoint) as resp:
                            ...
                            if resp.status == 200:
                                ...
                                health_data["services"][service_name] = {
                                    ...
                                    "data": data if detailed else {"status": data.get("status", "ok")}
                                }
                                healthy_count += 1
                            else:
                                health_data["services"][service_name] = {
    ...
    async def sophia_search_functionality(self, query: str, max_results: int, apis: List[str]) -> List[types.TextContent]:
        ...
        try:
                ...
                async with session.post(
                    ...
                    if resp.status == 200:
                        ...
                        summary = {
                            ...
                            "primary_response_preview": result.get("primary_response", "")[:200] + "..." if result.get("primary_response") else "No response",
                            "source_breakdown": {}
                        }
            ...
            return [types.TextContent(
                type="text",
                text=f"Search test error: {str(e)}"
            )]

    async def get_performance_metrics(self, timeframe: str, service: str) -> List[types.TextContent]:
    ...
    async def check_deployment_status(self, component: str, include_history: bool) -> List[types.TextContent]:
    ...
    async def sophia_neural_inference(self, prompt: str, model: str, max_tokens: int) -> List[types.TextContent]:
        ...
        try:
                ...
                async with session.post(
                    ...
                    if resp.status == 200:
                        ...
                        summary = {
                            ...
                            "response_preview": result.get("response", "")[:200] + "..." if result.get("response") else "No response"
                        }
            ...
            return [types.TextContent(
                type="text",
                text=f"Neural inference test error: {str(e)}"
            )]

    async def troubleshoot_issue(self, issue: str, service: str, severity: str) -> List[types.TextContent]:
