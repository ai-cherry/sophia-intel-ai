    ...
    def export_to_local_env(self, output_file: Optional[Path] = None) -> bool:
        ...
        try:
            ...
            with open(output_file, 'w') as f:
                f.write(f"# Sophia AI Environment Variables\n")
...
def main():
    ...
    parser.add_argument('action', choices=[
        'login', 'open', 'get', 'set', 'rotate', 'sync', 'export', 'validate', 'template'
    ])
    parser.add_argument('--env', default='production', choices=['production', 'development', 'staging', 'test'])
    ...
    if args.action == 'login':
        ...
        if args.key:
            ...
            if value:
                print(f"✅ {args.key}: {value[:10]}..." if len(value) > 10 else f"✅ {args.key}: {value}")
            else:
