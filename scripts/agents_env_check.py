...
def check_environment_files() -> List[CheckResult]:
    ...
    # Check for  env in secure location
    _locations = [
    ...
    for loc in _locations:
        if loc.exists():
            results.append(
                CheckResult(status="ok", message=f"Found  env: {loc}", details={})
            )
            _found = True
            break
    if not _found:
        results.append(
            CheckResult(
                ...
                details={"remediation": "Run: make -setup"},
            )
        )
            ...
            results.append(
                CheckResult(
                    ...
                    details={
                        ...
                        "remediation": "Move AI keys to ~/.config//env",
                    },
                )
            )
        else:
