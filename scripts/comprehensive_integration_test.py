...
class ComprehensiveIntegrationTest:
    ...
    async def test_mcp_server_integration(self):
        ...
        try:
            ...
            test_entry = {
                "content": "Test memory entry for AI agent",
        ...
        try:
            ...
            async with httpx.AsyncClient() as client:
                ...
                if response.status_code == 200:
                    ...
                    self.log(f"✅ Together AI: {len(together_models)} embedding models available via OpenRouter", "SUCCESS")
            ...
            async with httpx.AsyncClient() as client:
                ...
                if response.status_code == 200:
                    ...
                    latest_models = {
                        ...
                        "claude_4": [m for m in model_data if "claude-4" in m.get("id", "") or "claude-sonnet-4" in m.get("id", "")],
                        "llama_4": [m for m in model_data if "llama-4" in m.get("id", "")],
                        "reasoning_models": [m for m in model_data if "reasoning" in m.get("description", "").lower()]
                    }
                    
                    total_models = len(model_data)
                    latest_count = sum(len(models) for models in latest_models.values())
                    
                    self.log(f"✅ OpenRouter: {total_models} total models, {latest_count} latest 2025 models", "SUCCESS")
                    ...
                    chat_response = await client.post(
                        ...
                        json={
                            ...
                            "messages": [{"role": "user", "content": "Hi, test AI swarm connection"}],
                            "max_tokens": 20
                        },
                        timeout=15.0
                    )
                    ...
                    if chat_response.status_code == 200:
                        ...
                        response_text = chat_result.get("choices", [{}])[0].get("message", {}).get("content", "")
                        
                        self.log(f"✅ OpenRouter Chat: AI response received - '{response_text[:30]}...'", "SUCCESS")
    ...
    async def test_portkey_virtual_keys(self):
        ...
        try:
            ...
            from app.api.advanced_gateway_2025 import get_advanced_gateway, TaskType
            ...
            if healthy_services > 0:
                self.log(f"✅ Portkey Gateway: {healthy_services}/{total_services} services healthy", "SUCCESS")
                ...
                self.log("⚠️  Portkey Gateway: No services healthy (virtual keys need dashboard setup)", "WARNING")
                self.results["portkey_virtual_keys"] = {
                    "status": "configured",
    ...
    async def test_agent_ui_integration(self):
        ...
        try:
            ...
            if os.path.exists(ui_env_path):
                ...
                self.log(f"✅ Agent UI: Configuration complete - port 8003: {has_8003}, API URL: {has_api_url}", "SUCCESS")
                
                # Check if package.json exists
                package_path = os.path.join(os.path.dirname(__file__), '..', 'agent-ui', 'package.json')
                has_package = os.path.exists(package_path)
                
                # Check if Dockerfile exists
                dockerfile_path = os.path.join(os.path.dirname(__file__), '..', 'agent-ui', 'Dockerfile')
    ...
    async def test_complete_workflow(self):
        ...
        try:
            ...
            vector_store_path = os.path.join(os.path.dirname(__file__), '..', 'pulumi', 'vector-store', 'src', 'modern_embeddings.py')
            has_vector_store = os.path.exists(vector_store_path)
            
            # Test memory system
            memory_system_path = os.path.join(os.path.dirname(__file__), '..', 'pulumi', 'mcp-server', 'src', 'unified_memory.py')
            has_memory_system = os.path.exists(memory_system_path)
            
            # Test API gateway
            gateway_path = os.path.join(os.path.dirname(__file__), '..', 'app', 'api', 'advanced_gateway_2025.py')
    ...
    async def run_comprehensive_test(self):
        ...
        self.log(f"❌ Failed: {total_tests - passed_tests}", "ERROR" if passed_tests < total_tests else "SUCCESS")
