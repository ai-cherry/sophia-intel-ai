...
# Sophia-Artemis Service Mesh Validation Script
...
NAMESPACE_ARTEMIS="artemis-mesh"
...
verify_mtls() {
    ...
    # Get a pod from Artemis namespace
    ARTEMIS_POD=$(kubectl get pod -n ${NAMESPACE_ARTEMIS} -l app=artemis-orchestrator -o jsonpath='{.items[0].metadata.name}' 2>/dev/null || echo "")
    ...
    if [ ! -z "${ARTEMIS_POD}" ]; then
        ...
        if [ ${MTLS_STATUS} -gt 0 ]; then
            log_success "mTLS is active for Artemis services"
        else
            log_warning "mTLS status unclear for Artemis services"
        fi
    else
        log_warning "No Artemis pods found to check mTLS"
    fi
}
...
test_service_connectivity() {
    ...
    # Test Artemis orchestrator
    log_info "Testing Artemis orchestrator connectivity..."
    ARTEMIS_POD=$(kubectl get pod -n ${NAMESPACE_ARTEMIS} -l app=artemis-orchestrator -o jsonpath='{.items[0].metadata.name}' 2>/dev/null || echo "")

    if [ ! -z "${ARTEMIS_POD}" ]; then
        # Test health endpoint
        if kubectl exec -n ${NAMESPACE_ARTEMIS} ${ARTEMIS_POD} -c artemis-orchestrator -- \
            curl -s -o /dev/null -w "%{http_code}" http://localhost:8080/health/live 2>/dev/null | grep -q "200"; then
            log_success "Artemis orchestrator health check passed"
        else
            log_error "Artemis orchestrator health check failed"
        fi
        ...
        if kubectl exec -n ${NAMESPACE_ARTEMIS} ${ARTEMIS_POD} -c artemis-orchestrator -- \
            ...
            log_success "Artemis can connect to shared services"
        else
            log_error "Artemis cannot connect to shared services"
        fi
    else
        log_warning "No Artemis pods available for connectivity test"
    fi
...
validate_policies() {
    ...
    if [ ${VS_ARTEMIS} -gt 1 ]; then
        log_success "VirtualServices configured for Artemis (${VS_ARTEMIS} found)"
    else
        log_error "No VirtualServices found for Artemis"
    fi
    ...
    if [ ${DR_ARTEMIS} -gt 1 ]; then
        log_success "DestinationRules configured for Artemis (${DR_ARTEMIS} found)"
    else
        log_error "No DestinationRules found for Artemis"
    fi
...
test_concurrent_tasks() {
    ...
    # Check Artemis configuration
    ARTEMIS_CONFIG=$(kubectl get configmap artemis-config -n ${NAMESPACE_ARTEMIS} -o jsonpath='{.data.application\.yaml}' 2>/dev/null || echo "")
    if echo "${ARTEMIS_CONFIG}" | grep -q "maxConcurrentTasks: ${MAX_CONCURRENT_TASKS}"; then
        log_success "Artemis concurrent task limit is configured (${MAX_CONCURRENT_TASKS})"
    else
        log_warning "Artemis concurrent task limit configuration not found"
    fi
...
check_service_mesh_metrics() {
    ...
    POD=$(kubectl get pod -n ${NAMESPACE_ARTEMIS} -l app=artemis-orchestrator -o jsonpath='{.items[0].metadata.name}' 2>/dev/null || echo "")
...
print_summary() {
    ...
    if [ ${TESTS_FAILED} -eq 0 ]; then
        ...
        echo "The Sophia-Artemis service mesh is properly configured."
    else
...
# Main execution
main() {
    log_info "Starting Sophia-Artemis Service Mesh validation..."
    echo
