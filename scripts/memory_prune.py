...
async def prune_memories(threshold: float = RELEVANCE_THRESHOLD, 
                         batch_size: int = PRUNE_BATCH_SIZE,
                         max_count: int = MAX_PRUNE_COUNT) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        if stale_count == 0:
            return {"status": "success", "message": "No stale memories found", "pruned": 0, "total": total_count}
            ...
            for memory in stale_memories:
                ...
                if memory_id:
                    try:
                        ...
                        pruned += 1
                        logger.debug(f"Pruned memory {memory_id} with relevance {memory.get('relevance')}")
...
def main():
    ...
    parser.add_argument("--batch", type=int, default=PRUNE_BATCH_SIZE,
                       help=f"Batch size for processing (default: {PRUNE_BATCH_SIZE})")
    parser.add_argument("--max", type=int, default=MAX_PRUNE_COUNT,
                       help=f"Maximum memories to prune (default: {MAX_PRUNE_COUNT})")
    parser.add_argument("--force", action="store_true",
                       help="Force pruning even if recently run")
    ...
    if not args.force and last_prune_file.exists():
        with open(last_prune_file, "r") as f:
            try:
