...
start_environment() {
    ...
    if [[ -n "$WANT_MCP" ]]; then
        ...
        docker compose -f "$COMPOSE_FILE" build mcp-memory mcp-filesystem-sophia mcp-filesystem-sophia mcp-git >/dev/null 2>&1 || true
        docker compose -f "$COMPOSE_FILE" up -d mcp-memory mcp-filesystem-sophia mcp-filesystem-sophia mcp-git
        sleep 5
    fi
