...
class ComprehensiveTestSuite:
    ...
    async def performance_test(self, service_name: str, url: str, requests: int = 10) -> Dict[str, Any]:
    ...
    async def run_all_tests(self):
        ...
        for service_name, url in self.services.items():
            ...
            print(f"  {service_name}: {result['status']} ({result.get('response_time_ms', 'N/A')}ms)")
        ...
        for service_name, url in self.services.items():
            ...
            if 'avg_response_time_ms' in result:
                print(f"  {service_name}: {result['avg_response_time_ms']}ms avg, {result['p95_response_time_ms']}ms p95")
            else:
        ...
        passed_tests = sum(1 for r in health_results if r['status'] == 'PASS') + \
