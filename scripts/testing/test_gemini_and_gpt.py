#!/usr/bin/env python3
"""
TEST GEMINI 2.5 FLASH (via OpenRouter) AND GPT-4O-MINI
To compare with the other 3 scouts
"""
import asyncio
import glob
import json
...
class GeminiGPTTest:
    def __init__(self):
        ...
        # Same prompt as the 4-scout test
        self.scout_prompt = f"""
...
- Key directories: /app, /sophia-intel-app, /scripts, /tests, /k8s
YOUR OBJECTIVES:
...
2. ARCHITECTURE ANALYSIS (Priority: HIGH)
   ...
   - Analyze the dual Sophia/ pattern
   - Find circular dependencies
   - Evaluate the swarm implementation
   - FOCUS ON: app//unified_factory.py, app/sophia/sophia_orchestrator.py
...
- 95% code duplication between Sophia and  factories
BE SPECIFIC. Use real file paths. Provide actionable recommendations.
This is a REAL repository scan, not a simulation.
"""
    async def test_gemini_openrouter(self):
        """Test Gemini 2.5 Flash via OpenRouter"""
        print("\n" + "=" * 70)
        print("🚀 TESTING GEMINI 2.5 FLASH VIA OPENROUTER")
        print("=" * 70)
        model = "google/gemini-2.0-flash-exp:free"  # OpenRouter's Gemini model
        start = time.time()
        try:
            async with httpx.AsyncClient(timeout=60) as client:
                ...
                if response.status_code == 200:
                    ...
                    print("✅ SUCCESS: Gemini 2.5 Flash via OpenRouter")
                    print(f"  Model: {model}")
    ...
    async def test_gpt4_mini(self):
        """Test GPT-4o-mini via Portkey"""
        print("\n" + "=" * 70)
        print("🚀 TESTING GPT-4O-MINI VIA PORTKEY")
        print("=" * 70)
        model = "gpt-4o-mini"
        start = time.time()
        try:
            async with httpx.AsyncClient(timeout=60) as client:
                ...
                if response.status_code == 200:
                    ...
                    tokens = data.get("usage", {}).get("total_tokens", 0)
                    exec_time = time.time() - start
                    print("✅ SUCCESS: GPT-4o-mini via Portkey")
                    print(f"  Model: {model}")
                    print(f"  Execution Time: {exec_time:.2f}s")
                    print(f"  Tokens Used: {tokens}")
                    print(f"  Response Length: {len(content)} chars")
    ...
    def score_response(self, response, exec_time):
        ...
        return scores
async def main():
    tester = GeminiGPTTest()
    # Test both models
    gemini_result = await tester.test_gemini_openrouter()
    gpt_result = await tester.test_gpt4_mini()
    ...
    if gemini_result.get("success"):
        previous_results["Gemini 2.5 Flash (OpenRouter)"] = {
            "score": gemini_result["scores"]["Overall"],
            "time": gemini_result["execution_time"],
            "tokens": gemini_result["tokens"],
        }
    if gpt_result.get("success"):
        previous_results["GPT-4o-mini"] = {
            "score": gpt_result["scores"]["Overall"],
            "time": gpt_result["execution_time"],
    ...
    with open(f"complete_5_model_comparison_{timestamp}.json", "w") as f:
        json.dump(
            {
