...
Tests both Sophia and  memory services
"""
import asyncio
import sys
import time
...
class RAGServiceTester:
    ...
    async def test_indexing(
        ...
        for i, doc in enumerate(config["test_data"], 1):
            try:
                endpoint = f"{config['url']}/index"
                if service_name == "" and "code" in doc:
                    endpoint = f"{config['url']}/index-code"
                response = await client.post(endpoint, json=doc)
                if response.status_code == 200:
        ...
        for query in config["test_queries"]:
            try:
                response = await client.post(
                    f"{config['url']}/query",
                    json={"query": query, "limit": 5, "include_context": True},
                )
                if response.status_code == 200:
                    result = response.json()
...
async def test_integration():
    ...
    async with httpx.AsyncClient(timeout=30.0) as client:
        ...
        try:
            response = await client.post("http://localhost:8768/index", json=code_doc)
            if response.status_code == 200:
                print("✅ Code context indexed in ")
        except Exception as e:
            print(f"⚠️  Could not index to : {e}")
        # Query both services for authentication
        await asyncio.sleep(1)
        print("\n📍 Querying both services for 'authentication'...")
        for service_name, port in [("Sophia", 8767), ("", 8768)]:
            try:
                response = await client.post(
...
async def main():
    ...
    if len(sys.argv) > 1:
        if sys.argv[1] == "--help":
            print(
                "Usage: python3 test_rag_services.py [sophia||integration|all]"
            )
            print("  sophia:      Test only Sophia BI Memory")
            print("  :     Test only  Code Memory")
    ...
    if test_mode == "sophia":
        ...
        await tester.test_service("", SERVICES[""])
