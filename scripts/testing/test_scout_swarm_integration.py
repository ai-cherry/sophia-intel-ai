...
# Import scout swarm
from app.swarms.scout_swarm.ultimate_scout_swarm import (
    ScoutSwarmIntegration,
    UltimateScoutSwarm,
)
# Import base factory
...
TEST_TARGETS = [
    "app//scout_swarm/ultimate_scout_swarm.py",
...
async def test_base_factory():
    ...
    try:
        # Create  factory
        _factory = create_base_factory("")
        print(f"✅  Factory Created: {_factory.factory_id}")
        # Initialize factory
        init_results = await _factory.initialize()
        ...
        # Create a test agent
        agent_id = await _factory.create_agent(
            name="Scout Alpha",
        ...
        # Get factory status
        status = _factory.get_factory_status()
        print("\n  Factory Status:")
        ...
        return {"success": True, "factory": _factory, "status": status}
    except Exception as e:
...
#  INTEGRATION TEST
# ==============================================================================
async def test__integration():
    """
    Test  factory integration with scout swarm
    """
    print("\n" + "=" * 70)
    print("⚔️  TESTING  FACTORY INTEGRATION")
    print("=" * 70)
    try:
        # Create scout agent through  integration
        agent_id = await ScoutSwarmIntegration.create_scout_agent("rapid")
        print(f"✅ Scout Agent Created: {agent_id}")
        # Execute tactical scan
        print("\n  Executing Tactical Scan...")
        tactical_report = await ScoutSwarmIntegration.execute_tactical_scan(
...
async def run_all_tests():
    ...
    # 6. Test  Integration
    _result = await test__integration()
    test_results["_integration"] = _result
    # Generate summary
    print("\n" + "=" * 70)
    print("📊 TEST SUMMARY")
    print("=" * 70)
