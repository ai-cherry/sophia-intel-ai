    ...
    try:
        ...
        for model in data:
            ...
            if cost_per_million > COST_THRESHOLD * 1000000:
                logger.debug(f"Skipping {model.get('id')}: too expensive (${cost_per_million/1000000:.4f}/1M tokens)")
                continue
        ...
        try:
            ...
            if (cache_dir / "model_rankings.json").exists():
                with open(cache_dir / "model_rankings.json", "r") as f:
                    return json.load(f)
...
def update_continue_config(models: List[Dict[str, Any]]) -> bool:
    ...
    try:
        ...
        coding_model = next((m for m in models if "coder" in m['id'].lower() or "code" in m['id'].lower()), models[0])
...
def main():
        ...
        with open(last_update_file, "r") as f:
            try:
                ...
                if hours_since_update < 6:  # Update every 6 hours
                    logger.info(f"Skipping update, last update was {hours_since_update:.1f} hours ago")
                    return
            except Exception:
    ...
    if success:
        ...
        try:
            ...
            model_count = len([m for m in config.get("models", []) if m.get("provider") == "openrouter"])

            if model_count >= 3:
                logger.info(f"Self-audit passed: {model_count} OpenRouter models configured")
            else:
                logger.warning(f"Self-audit warning: Only {model_count} OpenRouter models configured")
        except Exception as e:
