...
class ConfigValidator:
    ...
    def _validate_api_keys(self) -> None:
        ...
        for key_name, expected_prefix in required_keys.items():
            ...
            if any(placeholder in value.lower() for placeholder in 
                   ["your-", "test-", "dev-", "placeholder", "example", "change"]):
                self.errors.append(f"❌ {key_name} appears to be a placeholder value: {value[:20]}...")
                continue
                
            # Check format for specific keys
            if key_name == "OPENROUTER_API_KEY" and not value.startswith(expected_prefix):
                self.errors.append(f"❌ {key_name} should start with '{expected_prefix}', got: {value[:10]}...")
            elif key_name == "PORTKEY_API_KEY" and not value.startswith(expected_prefix):
                self.errors.append(f"❌ {key_name} should start with '{expected_prefix}', got: {value[:10]}...")
    ...
    def _validate_security_settings(self) -> None:
        ...
        for key_name in security_keys:
            ...
            if any(placeholder in value.lower() for placeholder in 
                   ["dev-", "test-", "development", "change", "replace", "production"]):
                self.errors.append(f"❌ {key_name} appears to be a development placeholder: {value[:20]}...")
                continue
                
            # Encryption key specific validation
            if key_name == "ENCRYPTION_KEY":
                if len(value) < 32:
                    self.errors.append(f"❌ {key_name} must be at least 32 characters long, got {len(value)}")
                elif len(value) != 32:
                    self.warnings.append(f"⚠️ {key_name} should be exactly 32 characters for AES-256, got {len(value)}")
                else:
                    logger.debug(f"✅ {key_name} length is valid")
            
            # General security key length validation
            elif len(value) < 16:
                self.errors.append(f"❌ {key_name} is too short, must be at least 16 characters, got {len(value)}")
            else:
        ...
        if not db_url:
            ...
            try:
                ...
                if parsed.scheme not in ["postgresql", "postgresql+asyncpg"]:
                    self.errors.append(f"❌ DATABASE_URL should use postgresql or postgresql+asyncpg scheme, got: {parsed.scheme}")
        ...
        if not redis_url:
            ...
            try:
                ...
                if parsed.scheme != "redis":
                    self.errors.append(f"❌ REDIS_URL should use redis scheme, got: {parsed.scheme}")
                elif not parsed.hostname:
        ...
        if not qdrant_url:
            ...
            try:
                ...
                if parsed.scheme not in ["http", "https"]:
                    self.errors.append(f"❌ QDRANT_URL should use http or https scheme, got: {parsed.scheme}")
                elif not parsed.hostname:
                    ...
                    if "qdrant.io" in parsed.hostname:
                        ...
                        if not api_key:
                            self.warnings.append("⚠️ Using Qdrant Cloud but QDRANT_API_KEY is not set")
    ...
    def _validate_service_endpoints(self) -> None:
        ...
        for endpoint_name, endpoint_url in endpoints.items():
            ...
            try:
                ...
                if parsed.scheme not in ["http", "https"]:
                    self.errors.append(f"❌ {endpoint_name} should use http or https scheme, got: {parsed.scheme}")
                elif not parsed.hostname:
    ...
    def _validate_agent_settings(self) -> None:
        ...
        if concurrency is None:
            ...
            self.errors.append(f"❌ AGENT_CONCURRENCY must be at least 1, got: {concurrency}")
        elif concurrency > 10:
            self.warnings.append(f"⚠️ AGENT_CONCURRENCY is very high ({concurrency}), may cause resource issues")
        ...
        if timeout is None:
            ...
            self.warnings.append(f"⚠️ AGENT_TIMEOUT_SECONDS is very low ({timeout}s), may cause timeouts")
        elif timeout > 900:
            self.warnings.append(f"⚠️ AGENT_TIMEOUT_SECONDS is very high ({timeout}s), may cause resource issues")
        ...
        if not agno_db:
            ...
            if not db_path.parent.exists():
                self.warnings.append(f"⚠️ AGNO_STORAGE_DB directory does not exist: {db_path.parent}")
    ...
    def _validate_environment_settings(self) -> None:
        ...
        if embedding_dim and embedding_dim not in [384, 768, 1536, 3072]:
            self.warnings.append(f"⚠️ Unusual EMBEDDING_DIMENSION: {embedding_dim}, common values are 384, 768, 1536, 3072")
