...
class GitHubApprovalService:
    ...
    async def create_guarded_action_check(
        self, sha: str, action_id: str
    ) -> Optional[GitHubApproval]:
        """
        ...
        print(
            f"Creating 'Guarded Action' check for SHA {sha} and action {action_id}..."
        )
...
if __name__ == "__main__":
    ...
    mock_config = {"github": {"token": os.getenv("GH_FINE_GRAINED_TOKEN")}}
    # Make sure to set GITHUB_REPOSITORY, e.g., "your-org/your-repo"
    # and a valid SHA for GITHUB_SHA
    os.environ["GITHUB_REPOSITORY"] = os.getenv(
        "GITHUB_REPOSITORY", "ai-cherry/sophia-intel"
    )
    ...
    async def main():
        ...
        approval = await approval_service.create_guarded_action_check(
            GITHUB_SHA, "pulumi-up"
        )
        if approval:
            ...
            # Simulate approval
            await approval_service.update_check_status(
                approval.check_run_id, approved=True
            )
