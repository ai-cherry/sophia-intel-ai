...
class Orchestrator:
    ...
    def __init__(self):
        ...
        # Service endpoints
        self.services = {
            "agno_api": f"http://localhost:7777",
    ...
    async def _handle_gpu_request(
        ...
        try:
            ...
            if request_type == "quota":
                ...
                instance_id = payload.get("instance_id")
                if not instance_id:
                    raise ValueError("instance_id is required for terminate_instance request")
                response = await self.lambda_client.terminate_instance(instance_id)
                
            elif request_type == "restart_instance":
                instance_id = payload.get("instance_id")
                if not instance_id:
                    raise ValueError("instance_id is required for restart_instance request")
                ...
                if not name or not public_key:
                    raise ValueError("name and public_key are required for add_ssh_key request")
    ...
    async def _handle_health_request(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            "overall_status": ServiceStatus.HEALTHY.value if all_healthy else ServiceStatus.DEGRADED.value,
