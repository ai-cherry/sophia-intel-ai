...
class PortkeyClient:
    ...
    async def chat(
        ...
        if self.portkey_key:
            ...
            if not self.openrouter_key:
                raise ValueError(
                    "Either PORTKEY_API_KEY or OPENROUTER_API_KEY must be provided"
                )
        ...
        for attempt in range(retries + 1):
            try:
                ...
                async with httpx.AsyncClient(timeout=timeout) as client:
                    logger.debug(
                        f"Sending chat request to {url} (attempt {attempt + 1})"
                    )
                ...
                last_exception = e
                logger.warning(
                    f"Chat request failed (attempt {attempt + 1}/{retries + 1}): {e}"
                )
        ...
        logger.error(
            f"Chat request failed after {retries + 1} attempts: {last_exception}"
        )
    ...
    async def embeddings(
        ...
        for attempt in range(retries + 1):
            try:
                ...
                async with httpx.AsyncClient(timeout=timeout) as client:
                    logger.debug(
                        f"Sending embeddings request to {url} (attempt {attempt + 1})"
                    )
                ...
                last_exception = e
                logger.warning(
                    f"Embeddings request failed (attempt {attempt + 1}/{retries + 1}): {e}"
                )
        ...
        logger.error(
            f"Embeddings request failed after {retries + 1} attempts: {last_exception}"
        )
        raise last_exception
    ...
    async def get_usage(self) -> Dict[str, Any]:
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=30) as client:
                response = await client.get(f"{self.base_url}/usage", headers=headers)
                response.raise_for_status()
                return response.json()
