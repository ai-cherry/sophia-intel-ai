...
class OptimizedCrossEncoderReranker:
    ...
    async def rerank_results(
        ...
        if not rerank_candidates:
            return self._fallback_ranking(prepared_results), {'warning': 'No candidates for reranking'}
        ...
        logger.debug(f"Reranking completed: {len(final_results)} results in {self._metrics.total_time_ms:.1f}ms")
    ...
    async def _perform_reranking(
        self,
        query: str,
        candidates: List[RerankingResult]
    ...
    def _combine_results(
        ...
        reranked.sort(
            key=lambda x: x.metadata.get('combined_score', x.rerank_score),
            reverse=True
        )
        ...
        return {
            ...
            'performance_metrics': {
                ...
                'inference_efficiency': (
                    self._metrics.reranked_results / max(0.001, self._metrics.inference_time_ms / 1000)
                ) if self._metrics.inference_time_ms > 0 else 0
            }
        }
