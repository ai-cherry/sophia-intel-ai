...
const ClientHealthDashboard: React.FC = () => {
  ...
  const connectWebSocket = () => {
    try {
      ws.current = new WebSocket((typeof window !== 'undefined' ? (window.location.protocol === 'https:' ? 'wss:' : 'ws:') + '//' + window.location.host : 'ws://localhost:3000') + '/ws/client-health-asclepius' + (() => { try { const t = window.localStorage.getItem('AUTH_TOKEN') || process.env.NEXT_PUBLIC_API_TOKEN; return t ? `?token=${encodeURIComponent(t)}` : ''; } catch { return ''; } })());
