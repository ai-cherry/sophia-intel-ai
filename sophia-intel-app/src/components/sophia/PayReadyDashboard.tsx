...
const useWebSocketConnection = (url: string) => {
  ...
  useEffect(() => {
  const wsUrl = (typeof window !== 'undefined' && window.location.protocol === 'https:' ? 'wss:' : 'ws:') +
    `//${typeof window !== 'undefined' ? window.location.host : 'localhost:3000'}/ws/sophia_dashboard/${Date.now()}`;
  const token = (typeof window !== 'undefined' && window.localStorage.getItem('AUTH_TOKEN')) || process.env.NEXT_PUBLIC_API_TOKEN;
...
const PayReadyDashboard: React.FC = () => {
  const base = (typeof window !== 'undefined' && window.location.protocol === 'https:' ? 'wss:' : 'ws:') +
    `//${typeof window !== 'undefined' ? window.location.host : 'localhost:3000'}`;
  const token2 = (typeof window !== 'undefined' && window.localStorage.getItem('AUTH_TOKEN')) || process.env.NEXT_PUBLIC_API_TOKEN;
  const { connected, data } = useWebSocketConnection(token2 ? `${base}/ws?token=${encodeURIComponent(token2)}` : `${base}/ws`);
  ...
  const deploySwarmEnhancements = useCallback(async () => {
    ...
    try {
      const response = await fetch('/api/swarms/sophia/deploy-sophia-enhancements', {
        method: 'POST',
    ...
    <div className="space-y-6 p-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-muted-foreground">Real-time operational intelligence powered by Sophia agent swarm</p>
        </div>
              ...
              <div className="space-y-4">
                ...
                {operationalIntelligence.length === 0 && (
                  <div className="text-center py-8">
                    ...
                    <p className="text-sm text-muted-foreground">Deploy the Sophia swarm to generate intelligence</p>
                  </div>
                )}
              </div>
            </CardContent>
