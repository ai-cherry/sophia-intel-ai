...
async def test_memory_bridge():
    ...
    for query in queries:
        try:
            results = await bridge.retrieve(
                agent_id="_test", query=query, limit=5, include_entities=True
            )
            logger.info(
    ...
    # Test memory stats
    try:
        stats = await bridge.get_stats("_test")
        logger.info(f"✅ Memory stats: {json.dumps(stats, indent=2)}")
    except Exception as e:
        logger.error(f"❌ Error getting memory stats: {str(e)}")
    # Test pruning memories
    try:
        pruned = await bridge.prune("_test", max_relevance=0.7)
    ...
    try:
        success = await bridge.clear("_test")
        if success:
