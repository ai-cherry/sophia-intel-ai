...
class SophiaV33TestSuite:
    ...
    def test_pulumi_esc_integration(self) -> Dict[str, any]:
        ...
        try:
            ...
            if esc_manager_path.exists():
                ...
                with open(esc_manager_path, 'r') as f:
                    content = f.read()
    ...
    def test_lambda_labs_integration(self) -> Dict[str, any]:
        ...
        try:
            lambda_integration_path = self.project_root / "infrastructure" / "lambda_labs" / "lambda_labs_integration.py"
            if lambda_integration_path.exists():
                ...
                with open(lambda_integration_path, 'r') as f:
                    content = f.read()
    ...
    def _calculate_category_score(self, category: str, results: List[Dict]) -> Dict[str, any]:
        ...
        print(f"  📊 {category}: {passed_tests}/{total_tests} tests passed ({score_percentage:.1f}%)")
    ...
    def generate_final_report(self, results: Dict[str, any]):
        ...
        for category, result in results.items():
            ...
            print(f"{status_icon} {category}: {result['score_percentage']:.1f}% ({result['passed']}/{result['total_tests']})")
        ...
        critical_failures = []
        for category, result in results.items():
            if result["status"] == "FAIL" and category in ["Security Fixes", "File Permissions", "Production Readiness"]:
    ...
    def _save_detailed_report(self, results: Dict[str, any], summary: Dict[str, any]):
        ...
        with open(md_report, 'w') as f:
            ...
            for category, result in results.items():
                ...
                f.write(f"**Score:** {result['score_percentage']:.1f}% ({result['passed']}/{result['total_tests']})\n\n")
                
                for test_result in result['results']:
                    test_icon = {"PASS": "✅", "FAIL": "❌", "ERROR": "⚠️"}.get(test_result['status'], "❓")
...
def main():
    ...
    overall_score = sum(r["passed"] for r in results.values()) / sum(r["total_tests"] for r in results.values()) * 100
