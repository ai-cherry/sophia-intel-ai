...
async def test_fs_read_augmentation():
    ...
    # Fake MCP client
    fake_client = SimpleNamespace(fs_read=lambda path, max_bytes=1000: {"content": "hello world"})

    async def run():
        # Patch detect_stdio_mcp to return fake client
        with patch("app.swarms.core.micro_swarm_base.detect_stdio_mcp", return_value=fake_client):
