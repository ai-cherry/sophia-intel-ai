...
def _factory():
    ...
    with patch("app..unified_factory.UnifiedFactory") as MockFactory:
        ...
        async def create_agent_impl(name, personality):
            return create_test_agent(name, "", personality)
        factory.create_agent.side_effect = create_agent_impl
        # Configure swarm creation
        factory.create_swarm.return_value = create__test_swarm()
        factory.create_tactical_swarm.return_value = create__test_swarm()
        yield factory
@pytest.fixture
def sophia_factory():
...
def domain_enforcer():
    ...
    with patch("app.core.domain_enforcer.DomainEnforcer") as MockEnforcer:
        ...
        enforcer.access_matrix = {
            "": {"sophia": False, "shared": True},
            "sophia": {"": False, "shared": True},
            "shared": {"": True, "sophia": True},
        }
        yield enforcer
# ============= MCP Servers =============
@pytest.fixture
def _mcp_servers() -> Dict[str, MockMCPServer]:
    """Create mock MCP servers for  domain"""
    return create__mock_servers()
@pytest.fixture
...
# ============= Test Agents =============
@pytest.fixture
def _test_agents() -> List[MockAgent]:
    """Create  test agents"""
    return create__test_swarm()
@pytest.fixture
...
# ============= Orchestrators =============
@pytest.fixture
def _orchestrator(_factory, domain_enforcer, mcp_router):
    """Mock  orchestrator"""
    with patch(
...
def pytest_configure(config):
    ...
    config.addinivalue_line(
        "markers", ": marks tests specific to  domain"
    )
