    ...
    async def setup_test_data(self):
        ...
        self.test_data = {
            ...
            "users": [
                ...
                TestUser(
                    "analyst_001", "analyst_api_key_456", "analyst", ["read", "write"]
                ),
...
class TestCompleteUserWorkflows:
    ...
    async def test_customer_onboarding_workflow(self, e2e_fixture):
        ...
        # Step 3: Create onboarding workflow in 
        ...
        async with fixture.session.post(
            ...
            fixture.cleanup_tasks.append(
                lambda: fixture.session.delete(
                    f"{fixture.env._url}/workflows/{workflow_id}",
                    headers=headers,
                )
            )
        ...
        async with fixture.session.get(
            f"{fixture.env._url}/workflows/{workflow_id}/results",
            headers=headers,
        ) as response:
            assert response.status == 200
        ...
        print(
            f"🎉 Customer onboarding workflow completed in {total_workflow_time:.2f} seconds"
        )
    ...
    async def test_business_intelligence_analysis_workflow(self, e2e_fixture):
        ...
        async with fixture.session.post(
            ...
            fixture.cleanup_tasks.append(
                lambda: fixture.session.delete(
                    f"{fixture.env.unified_mcp_url}/analysis/{analysis_id}",
                    headers=headers,
                )
            )
        ...
        # Step 5:  creates insight generation workflow
    ...
    async def test_real_time_collaboration_workflow(self, e2e_fixture):
        ...
        async with websockets.connect(websocket_uri) as websocket:
            ...
            if implementation_plans:
                approval = {
                    ...
                    "session_id": session_id,
                    "approved_plans": [
                        plan["plan_id"] for plan in implementation_plans
                    ],
        ...
        print(
            f"🌟 Real-time collaboration completed in {total_collaboration_time:.2f} seconds"
        )
        return WorkflowResult(
    ...
    async def test_system_failure_recovery_workflow(self, e2e_fixture):
        ...
        headers = {"Authorization": f"Bearer {admin_user.api_key}"}
        async with fixture.session.post(
            f"{fixture.env._url}/workflows/failure-test",
        ...
        assert len(recovery_actions) >= len(
            failure_events
        ), "Not all failures had recovery actions"
        ...
        for service_name, health_url in stability_tests:
            async with fixture.session.get(health_url, headers=headers) as response:
                assert (
                    response.status == 200
                ), f"{service_name} not stable after recovery"
        ...
        while waited_recovery < max_wait_recovery:
            async with fixture.session.get(
                f"{fixture.env._url}/workflows/{test_workflow_id}/status",
                headers=headers,
            ) as response:
                if response.status == 200:
        ...
        if final_workflow_status == "failed":
            async with fixture.session.get(
                f"{fixture.env._url}/workflows/{test_workflow_id}/error-details",
                headers=headers,
            ) as response:
                assert response.status == 200
        ...
        print(
            f"🛡️ Failure recovery test completed in {total_recovery_test_time:.2f} seconds"
        )
        return WorkflowResult(
...
class TestPerformanceValidation:
                ...
                try:
                    if request_type == "memory_query":
                        async with fixture.session.get(
                            f"{fixture.env.mem0_url}/memories/search",
                            params={
                                "query": f"test query {user_id}_{req_num}",
                                "limit": 5,
                            },
                            headers=headers,
                        ) as response:
                        ...
                        async with fixture.session.post(
                            f"{fixture.env._url}/workflows/quick-test",
                            json=payload,
                            headers=headers,
                        ) as response:
                            status = response.status
    ...
    async def test_memory_performance_scaling(self, e2e_fixture):
        ...
        query_time_increase = (
            largest_result["avg_query_time"] / baseline_result["avg_query_time"]
        )
        assert (
            query_time_increase <= 3.0
        ), f"Query time increased by {query_time_increase:.1f}x"
        return {
