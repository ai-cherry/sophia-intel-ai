...
import pytest
from app.sophia.unified_factory import SophiaUnifiedFactory as UnifiedFactory
...
class TestConcurrentOperations:
    ...
    async def test_eight_concurrent_task_limit_enforcement(
        self, _factory, sophia_factory
    ):
        """Test that both factories enforce the 8 concurrent task limit"""
        # Fill  factory to capacity
        _tasks = []
        for i in range(8):
            acquired = await _factory._acquire_task_slot()
            assert acquired is True
            _tasks.append(i)
        assert _factory._concurrent_tasks == 8
        ...
        # Clean up
        for _ in range(8):
            await _factory._release_task_slot()
    ...
    async def test_concurrent_agent_creation_across_domains(
        ...
        async def create__agents():
            ...
            for template in [
                ...
                agent_id = await _factory.create_technical_agent(template)
        ...
        _task = asyncio.create_task(create__agents())
        sophia_task = asyncio.create_task(create_sophia_agents())
        _agents, sophia_agents = await asyncio.gather(_task, sophia_task)
        # Verify all agents created
        assert len(_agents) == 3
        assert len(sophia_agents) == 3
        # Verify no ID collisions
        all_ids = _agents + sophia_agents
        assert len(set(all_ids)) == 6
    @pytest.mark.asyncio
    async def test_concurrent_mission_execution(self, _factory):
        """Test concurrent mission execution with task limit"""
        missions = []
        with patch.object(
            _factory, "_execute_mission_phase", new_callable=AsyncMock
        ) as mock_phase:
            mock_phase.return_value = {"success": True}
            # Create multiple missions concurrently
            async def execute_mission(mission_type, target):
                result = await _factory.execute_mission(
            ...
            ninth_result = await _factory.execute_mission(
    ...
    @pytest.mark.asyncio
    async def test_task_queue_under_load(self, _factory):
        """Test task queueing behavior under load"""
        # Fill all task slots
        for _ in range(8):
            await _factory._acquire_task_slot()
    ...
    async def test_circuit_breaker_under_concurrent_load(self, connection_manager):
        ...
        breaker = connection_manager.circuit_breakers["_filesystem"]
    ...
    async def test_resource_isolation_between_domains(
        ...
        # Verify isolation
        assert _factory._concurrent_tasks == 8  # Still full
    ...
    async def test_load_balancing_between_factories(
        ...
        """Test load distribution between  and Sophia factories"""
        load_metrics = {
            "_tasks": 0,
            "sophia_tasks": 0,
            "_time": 0,
        ...
        async def _work():
            start = time.time()
            agent = await _factory.create_technical_agent("code_reviewer")
            load_metrics["_tasks"] += 1
            load_metrics["_time"] += time.time() - start
            return agent
    ...
    async def test_dynamic_load_rebalancing(self, connection_manager):
        ...
        assert connection_counts["_filesystem"] >= 4  # ~40%
    ...
    async def test_orchestrator_coordination_under_load(
        ...
        coordination_events = []
        async def _orchestrated_task():
            with patch.object(
                _factory, "_execute_mission_phase", new_callable=AsyncMock
            ) as mock:
                mock.return_value = {"success": True}
                result = await _factory.execute_mission(
                    "rapid_response", target="/orchestrated/"
                )
                coordination_events.append(("", datetime.now(timezone.utc)))
                return result
        ...
        for i in range(6):
            if i % 2 == 0:
                task = asyncio.create_task(_orchestrated_task())
            else:
                task = asyncio.create_task(sophia_orchestrated_task())
            tasks.append(task)
        results = await asyncio.gather(*tasks)
        # Verify both orchestrators handled tasks
        _events = [e for e in coordination_events if e[0] == ""]
    ...
    async def test_sustained_concurrent_operations(
        ...
        duration_seconds = 2
        operations_count = {"": 0, "sophia": 0}
        errors_count = {"": 0, "sophia": 0}
        async def continuous__operations():
            end_time = time.time() + duration_seconds
            while time.time() < end_time:
                try:
                    agent = await _factory.create_technical_agent(
                        "code_reviewer"
                    )
                    operations_count[""] += 1
        ...
        tasks = [
            asyncio.create_task(continuous__operations()),
    ...
    @pytest.mark.asyncio
    async def test_burst_load_handling(self, _factory):
        ...
        async def burst_operation(index):
            try:
                # Try to acquire slot
                if await _factory._acquire_task_slot():
                    await asyncio.sleep(0.05)  # Simulate work
                    await _factory._release_task_slot()
                    return ("success", index)
                else:
                    # Queue the task
                    task_id = await _factory.queue_task(
    ...
    async def test_graceful_degradation_under_overload(self, connection_manager):
        ...
        connection_manager.health_status["_filesystem"] = False
        connection_manager.health_status["_code_analysis"] = False
        ...
        with pytest.raises(Exception) as exc_info:
            await connection_manager.get_connection("_filesystem")
