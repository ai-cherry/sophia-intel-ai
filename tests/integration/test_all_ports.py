...
class PortManager:
    ...
    async def test_port(self, port: int, config: Dict) -> Tuple[str, Optional[Dict]]:
    ...
    async def test_redis(self) -> Tuple[str, Optional[Dict]]:
    ...
    async def scan_all_ports(self):
        ...
        for (port, config), (status, info) in zip(PORT_REGISTRY.items(), results):
