...
class TestMCPConnectionManager:
    ...
    async def test_connection_manager_initialization(self, connection_manager):
        ...
        # Check pools created
        assert "_filesystem" in connection_manager.pools
        ...
        assert "_filesystem" in connection_manager.circuit_breakers
        assert "sophia_analytics" in connection_manager.circuit_breakers
        # Check retry managers created
        assert "_code_analysis" in connection_manager.retry_managers
    ...
    async def test_get_connection_with_circuit_breaker(self, connection_manager):
        ...
        mock_connection.server_name = "_filesystem"
        connection_manager.pools["_filesystem"].acquire = AsyncMock(
            return_value=mock_connection
        )
        connection = await connection_manager.get_connection("_filesystem")
        assert connection == mock_connection
        assert (
            connection_manager.connection_metrics["_filesystem"]["acquired"] == 1
        )
    @pytest.mark.asyncio
    async def test_get_connection_with_unhealthy_server(self, connection_manager):
        ...
        assert "_filesystem is unhealthy" in str(exc_info.value)
    @pytest.mark.asyncio
    async def test_get_connection_with_retry_on_failure(self, connection_manager):
        ...
        async def mock_acquire():
            ...
            return MagicMock(spec=Connection, server_name="_filesystem")
        connection_manager.pools["_filesystem"].acquire = mock_acquire
    ...
    async def test_release_connection(self, connection_manager):
        ...
        mock_connection = MagicMock(spec=Connection)
        mock_connection.server_name = "_filesystem"
        connection_manager.pools["_filesystem"].release = AsyncMock()
        await connection_manager.release_connection(mock_connection, "")
        connection_manager.pools["_filesystem"].release.assert_called_once_with(
            mock_connection
        )
        assert (
            connection_manager.connection_metrics["_filesystem"]["released"] == 1
        )
    @pytest.mark.asyncio
    ...
    async def test_connection_metrics_tracking(self, connection_manager):
        ...
        # Track acquisition
        connection_manager._track_connection_acquired("test_server", "")
        metrics = connection_manager.connection_metrics["test_server"]
        assert metrics["acquired"] == 1
        assert metrics["by_domain"][""]["acquired"] == 1
        # Track release
        connection_manager._track_connection_released("test_server", "")
        assert metrics["released"] == 1
        assert metrics["by_domain"][""]["released"] == 1
        # Track failure
        connection_manager._track_connection_failed("test_server", "")
        assert metrics["failed"] == 1
        assert metrics["by_domain"][""]["failed"] == 1
    @pytest.mark.asyncio
    async def test_get_status_comprehensive(self, connection_manager):
        ...
        # Check pool status
        assert "_filesystem" in status["pools"]
        pool_status = status["pools"]["_filesystem"]
    ...
    async def test_circuit_breaker_opens_after_failures(self, connection_manager):
        ...
        async def failing_acquire():
            raise Exception("Connection failed")
        connection_manager.pools["_filesystem"].acquire = failing_acquire
    ...
    async def test_auto_reconnect_after_circuit_recovery(self, connection_manager):
        ...
        mock_connection = MagicMock(spec=Connection, server_name="_filesystem")
        connection_manager.pools["_filesystem"].acquire = AsyncMock(
            return_value=mock_connection
        )
