...
class TestOpenAIIntegration:
    ...
    async def test_chat_completion(self, client):
        ...
        try:
            response = await client.chat.completions.create(
                model="gpt-4",
                messages=[
                    {"role": "user", "content": "Hello, this is a test. Please respond with 'OpenAI integration working'."}
                ],
                max_tokens=50,
                temperature=0.1
            )
    ...
    async def test_rate_limiting(self, client):
        ...
        try:
            ...
            print(f"✅ Rate limiting test passed - {len(successful_responses)}/5 requests successful")
