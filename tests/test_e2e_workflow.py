...
class TestE2EWorkflow:
    ...
    async def test_complete_agent_workflow(
        self, orchestrator, memory_service, coding_agent
    ):
        """
        ...
        with patch.object(orchestrator.openrouter, "chat") as mock_chat, patch.object(
            ...
            result = await orchestrator.handle_request(
                request_type="code", payload=request_payload, timeout=60, retries=2
            )
    ...
    async def test_memory_storage_and_retrieval(self, memory_service):
        ...
        query_results = await memory_service.query_context(
            session_id="test-session", query="Python decorators", top_k=5, threshold=0.5
        )
    ...
    async def test_orchestrator_error_handling(self, orchestrator):
        ...
        result = await orchestrator.handle_request(
            request_type="invalid_type", payload={}, timeout=30, retries=1
        )
    ...
    async def test_gpu_request_handling(self, orchestrator):
        ...
        with patch.object(orchestrator.lambda_client, "quota") as mock_quota:
            mock_quota.return_value = {
                "quota": {"gpu_time_remaining": 3600, "instance_limit": 5},
                "usage": {"gpu_time_used": 0, "instances_running": 0},
            }

            result = await orchestrator.handle_request(
                request_type="gpu", payload={"type": "quota"}, timeout=30, retries=2
            )
    ...
    async def test_agent_json_response_parsing(self, coding_agent):
        ...
        with patch("httpx.AsyncClient") as mock_client:
            ...
            result = await coding_agent.execute(
                "test-task-1",
                {"session_id": "test-session", "code": test_code, "query": test_query},
            )
    ...
    async def test_memory_session_management(self, memory_service):
        ...
        session1_results = await memory_service.query_context(
            session_id="session-1", query="session content", global_search=False
        )

        session2_results = await memory_service.query_context(
            session_id="session-2", query="session content", global_search=False
        )
