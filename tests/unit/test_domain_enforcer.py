...
class TestDomainEnforcer:
    ...
    @pytest.fixture
    def sample__request(self):
        """Create a sample  domain request"""
        return DomainRequest(
            id="test_req_001",
            user_id="dev_user_123",
            user_role=UserRole.DEVELOPER,
            target_domain=MemoryDomain.CODE,
    ...
    def test_enforcer_initialization(self, enforcer):
        ...
        # Check  operations
        assert OperationType.CODE_GENERATION in enforcer._operations
        assert OperationType.CODE_REVIEW in enforcer._operations
        assert OperationType.SECURITY_SCANNING in enforcer._operations
        assert OperationType.REPOSITORY_ANALYSIS in enforcer._operations
    ...
    def test_access_matrix_initialization(self, enforcer):
        ...
        assert matrix[(UserRole.ADMIN, MemoryDomain.CODE)] == AccessLevel.FULL
        assert matrix[(UserRole.ADMIN, MemoryDomain.SOPHIA)] == AccessLevel.FULL
        # Developer roles for 
        assert matrix[(UserRole.DEVELOPER, MemoryDomain.CODE)] == AccessLevel.FULL
        assert matrix[(UserRole.DEVOPS, MemoryDomain.CODE)] == AccessLevel.FULL
        assert matrix[(UserRole.ARCHITECT, MemoryDomain.CODE)] == AccessLevel.FULL
        ...
        assert (
            matrix[(UserRole.BUSINESS_ANALYST, MemoryDomain.CODE)]
            == AccessLevel.READ
        )
        assert matrix[(UserRole.DEVELOPER, MemoryDomain.SOPHIA)] == AccessLevel.READ
        # Readonly has read access to both
        assert matrix[(UserRole.READONLY, MemoryDomain.CODE)] == AccessLevel.READ
        assert matrix[(UserRole.READONLY, MemoryDomain.SOPHIA)] == AccessLevel.READ
    # ==============================================================================
    # ACCESS CONTROL TESTS
    # ==============================================================================
    def test_developer_access_to_(self, enforcer, sample__request):
        """Test that developers have full access to  domain"""
        result = enforcer.validate_request(sample__request)
    ...
    def test_business_analyst_limited_access_to_(self, enforcer):
        """Test that business analysts have only read access to  domain"""
        request = DomainRequest(
            id="test_req_004",
    ...
    def test_admin_full_access_both_domains(self, enforcer):
        ...
        result = enforcer.validate_request(_request)
        assert result.allowed is True
        assert result.access_level == AccessLevel.FULL
        # Test Sophia access
    ...
    # ==============================================================================
    def test_cross_domain_violation__operation_in_sophia(self, enforcer):
        """Test detection of  operation attempted in Sophia domain"""
        request = DomainRequest(
            id="test_req_008",
            user_id="analyst_user",
            user_role=UserRole.BUSINESS_ANALYST,
            target_domain=MemoryDomain.SOPHIA,
            operation_type=OperationType.CODE_GENERATION,  #  operation!
        )
        result = enforcer.validate_request(request)
        assert result.allowed is False
        assert "Operation code_generation not allowed in SOPHIA domain" in result.reason
        assert result.suggested_domain == MemoryDomain.CODE
        assert enforcer.validation_stats["violations_detected"] == 1
        assert enforcer.validation_stats["denied_requests"] == 1
    def test_cross_domain_violation_sophia_operation_in_(self, enforcer):
        """Test detection of Sophia operation attempted in  domain"""
        request = DomainRequest(
            id="test_req_009",
            user_id="dev_user",
            user_role=UserRole.DEVELOPER,
        ...
        assert result.allowed is False
        assert (
            "Operation sales_analytics not allowed in  domain" in result.reason
        )
    ...
    def test_unauthorized_role_access(self, enforcer):
        ...
        result = enforcer.validate_request(request)
        assert result.allowed is False
        assert "User role sales has no access to  domain" in result.reason
    ...
    def test_create_cross_domain_request(self, enforcer):
        ...
        request = enforcer.create_cross_domain_request(
            source_domain=MemoryDomain.CODE,
            target_domain=MemoryDomain.SOPHIA,
        ...
        assert request.source_domain == MemoryDomain.CODE
    ...
    # AUDIT LOGGING TESTS
    # ==============================================================================
    def test_audit_logging_for_allowed_request(self, enforcer, sample__request):
        """Test that audit logs are created for allowed requests"""
        result = enforcer.validate_request(sample__request)
        assert result.audit_logged is True
        assert len(enforcer.audit_logs) == 1
        log = enforcer.audit_logs[0]
        assert log.request_id == sample__request.id
        assert log.domain == MemoryDomain.CODE
    ...
    def test_get_audit_logs_with_filtering(self, enforcer):
        ...
        requests = [
            DomainRequest(
                ...
                target_domain=(
                    MemoryDomain.CODE if i % 2 == 0 else MemoryDomain.SOPHIA
                ),
                operation_type=(
        ...
        _logs = enforcer.get_audit_logs(domain=MemoryDomain.CODE)
        assert all(log.domain == MemoryDomain.CODE for log in _logs)
    ...
    def test_disable_audit_logging(self, enforcer, sample__request):
    ...
    def test_get_domain_summary(self, enforcer):
        ...
        # Check  domain
        assert (
            summary["_domain"]["description"] == "Repository and code operations"
        )
        assert OperationType.CODE_GENERATION in summary["_domain"]["operations"]
        assert UserRole.DEVELOPER.value in summary["_domain"]["allowed_roles"]
        assert UserRole.ADMIN.value in summary["_domain"]["allowed_roles"]
        # Check Sophia domain
    ...
    def test_get_domain_access_summary_convenience_function():
        ...
        assert dev_summary["_access"] == AccessLevel.FULL.value
        ...
        assert analyst_summary["_access"] == AccessLevel.READ.value
