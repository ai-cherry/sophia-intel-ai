...
class RepoAuditor:
    ...
    def analyze_code_duplication(self) -> Dict:
        ...
        duplicate_lines = sum(
            sum(b["lines"] for b in blocks) for blocks in duplicates.values()
        )
    ...
    def analyze_config_consistency(self) -> List[Dict]:
        ...
        for key, sources in config_keys.items():
            ...
            definitions = [
                s for s in sources if s["type"] in ["yaml_definition", "env_definition"]
            ]
                ...
                for node in ast.walk(tree):
                    if isinstance(node, (ast.ClassDef, ast.FunctionDef)):
                        ...
                        if "agent" in name_lower:
                            responsibilities.add("agent_system")
                        if any(
                            word in name_lower for word in ["memory", "embed", "vector"]
                        ):
                            responsibilities.add("memory_system")
                        if any(word in name_lower for word in ["config", "setting"]):
                            responsibilities.add("configuration")
                        if any(
                            word in name_lower for word in ["client", "api", "service"]
                        ):
                            responsibilities.add("api_service")
    ...
    def find_unused_code(self) -> Dict:
        ...
        try:
            result = subprocess.run(
                ["ruff", "check", "--select=F401", "--format=json", "."],
                capture_output=True,
                text=True,
                timeout=30,
            )
    ...
    def capture_lint_baseline(self) -> Dict:
        ...
        for tool, cmd in tools.items():
            try:
                ...
                results[tool] = {"exit_code": -1, "error": str(e), "passed": False}
            ...
            [
                {
                    ...
                    "description": "Remove unused imports found by ruff",
                },
                {
                    "action": "standardize_formatting",
...
def main():
    ...
    print("=" * 60)
    print(
        f"📊 Duplication: {report['duplication_analysis']['duplication_percentage']}%"
    )
